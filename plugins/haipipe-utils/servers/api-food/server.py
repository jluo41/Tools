...
describe-food as a SERVICE. The consumer holds a URL, not an import.
    ...
    python servers/_host/serve.py --only food        # the plugin host, mounts this at /food
    uvicorn server:app --app-dir servers/api-food    # or standalone, skills/describe-food on PYTHONPATH
    curl -s localhost:8070/food/healthz
...
async def normalize_image(files: List[UploadFile] = File(...),
    ...
    photographed before and after eating, which is how CGMacros logs one.
        ...
        curl -F files=@before.jpg -F files=@after.jpg \\
             http://127.0.0.1:8070/food/normalize/image
    """
    if not files:
