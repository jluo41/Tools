...
visible from inside Python.
    ...
    FOODNORM_URL=http://host:8070/food python test_server.py
"""
import json
import os
import pathlib
...
URL = os.environ.get("FOODNORM_URL", "http://127.0.0.1:8070/food").rstrip("/")
PASS, FAIL = [], []
...
def t_real_cook():
    import glob
    import pandas as pd
    # The frame lives in a SPACE's SourceStore. Find it from $LOCAL_SOURCE_STORE or
    # from a SPACE root above this file; with neither, skip like the image tests do,
    # instead of dying in next() on a bare Tools checkout.
    _root = next((a for a in pathlib.Path(__file__).resolve().parents
                  if (a / "pyproject.toml").exists() and (a / "code").is_dir()), None)
    declared = os.environ.get("LOCAL_SOURCE_STORE")
    if declared:
        d = pathlib.Path(declared)
        store = d if d.is_absolute() else (_root or pathlib.Path.cwd()) / d
    elif _root is not None:
        store = _root / "_WorkSpace" / "1-SourceStore"
    else:
        return "SKIPPED (no SPACE root above this file and no LOCAL_SOURCE_STORE)"
    paths = sorted(glob.glob(str(store / "Shanghai/@*/Diet.parquet")))
    if not paths:
