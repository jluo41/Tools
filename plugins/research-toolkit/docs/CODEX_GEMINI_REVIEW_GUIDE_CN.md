...
这份指南是对上游 `skills/00_meta/skills-codex/` 的**叠加方案**，不是替换。

## 架构

- 基础 skill 集：`skills/00_meta/skills-codex/`
- 审稿覆盖层：`skills/00_meta/skills-codex-gemini-review/`
- 审稿 bridge：`mcp-servers/gemini-review/`

安装顺序很重要：

1. 先安装 `skills/00_meta/skills-codex/*`
2. 再安装 `skills/00_meta/skills-codex-gemini-review/*`
...
- **全量 overlay 覆盖检查**：`skills/00_meta/skills-codex-gemini-review/` 覆盖的 `15` 个预定义 reviewer-aware Codex skill 都逐一核对过，确认已经指向 `gemini-review`，不再依赖旧的 reviewer transport。
...
其它能力仍然来自上游 `skills/00_meta/skills-codex/`。
...
- 基础能力继续复用 `skills/00_meta/skills-codex/*`
- 只在 `skills/00_meta/skills-codex-gemini-review/*` 里覆盖 reviewer-aware skills
