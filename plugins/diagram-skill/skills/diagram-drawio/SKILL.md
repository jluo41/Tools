...
- **User specifies the path when invoking this skill.** Example: *"make a sequence diagram at `examples/hainn-v0319/diagrams/inference.drawio`"*.
- If no path given: default to CWD, confirm with user before writing.
- If only a directory given: pick a sensible filename (e.g. `architecture.drawio`, `flow.drawio`).
- Exports (`.png` / `.svg` / `.pdf`) land next to the source `.drawio` by default unless a separate path is specified.
