...
- **User specifies the path when invoking this skill.** Example: *"sketch the system flow and save to `examples/hainn-v0319/diagrams/flow.excalidraw`"*.
- If no path given: default to CWD, confirm with user before writing.
- If only a directory given: pick a sensible filename (e.g. `architecture.excalidraw`, `flow.excalidraw`).
- Rendered exports (e.g. `.svg`, `.png`) land next to the source `.excalidraw` by default unless a separate path is specified.
