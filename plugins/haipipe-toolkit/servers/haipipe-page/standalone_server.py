...
# Page's own grammar first: the importer that binds `src` decides which grammar
# this process runs on, and a standalone Page must never pick up the Board's.
from src import page_workspace  # noqa: E402  (cli/page.py puts the Page engine on sys.path)

_HOST = Path(__file__).resolve().parents[1] / '_host'
if str(_HOST) not in sys.path:
    sys.path.insert(0, str(_HOST))
from live.delivery import DeliveryTabMixin  # noqa: E402
from live.evidence import EvidenceTabMixin  # noqa: E402
from live.folderstat import FolderStatMixin  # noqa: E402
...
class PageHandler(OutlineMixin, EvidenceTabMixin, ValueMixin, FolderStatMixin,
    ...
    def _workbench_short(self, parsed):
        tab = parsed.path[2:].strip('/').lower()
        if '/' in tab or tab not in self.WORKBENCH_TABS:
            return self._error(404, 'unknown workbench tab; one of ' +
                               ', '.join(sorted(k for k in self.WORKBENCH_TABS if k)))
        page_rel = self.server.context.source.relative_to(self.root).as_posix()
        target = '/_board/%s?path=%s&file=%s' % (
            self.WORKBENCH_TABS[tab], quote(page_rel, safe='/'), quote(page_rel, safe='/'))
        if parsed.query:
            target += '&' + parsed.query
    ...
    def do_POST(self):
        ...
        try:
            ...
            with self.server.write_lock:
                ...
                if route not in workbenches:
                    return self._error(404, 'Unsupported Page endpoint')
                result, err = workbenches[route](payload)
    ...
    def labeling_view(self, head_only=False):
        ...
        module = _labeling_workbench_module()
        if module is None or not module.applicable(page_src):
            return self.reply(404, {'ok': False, 'err': 'Page has no available Labeling workbench'})
    ...
    def plug_labeling(self, payload):
        ...
        module = _labeling_workbench_module()
        if module is None or not module.applicable(page_src):
            return None, 'Page has no available Labeling workbench'
...
def serve(context, host='127.0.0.1', port=0, token=None,
    ...
    origin = urlsplit(server.startup_url)._replace(path='', query='', fragment='').geturl()
    print(f'Workbench:   {origin}/w   (tabs: /w/runs · /w/delivery · /w/folder)\n'
          f'DOMAIN:      {origin}   · pass --public-url to print the Tailscale or public origin instead',
          flush=True)
    try:
