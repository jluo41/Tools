...
from standalone_server import create_server, serve
...
class StandaloneServerTests(unittest.TestCase):
    ...
    def test_root_get_head_and_assets_without_board(self):
        ...
        self.assertIn(b'id="page-workbench-button"', body)
        self.assertIn(b'id="page-workbench-pane"', body)
        self.assertNotIn(b'id="source-editor"', body)
        self.assertNotIn(b'href="#source-editor"', body)
        self.assertIn('📃 Page'.encode(), body)
    ...
    def test_external_labeling_workbench_mounts_without_leaking_into_page_run_workspace(self):
    ...
    def test_serve_prints_token_free_login_url(self):
        ...
        with patch('standalone_server.create_server', return_value=fake), \
                redirect_stdout(output), redirect_stderr(errors):
            serve(self.context, token='private token/+')
        lines = output.getvalue().splitlines()
        self.assertEqual(lines[0], f'Page server: {self.origin}/_page/login')
        self.assertEqual(lines[1], f'Workbench:   {self.origin}/w   (tabs: /w/runs · /w/delivery · /w/folder)')
        self.assertTrue(lines[2].startswith(f'DOMAIN:      {self.origin}'), lines[2])
    ...
    def test_outline_keeps_prose_without_editors_in_every_mode(self):
        ...
        # Revise is the second bounded write: one form per paragraph, one box
        # per Bullet Draft, hidden outside the Revise view.
        self.assertEqual(writable.count('<form class="revise-form"'), 1)
        self.assertIn('data-revise-save', writable)
        self.assertIn('class="revise-box" data-bullets="[{&quot;address&quot;:&quot;C1.P1.B1&quot;', writable)
        ...
        self.assertNotIn('data-revise-save', body.decode())
        self.assertIn('class="revise-box" data-bullets=', body.decode())
        self.assertNotIn('data-structure-edit', body.decode())
    ...
    def test_imports_are_independent_in_fresh_process(self):
        script = ('import sys; from pathlib import Path; '
                  ...
                  'from standalone_server import create_server; '
        ...
        result = subprocess.run([sys.executable, '-c', script, str(PAGE_ROOT), str(self.source), str(SERVER_DIR)],
