...
_MARK = (Path(__file__).resolve().parent / "assets" / "board-mark.svg")
...
/* One rule for BOTH bar menus (JL 260808: Workbench and Workflow). `left` is set from the
...
.rp-sp{flex:1}
/* The page's applicable category workbenches are direct tabs.  The list can be
...
<div id="bar">
  ...
  <button id="mplugbtn" type="button" aria-haspopup="menu" aria-expanded="false"
    title="Surfaces this page can open, to the right">🔌 Workbench ▾</button>
  ...
  <!-- 🔌 A workbench surface the OPEN PAGE contributes. The shell hides the page's own
       FAB (see pane-page CSS), so without this row a registered workbench is unreachable
...
<div id="split">
  ...
  <div id="rp">
    <!-- 💬 ONE CHAT (JL 260815: "just have one Chat in the workbench, not more
    ...
    <!-- 🧩 TABS ARE AN OPEN SET, PER PAGE (haipipe-workbench, JL 260815): the
         ...
         into #rptset from the set, so a new workbench never edits this markup. -->
    <div id="rptabs" role="tablist">
      <span id="rptset"></span>
      <button id="rpplus" type="button" title="open a workbench of this page">＋</button>
...
(function () {
  ...
  function mirror() {
    ...
    /* ...AND SO DOES EVERY OTHER TAB (JL 260816: "when I change the page, the
       workbench is not changed accordingly — it is not attached to the focal
  ...
  /* ② THE REFRESH IS NOT HERE. Each pane asks about its own URL and reloads
     itself (`servers/_host/assets/js/20-live-refresh.js`, guarded by `window.__boardPane`),
  ...
  /* A first-time reader sees the direct workbench strip immediately.  A saved
  ...
  /* GUI is the form a fresh reader gets (JL 260831: "if choose the Chat
     Workbench, make the GUI the default"); a stored choice still wins. */
  var wanted = 'gui';
  ...
  function pageEntries(menu) {
    var w = pageWin(), rows = [];
    try {
      if (w && w.boardWorkbenches) {
        w.boardWorkbenches.applicable(w.boardWorkbenches.livePage(), menu).forEach(function (e) {
          ...
          rows.push({ id: e.id, label: e.label, hint: e.hint || '',
                      run: function () { e.open(w.boardWorkbenches.livePage()); } });
        });
      }
    } catch (err) {}
    return rows;
  ...
  /* 🔌 WORKBENCH · surfaces, which open to the RIGHT. The two chats are the SHELL's and
  ...
  function plugEntries() {
    ...
    /* Registry tabs (haipipe-workbench): each row opens its right-pane tab
       through the same showTab the strip uses — one opener, one owner. */
    xdefs().forEach(function (e) {
      rows.push({ id: e.id, label: e.label, hint: e.hint || '',
                  run: function () { showTab(e.id); } });
    });
    var all = rows.concat(pageEntries('workbench'));
    /* THE DEFAULT LEADS THE MENU (JL 260831: "make the outline the first
       workbench and the default workbench"). rankDefault() has put it first in the
  ...
  function wireMenu(btn, menu, entries) {
    ...
    /* An EMPTY menu hides its button rather than opening onto nothing. Most pages have
       ...
       not-applicable. The Workbench button always has the two chats, so it never hides. */
    return function () {
  ...
  /* ── 🗂 the tab strip ────────────────────────────────────────────────────────
     TABS ARE AN OPEN SET, PER PAGE (haipipe-workbench, JL 260815): the strip
     ...
     registry entry carrying `tab: {url, write}`, so workbench N+1 ships by
  ...
  function drawURL() {
    try {
      var w = frames.page;
      if (!w || !w.boardDrawOwner || !w.boardWorkbenches) return '';
      var o = w.boardDrawOwner(w.boardWorkbenches.livePage());
      return (o && o.url) || '';
    } catch (e) { return ''; }
  }
  ...
  /* Slides joins the strip the way Draw did (JL 260815: "why not together with
     them"): the URL comes from the page's own workbench; the deck itself is
  ...
  function slidesURL() {
    try {
      ...
      return w.boardSlidesURL(w.boardWorkbenches && w.boardWorkbenches.livePage()) || '';
    } catch (e) { return ''; }
  }
  ...
  function loadDeck(surl) {
    ...
    var noDeck = function () {
      ...
      fs.setAttribute('srcdoc',
        ...
        + 'author one from the page into its slide/ workbench.</p></div></body>');
    };
  ...
  /* Registry entries carrying a `tab` spec — the workbench contract's whole
  ...
  function xdefs() {
    ...
    try {
      var w = pageWin();
      if (w && w.boardWorkbenches) {
        /* The strip, top Workbench menu, and ＋ picker are all doors for THIS
        ...
        w.boardWorkbenches.applicable(w.boardWorkbenches.livePage()).forEach(function (e) {
  ...
  /* THE DIRECT WORKBENCH STRIP, per page.  JL asked to SEE the page's category
     workbenches — Outline · Studio · Evidence · Runs · Delivery … — rather than
  ...
  /* 🧭 THE DEFAULT TAB IS THE REGISTRY'S, NOT CHAT (JL 260830: "the outline
     ...
     `boardWorkbenches.setDefault('outline')` since 260818, but only the FAB read
  ...
  function defaultTab() {
    try {
      var w = pageWin();
      var d = w && w.boardWorkbenches && w.boardWorkbenches.getDefault(w.boardWorkbenches.livePage());
  ...
  function loadSet() {
    ...
    /* Every applicable category is open on a first visit.  `xdefs()` already
       applies each workbench's own gate and registry order; later visits retain
  ...
  function aimTab(which) {
    ...
    try { page = w && w.boardWorkbenches && w.boardWorkbenches.livePage(); } catch (e) {}
    var u = '';
  ...
  /* LANDING IS FRESH BY CONTRACT (JL 260816: "make sure the folder workbench
  ...
  function landFrame(f, u) {
    /* Inspect the URL PATH, not the raw string. A live workbench endpoint can end
       ...
       inside its query, corrupting the page binding and leaving the workbench
       frame blank. */
    var src = u;
    try {
  ...
  function directOutlineOwnsPage() {
    ...
    try {
      ...
      var w = pageWin(), page = w && w.boardWorkbenches && w.boardWorkbenches.livePage();
  ...
  function showTab(which, directURL) {
    /* A compact Page Run owns a precise place inside Outline.  Accept only a
       ...
       currently active workbench and losing its focus/run query in the swap. */
    ...
    /* A REGISTRY TAB (haipipe-workbench): frame the saved artifact, or ask
  ...
  function closeTab(id) {
    ...
    saveSet();
    if (tab === id) {
      /* Pick a REAL neighbor.  Persisted sets may still name a workbench that
  ...
  /* ＋ · WHAT THIS PAGE COULD OPEN (JL 260815: a tab appears on an explicit
     click). One row per offerable-but-closed workbench; ● marks the ones whose
  ...
  /* The direct strip supersedes the closed-workbench picker.  Leave the menu
  ...
  if (plus && pmenu) plus.onclick = function () {
    ...
    if (!rows.length) {
      pmenu.innerHTML = '<div class="xrow" style="cursor:default;color:var(--mut)">'
        + 'every workbench of this page is open</div>';
    } else {
      ...
      [].forEach.call(pmenu.querySelectorAll('button.xrow'), function (b) {
        ...
        try {
          if (r.id === 'draw') u = drawURL();
            ...
            var w = pageWin();
            u = r.def.tab.url(w && w.boardWorkbenches && w.boardWorkbenches.livePage()) || '';
          }
        } catch (e) {}
  ...
  reaimOK = true;
  var requestedWorkbench = '';
  try { requestedWorkbench = new URLSearchParams(location.search).get('workbench') || ''; }
  catch (e) {}
  if (requestedWorkbench && offerable(requestedWorkbench)) {
    setTimeout(function () { showTab(requestedWorkbench); }, 350);
  }
