  ...
  function pickOpen() {
    ...
    // a workbench contributes its own surface, and an entry that cannot act on the open
    // page is never drawn. `data-v` stays the id so the existing handler still reads it.
    var page = window.boardWorkbenches ? window.boardWorkbenches.livePage() : null;
    ...
    function group(title, menu) {
      var rows = window.boardWorkbenches
        ? window.boardWorkbenches.applicable(page, menu) : [];
    ...
    pick.innerHTML = group('\u{1F50C} Workbench', 'workbench');
    pick.hidden = false;
    ...
    pick.querySelectorAll('.pk').forEach(function (b) {
      b.onclick = function () {
        var id = b.dataset.v;
        var hit = (window.boardWorkbenches ? window.boardWorkbenches.all() : [])
                    .filter(function (e) { return e.id === id; })[0];
        pickClose();
        if (hit) hit.open(window.boardWorkbenches.livePage());
      };
    });
  ...
  if (window.boardWorkbenches) {
    window.boardWorkbenches.register({
      ...
      open: function () {
        ...
        window.location.assign(location.pathname + '?split&workbench=studio');
      } });
  }

  /* JL 260818: "how to make the outline be the default workbench when we open
  ...
  fabMore.type = 'button';
  fabMore.setAttribute('aria-label', 'Open the workbench picker');
  fabMore.title = 'Workbenches';
  fabMore.textContent = '⋯';
  ...
  fab.onclick = function () {
    ...
    var page = window.boardWorkbenches ? window.boardWorkbenches.livePage() : null;
    var def = window.boardWorkbenches ? window.boardWorkbenches.getDefault(page) : null;
  ...
  function fabLbl() {
    ...
    fab.innerHTML = def ? def.label
                  ...
                  : (tgt.group ? '\u{1F916} Group chat' : 'Workbenches');
  }
