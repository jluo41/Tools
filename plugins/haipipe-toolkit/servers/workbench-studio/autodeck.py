...
endpoint behind both, the slide workbench's twin of autodraw.
...
from host_paths import SKILLS, bootstrap
bootstrap()                                   # the Board engine's src/ grammar
...
def autodeck(root, payload):
    ...
    if md_path.suffix != ".md" or md_path.parent.name != md_path.stem:
        return _fail("auto-deck only writes for a folded page "
                     "(<name>/<name>.md owns the slide/ workbench)")
    ...
    # live/ -> haipipe-board -> board -> skills, then the display workbench.
    skill = SKILLS / "display" / "html-ppt"
