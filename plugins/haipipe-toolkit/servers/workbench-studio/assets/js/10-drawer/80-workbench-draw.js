 ...
 * ordered set of steps you RUN, so it goes along the bottom; a workbench is a surface
...
(function () {
  ...
  function reaim() {
    ...
    var o = owner(window.boardWorkbenches && window.boardWorkbenches.livePage());
    if (!o) return close();
  ...
  function open(page) {
    page = page || (window.boardWorkbenches && window.boardWorkbenches.livePage());
    var o = owner(page);
    if (!o) return;
    var d = mount();
  ...
  /* Draw is an internal Studio lane, so it exports its owner/opener without
     registering a second top-level Workbench row. The shell needs the scene URL to
