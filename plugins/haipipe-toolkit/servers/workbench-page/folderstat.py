...
WHAT THIS ANSWERS (JL 260815: "a first item in the workbench to show the content
...
WHY A LIVE GET AND NOT A WRITTEN VIEW: every other workbench's view page is
...
STALENESS IS THE POINT, and it is claimed narrowly: only a DERIVED workbench
    ...
    def folderstat_view(self, head_only=False):
        ...
        page = _PAGE.format(title=html.escape(title),
                            ...
                            rows="".join(present) or
                                 "<tr><td colspan=5 class=mut>no workbench folders yet</td></tr>",
                            absent=" ".join(absent))
