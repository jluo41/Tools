"""The Run Space inside 📃 Page, read-only.
...
# Revise: compare two frozen texts of one target and settle each change. Its
...
.run-structure{margin:8px 0 12px}.run-structure h4{margin-top:0}.run-structure-source{margin:0 0 6px;color:var(--mut);font-size:11px}
...
@media(max-width:700px){body{font-size:15px}.wrap{padding:0 10px 14px;overflow:visible}.summary{padding:7px 10px}.run-space-switcher{margin-bottom:9px}.run-space-tab{font-size:13px;padding:5px 9px}.run-space-overview{gap:5px}.run-space-overview span{font-size:11px;padding:2px 7px}.support-grid{grid-template-columns:1fr}
 ...
 .detail-head{display:none}.review-card{padding:10px}.md-table{display:table}.detail-title{font-size:17px}}
"""
...
def _page_run_label(run_id: str) -> str:
    ...
    if _is_structure_run(run_id):
        return f"{run_id.lower()} · Structure"
...
def _audit_page_run_order(rows: list[dict]) -> None:
    ...
    for row in page_rows:
        ...
        if _is_structure_run(run_id):
            continue
        if row.get("mode") in {"scratch", "revise"}:
            # Scratch and Revise are the person's direct lanes in Draft Space.
            # They may be opened while the fused Structure Run is still being
...
def _track_change_html(section: str) -> str:
    ...
    for index, match in enumerate(matches):
        ...
        decision = _level_heading_body(body, 6, "Decision")
        ...
        cards.append(
            ...
            '<span class=added>Added</span></div><div class=track-diff>%s</div>%s%s%s%s%s</section>' % (
                ...
                ('<p class=track-decision><b>Decision</b><br>%s</p>' % _inline_markdown(decision)) if decision else "",
...
def _structure_run_visual(page_src: Path, row: dict) -> str:
    """The Structure Run's Result opens with the Outline as a text list.

    Same list Draft Space shows at its top (`live.outline_structure`), read
    from the selected Outline on every open. No Mermaid map (JL 260922).
    """
    ...
    if not text:
        return '<p class=note>The Outline declares no C<n> divisions yet.</p>'
...
def _page_run_detail(row: dict, root: Path, page_src: Path) -> str:
    ...
    visual = _structure_run_visual(page_src, row)
...
def _labeling_runs(page_src: Path) -> list[dict]:
    ...
    the Labeling workbench and never become previews here.
    """
...
def render(page_src: Path, _path_q: str, _file_q: str,
    ...
    # in Folder; they are not a catch-all lane. Page-integrated workbench records
    ...
    root = repo_root(page_src.parent)
    writing = {name: [] for name in ("Structure", "Scratch", "Section", "Paragraph", "Revise")}
    for row in run_p:
    ...
    writing_sections = "".join(
        ...
        for name in ("Structure", "Scratch", "Section", "Paragraph", "Revise")
    )
...
<title>Run Space · {html.escape(page_src.stem)}</title><style>{_CSS}{STRUCTURE_CSS}</style>
