...
def save_scratch(page_src: Path, payload: dict, *, read_only: bool = False,
    ...
    try:
        ...
        step = _clean(payload.get("step") or payload.get("phase", "save"), "step").lower()
        if step not in {"save", "finish"}:
            raise ValueError("step must be save or finish")
        summary = ""
        if step == "finish":
...
(function(){
  ...
  function scratchPayload(form,step){var payload={action:'scratch',step:step};new FormData(form).forEach(function(value,key){payload[key]=value;});var params=new URLSearchParams(location.search);if(!payload.path)payload.path=params.get('path')||'';if(!payload.file)payload.file=params.get('file')||'';return payload;}
  function requestScratch(form,step){return fetch('/_board/outline',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(scratchPayload(form,step))}).then(function(response){return response.json().then(function(result){if(!response.ok||!result.ok)throw new Error(result.err||'Unable to save Scratch');return result;});});}
