...
class BaseMixin:
    ...
    def derive_file(self, page):
        """`file=` for a `path=` that names a Board, a Page folder, or a Page Face.

        `file` is the Page Face relative to the Board root, and every caller
        used to compose it by hand (and got it wrong: a bare basename is a 404
        two folders down). The disk already knows the answer, so when `file` is
        omitted the server derives it: a `.md` path is itself; a Page folder is
        its registered or same-stem Face; the Board folder, or anything inside
        its generated `board/` tree, is `board.md`. None when nothing fits; the
        ...
        board = None
        probe = folder
        while probe == root or root in probe.parents:
            if (probe / "board.md").is_file():
                board = probe
                break
            if probe == root:
                break
            probe = probe.parent
        base = board if board is not None else root
        if location.is_file() and location.suffix == ".md":
            try:
                return location.relative_to(base).as_posix()
            except ValueError:
                return None
        if board is not None and (location == board or (board / "board") in (location, *location.parents)):
            return "board.md"
        if location.is_dir():
            face = registered_page_source(location)
            if face is None and (location / f"{location.name}.md").is_file():
                face = location / f"{location.name}.md"
            if face is not None:
                try:
                    return face.relative_to(base).as_posix()
                except ValueError:
                    return None
        return None
    ...
    def target(self, payload):
        ...
        `file` may be omitted or empty; it is then derived from `path` (see
        `derive_file`). A supplied `file` is still vetted exactly as before.
        """
        ...
        if raw_name is None or (isinstance(raw_name, str) and not raw_name.strip()):
            raw_name = self.derive_file(page)
            if raw_name is None:
                return None, "file is missing and cannot be derived from path"
            try:
                payload["file"] = raw_name      # views build their sibling links from the payload
            except TypeError:
                pass
