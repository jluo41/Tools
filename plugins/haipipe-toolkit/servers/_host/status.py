...
HERE = Path(__file__).resolve().parent  # plugins/haipipe-toolkit/servers/_host
...
try:                                            # the progress row, JL 260820
    from src.page_progress import run_progress, compact as progress_compact
...
def configured_base_url(root, explicit=None):
    ...
    value = configured_domain("", config)
    if value:
        return value.rstrip("/")
...
def render(board, focus="board", mode="status", status="ready", next_action="",
    ...
    # know which phase of the page I am in?", answered today as which Run is next). Read from disk by
    # src/page_progress.py, the same module `cli/pageprogress.py` prints in full, so
    ...
    if target["kind"] == "page" and not problem:
        st = run_progress(board / target["file"], board)
        if st:
            rows.append(progress_compact(st) + "  ")
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "reader-facing origin; defaults to DOMAIN from the root "
