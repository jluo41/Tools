 ...
 * had to know every surface by name. A workbench now registers its own entry, and the
 ...
 * present. Chat + Draw live in Studio; Slides lives in Delivery. Page Runs is
 * lifecycle machinery, not a reader-facing Workbench row.
 *
...
(function () {
  ...
  function ordered(entries) {
    return entries.slice().sort(function (a, b) {
      ...
      return ao - bo || a._workbenchSeq - b._workbenchSeq;
    });
  }
  ...
  /* THE DEFAULT, and why it is a second call rather than a `register` field:
     a workbench ships not knowing whether it wants to be the default, and the
