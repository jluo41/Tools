 ...
 * directions: a five-Run index wants a third of that, and a run with a long trail
 * wants two thirds. A surface whose size the reader cannot set is a surface that is
 * the wrong size most of the time.
 *
 * ONE OWNER, BOTH PANELS. #wfpanel (Labeling) and #pfpanel (Page Runs) are built
 * by their own workbenches; a second copy of drag-and-persist is how the two would
...
(function () {
  ...
  /* Give one bottom panel a top-edge grip. Safe to call twice: the grip is added
     once, so a workbench that rebuilds its panel does not stack handles. */
