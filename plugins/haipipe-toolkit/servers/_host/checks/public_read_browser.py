...
Page, and open one Workbench from the picker.  It records a screenshot per Board
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for board_name in args.board:
            ...
            surface = page.frame(name="page") or page.main_frame
            workbench_button = surface.locator("#mplugbtn")
            if workbench_button.count():
                workbench_button.tap()
                workbench_row = surface.locator("#mplugmenu .mrow").first
                workbench_row.wait_for(timeout=10_000)
                workbench_row.tap()
                page.wait_for_timeout(1_000)
            ...
            print(f"PASS {board_name} · Task Page + Workbench · {surface.url} · {shot}")
            page.close()
        browser.close()
