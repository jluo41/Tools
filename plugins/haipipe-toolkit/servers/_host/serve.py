#!/usr/bin/env python3
"""Serve read-only Pages and explicit Board workbench workspaces.
...
The explicit workbench workspaces keep their own APIs:
...
Needs Python 3.10+ (the SDK does). Run it with the repo's own venv:
    .venv/bin/python plugins/haipipe-toolkit/servers/_host/serve.py --root .
...
bootstrap()                                     # the skill grammar (src/) and cli/ helpers
...
class Handler(AuthMixin, BaseMixin, ActivityMixin, HomeMixin, WriteMixin, ChatMixin, TermMixin, XcalMixin, ShellMixin, ExportMixin, LegacyPagexViewMixin, PlugViewMixin, FolderStatMixin, InsightBoardMixin, DesignMixin, DesignBoardMixin, OutlineMixin, PaperWorkbenchMixin, ValueMixin, EvidenceTabMixin, DeliveryTabMixin, LabelingMixin, PageRunsMixin, RunsTabMixin, SimpleHTTPRequestHandler):
    ...
    only = frozenset()          # `--only labeling`: workbenches this host serves; empty = all
    ...
    def reject_outside_only(self):
        """404 for anything a `--only <workbench>` host does not serve."""
        if self.only and not route_allowed(self.path, self.only):
            self.send_error(404, "not served by this host (--only %s)" % ",".join(sorted(self.only)))
            return True
        return False
    ...
    def do_POST(self):
        ...
        # The EVIDENCE workbenches' read-only surfaces (QPf5 · QPf9): the view
...
if __name__ == "__main__":
    ...
    ap.add_argument("--space-name", default="",
                    help="display name for the SPACE Home (settings.env SPACE_NAME)")
    ...
    unknown = sorted(only - set(WORKBENCH_ROUTES))
    if unknown:
    ...
    # A SPACE whose settings.env says NO_AUTH=1 chose the tailnet as its only
    # gate, so a plain start keeps that choice instead of turning login on from
    # AUTH_FILE. An explicit --auth-file or --public-read still wins.
    if (not a.auth_file and not a.public_read
            and config.get("NO_AUTH", "").lower() in {"1", "true", "yes"}):
        a.no_auth = True
    host = (a.host or config.get("BIND_HOST") or
            config.get("TAILSCALE_ADDRESS") or "127.0.0.1").strip()
    port = a.port
    if port is None:
    ...
    if a.no_auth:
        ...
        auth_arg = a.auth_file or config.get("AUTH_FILE")
        if auth_arg:
    ...
    domain_lines = "".join(
        f"   DOMAIN: {origin:<32} ({why})\n" for origin, why in domains(config, host, port, public_url))
    first_domain = (domains(config, host, port, public_url) or [(f"http://{host}:{port}", "")])[0][0]
    print(f"📡 http://{host}:{port}  root={Handler.root}\n"
          + ("" if not loop else
             f"   ＋ http://127.0.0.1:{port} 也在听（VS Code / ssh -L 走的是这个）\n")
          + domain_lines
          + f"   short routes: {first_domain}/b/<board>[/<page>]  ·  {first_domain}/w/<board>[/<page>[/<tab>]]\n"
          + ("" if not only else
             f"   only: {', '.join(sorted(only))} workbench routes; terminal, chat and Board writes are 404\n")
