...
- Coverage: 10 sections; 58 reader-move Bullets; 195 source sentences.
...
| `input_preservation` · Input preservation | **untested** | informational | The editable source differs from the intake hash; this may be an intentional later edit, so resume does not certify original-byte preservation. |
| `opening` · Opening | **pass** | blocking | Opening is Page-specific, non-placeholder prose that names the Page or its subject. |
| `outline_structure` · Outline | **pass** | blocking | Shape has 10 divisions, 58 paragraphs, 58 Bullets, and one Evidence decision per Bullet. |
| `paragraph_global_order` · Page-global paragraph order | **pass** | blocking | Paragraph identities form one uninterrupted P1–P58 Page sequence. |
| `content_draft_mapping` · Bullet ↔ Content Draft | **pass** | blocking | All 58 Bullets have nonempty, fingerprint-current Content Draft records. |
| `semantic_role_syntax` · Semantic role syntax | **pass** | blocking | All 58 Bullets use a recognized semantic role label. |
| `bullet_head_readability` · Bullet-head readability | **pass** | blocking | All 58 Bullet heads are 4–11 word planning statements without truncation or planner imperatives. |
...
| `semantic_role_judgment` · Semantic-role judgment | **untested** | informational | Role labels are mechanically valid, but an agent or person must review whether each role fits its reader move. |
| `outline_logic_judgment` · Bullet-only argument review | **untested** | informational | An agent or person must read the Bullet heads without the Content Draft and confirm that the argument remains intelligible, ordered, and complete. |
