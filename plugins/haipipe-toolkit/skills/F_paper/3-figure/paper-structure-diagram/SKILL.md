...
- Pairing with `/paper-revise` or `/manuscript-optimizer` to pick *which* paragraph or sentence to revise next

## When to Defer
- Want figures inside the paper rendered → use `/paper-figure`
- Want a logical-argument diagram (claim flow across sections) → use `/diagram-ascii` ref/08-paper-section
- Want word-by-word style review → use `/scientific-writing` or `/paper-revise`
...
```
/paper-structure-diagram                              section view of paper at cwd
/paper-structure-diagram sections [path]              section view (whole paper)
/paper-structure-diagram paragraphs <sec> [path]      paragraph view of one section
/paper-structure-diagram sentences <sec> <par> [path] sentence view of one paragraph
/paper-structure-diagram all [path]                   all three for whole paper
```
...
Compatible with both `paper-bootstrap` layout and the user's `0-` prefix convention:
...
7. **Append** a one-line "next action" hint when something is flagged:
   ...
   - `→ §1 ¶3 has no [claim]. Run /paper-structure-diagram sentences 1 3 to inspect.`

---

## Cross-references

- `paper-bootstrap` (`05_prewrite/`) — initial folder layout this skill reads.
- `paper-revise` (`07_revise/`) — interactive section/paragraph revision; pair with this to pick *what* to revise next.
- `manuscript-optimizer` (`07_revise/`) — structural revision pass; this skill is its visual companion.
- `paper-architecture` (deprecated) — see this skill instead for paper-shaped structure views.
