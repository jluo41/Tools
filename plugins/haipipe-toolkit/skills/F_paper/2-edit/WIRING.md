...
|-------|-----------------|------------------|
| **Skills** | `skills/**/SKILL.md` with a `name:` frontmatter | by skill name (e.g. invoke `paper-edit`; `/paper-edit` resolves to it) |
...
|-------|------|-------------|
| `paper-edit` (orchestrator) | `skills/F_paper/4-edit/paper-edit/SKILL.md` | ✅ on next reload (valid `name:`) |
| `paper-edit-content` + 5 topic stubs | `skills/F_paper/4-edit/paper-edit-*/SKILL.md` | ✅ on next reload |
| 4 stage agents | `skills/F_paper/4-edit/agents/*.md` | ⚠️ **not** as `subagent_type` (nested, not in top-level `agents/`) |
| `/paper-edit` command | — | n/a — invoked by skill name, like every skill here |

> Skills created mid-session aren't hot-loaded; `paper-edit` etc. appear in the
...
`../skills/D_probe/agents/reviewers/claim-verifier-agent.md`). Our 4 agents are
...
works now. This is what `paper-edit/SKILL.md` instructs.
...
for a in format-checker annotator improver cleaner; do
  ln -s ../skills/F_paper/4-edit/agents/paper-edit-$a.md paper-edit-$a.md
done
```
...
- [ ] Reload Claude Code so the skills register (then `paper-edit` is invocable).
...
- [ ] Wire `scripts/check_comment_only.sh` into the orchestrator's verify step
      (already referenced in `paper-edit/SKILL.md`).
