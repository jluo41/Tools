...
Venue-agnostic. Works on any tex root that follows the F_paper paper-
...
```
1-narrative → 2-plan → 3-write → ┌──────────────────┐  → 5-review → 6-respond → 7-present
                                 │ haipipe-paper-   │
                                 │  revise (HERE)   │
                                 │                  │
                                 │  per-section:    │
...
2. **Glob `0-sections/*.tex`.** Standard F_paper paper-folder layout.
...
Standard F_paper layout (Phase 1.2 hit):
```
papers/lhm-a/
