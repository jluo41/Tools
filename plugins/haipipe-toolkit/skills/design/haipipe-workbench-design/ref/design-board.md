...
**LOAD `haipipe-workbench` and `../SKILL.md` (`haipipe-workbench-design`) FIRST.**
This reference is the Board-level grain of the Design workbench: one 🎨
Design Board surface over a whole DesignBoard, in the same five Spaces as
the Page level, one grain up; served by `servers/workbench-design/designboard.py`
at `/_board/design-board` (short form `/w/<board>` on a Design Board). The Page-level
...
should we make for `<audience>`?". The workbench writes the folder's name into
...
this workbench makes; the Brief's prose, needs, and signed inputs stay with
...
<Insight board>/…                       signed pages, through the Insight workbench's records
```
...
See `design-board-space-mapping.md` for the Space ↔ file map at board grain; `space-mapping.md` is the Page grain.
