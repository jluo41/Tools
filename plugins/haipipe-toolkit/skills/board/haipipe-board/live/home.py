...
def render_home(root: Path, space_name: str = "", public_url: str = "") -> str:
    ...
    project_keys = list(dict.fromkeys(str(card["project_key"]) for card in cards))
    project_total = len({str(card["project_key"]) for card in cards
                         if card["project_scope"] == "project"})
    for project_key in project_keys:
        project_cards = [card for card in cards
                         if card["project_key"] == project_key]
        owner = project_cards[0]
        kind_sections = []
        for kind, icon in BOARD_KINDS:
            kind_cards = [card for card in project_cards if card["kind"] == kind]
            if not kind_cards:
                continue
            board_cards = []
  ...
  <div class="row-path"><span class="meta-label">Full path</span><a class="path" href="{href}" title="Open {path}">{path}</a></div>
  {actions}
</article>''')
            kind_sections.append(
                f'''<div class="kind-section">
  <div class="kind-head"><h3>{icon} {html.escape(kind)}s</h3><span>{len(kind_cards)}</span></div>
  <div class="board-list">{"".join(board_cards)}</div>
</div>''')
        project = html.escape(str(owner["project"]))
        project_path = html.escape(str(owner["project_path"]))
        scope_icon = "📁" if owner["project_scope"] == "project" else "🧭"
        sections.append(
            f'''<section class="project-group" data-group>
  <div class="project-head"><div><h2>{scope_icon} {project}</h2><p>{project_path}</p></div><span>{len(project_cards)} boards</span></div>
  {"".join(kind_sections)}
</section>''')
...
.summary{{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-bottom:30px}}.metric{{border:1px solid var(--line);border-radius:8px;background:var(--surface);padding:13px 15px}}.metric strong{{display:block;font-size:24px;line-height:1.15}}.metric span{{display:block;color:var(--mut);font-size:12px;margin-top:3px}}
.project-group{{margin:0 0 34px}}.project-head{{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;border-bottom:2px solid var(--ink);padding:0 2px 9px;margin-bottom:13px}}.project-head h2{{font-size:20px;line-height:1.2;margin:0;font-weight:850}}.project-head p{{color:var(--mut);font:11px ui-monospace,SFMono-Regular,Menlo,monospace;margin:4px 0 0}}.project-head>span{{color:var(--mut);font-size:12px;font-weight:800;white-space:nowrap}}.kind-section{{margin:0 0 18px}}.kind-head{{display:flex;align-items:center;gap:9px;margin:0 0 8px}}.kind-head h3{{font-size:14px;line-height:1.2;margin:0;font-weight:800}}.kind-head span{{min-width:25px;border-radius:999px;background:#e7ebef;color:var(--mut);font-size:12px;font-weight:800;text-align:center;padding:2px 7px}}
...
@media (max-width:680px){{main{{padding:22px 14px 38px}}h1{{font-size:28px}}.toolbar{{align-items:stretch;flex-direction:column;margin-top:22px}}.search{{max-width:none}}.filter-count{{font-size:12px}}.summary{{grid-template-columns:1fr 1fr;gap:7px;margin-bottom:26px}}.metric{{padding:11px 10px}}.metric strong{{font-size:20px}}.metric span{{font-size:11px}}.project-head{{align-items:flex-start}}.project-head h2{{font-size:18px}}.board-row{{grid-template-columns:minmax(0,1fr) auto;padding:13px 14px;row-gap:9px}}.row-main{{grid-column:1;grid-row:1}}.row-status{{grid-column:2;grid-row:1}}.row-pages{{grid-column:1;grid-row:2}}.row-path{{grid-column:1 / 3;grid-row:3}}.actions{{grid-column:1 / 3;grid-row:4}}.board-row h3{{font-size:17px}}}}
...
<section class="summary" aria-label="Board summary"><div class="metric"><strong>{project_total}</strong><span>Projects</span></div><div class="metric"><strong>{len(cards)}</strong><span>Total boards</span></div><div class="metric"><strong>{ready_total}</strong><span>Ready to open</span></div><div class="metric"><strong>{settled_total}/{page_total}</strong><span>Pages settled</span></div></section>
