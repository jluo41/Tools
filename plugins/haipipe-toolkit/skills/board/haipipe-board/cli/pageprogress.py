#!/usr/bin/env python3
"""The progress strip: which Run is next for ONE Page, read from its Run list.
...
Three strips, three questions, and none substitutes for another:
    ...
    pageprogress.py which RUN is next for this PAGE  (this file)

The computation lives in `src/page_progress.py` and is shared with `status.py`'s
fourth row; this file only prints it. Never writes.

    python3 pageprogress.py PAGE_DIR           the strip
    python3 pageprogress.py PAGE_DIR --md      fenced, for pasting on a page
    python3 pageprogress.py PAGE_DIR --row     the one-row form status.py prints
    python3 pageprogress.py PAGE_DIR --owed    the LEDGER: every human tick still
...
the owning Run may declare no Page RULING, reuse a domain gate, or require a
local one.

⚠️ The `→ now` row is the first Run whose exit test FAILS, read in loop
order. That is a REPORT, not a routing: which Run actually runs next is
...
from src.page_progress import (  # noqa: E402
    EMOJI, LABEL, MARK, compact, run_progress, render_ledger, run_of)
...
def main():
    ...
    L = [f"{st['page']} · progress strip (from disk, "
    ...
    it = st.get("items")
    L.append(f"{row('structure')} v{o['version']} "
    ...
    L.append(f"{row('writing')} {st['divisions']} content divisions"
             + ("" if not o["file"] else
    ...
    L.append(f"{row('check')} last receipt: "
             + (f"{run_of(last.get('run') or last.get('phase'))} → {last.get('route')} (round {last.get('round')})"
                if last else "none"))
