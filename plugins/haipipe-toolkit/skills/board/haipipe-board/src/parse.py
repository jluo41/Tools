...
from .page_parse import parse_page, split_sections, strip_notes
...
def parse_dir(d):
    ...
    for p in page_files(d):
        ...
        if (task_info or qm or sm or named_qm or skill_m or agent_m
                or meeting_m or design_m or story_m or app_m or registered):
            if task_info:
                ...
                family = story_m.group(1).lower()
                page_id = p.stem
                letter = story_m.group(2).upper()
                suffix = story_m.group(3) or ""
                key = (0, "Story", ord(letter), suffix.casefold())
                kind = "stage"
            elif app_m:
                ...
                key = (2, p.relative_to(d).as_posix())
                page_id, kind, family = p.stem, "page", ""
            disk_key = (task_info["reference"] if task_info else
                        p.relative_to(d).as_posix() if registered else p.name)
            if disk_key in disk:
                dupes.append(
    ...
    for raw in pages_txt.split("\n"):
        ...
        if ln.startswith("### "):
            ...
            ambiguous = False
            if not key_name and "/" in name:
                key_name = next((key for key, entry in disk.items()
                                 if entry[2].relative_to(d).as_posix() == name), "")
            if not key_name and "/" not in name:
                ...
                if len(matches) == 1:
                    ...
                    warn.append(
                        ...
                        "job/task/page relative path" if block_board else
                        f"{name} is ambiguous in this Board; list its Page-relative path")
            if key_name:
