...
(`page-plugins/haipipe-plugin-display`). This module only says which step a unit
...
# (`haipipe-plugin-display`, "Citation"): the bare Page-local id inside its own
...
def unit_state(unit: Path) -> dict:
    ...
    The step names and their order are `haipipe-plugin-display`'s, adopted
