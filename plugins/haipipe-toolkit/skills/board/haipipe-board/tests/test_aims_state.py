...
class AimsStateTest(unittest.TestCase):
    ...
    def test_public_paper_door_routes_current_page_types_without_compatibility_copies(self):
        # 260907: the journey is Ideation → Story → Evidence/Execution →
        # Section → Compile → Round. The Story retains its substantive C6/C7/C8
        # content, but retired child Page names have no router or registry copy.
        root = Path(__file__).resolve().parent.parent  # the engine dir
        paper_root = root.parents[1] / "paper"
        door = (paper_root / "haipipe-paper" / "SKILL.md").read_text(
            encoding="utf-8"
        )
        for page_type in ("ideation", "story", "section", "round"):
