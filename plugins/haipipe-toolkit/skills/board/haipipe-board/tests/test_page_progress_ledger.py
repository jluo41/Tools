...
from src.page_progress import owed_ledger, run_progress, render_ledger


def _page(
    root, *, approved=False, cards=(), bib=(), displays=(),
    folder_kind=None, legacy_design_kind=None,
):
    ...
    if legacy_design_kind:
        phase = "D1" if legacy_design_kind == "design-card" else "D2"
        (pd / "workflow").mkdir()
        (pd / "workflow" / "phase.yaml").write_text(
            f"current:\n  phase: {phase}\n"
            f"  folder-kind: {legacy_design_kind}\nhistory: []\n",
            encoding="utf-8",
        )
...
class OwedLedgerTest(unittest.TestCase):
    def state(self, **kw):
        ...
        return run_progress(_page(self.tmp.name, **kw))
    ...
    def test_run_owned_mechanical_page_owes_no_owner_ruling(self):
    ...
    def test_run_owned_wisdom_reuses_its_domain_gate(self):
    ...
    def test_retired_design_lifecycle_file_is_unsupported(self):
        st = self.state(approved=True, legacy_design_kind="design-card")
    ...
    def test_legacy_lifecycle_file_and_markdown_kind_mismatch_is_never_guessed(self):
        st = self.state(
            approved=True,
            folder_kind="design-card",
            legacy_design_kind="design-unit",
        )
    ...
    def test_full_cli_renders_the_current_progress_strip(self):
        with tempfile.TemporaryDirectory() as directory:
            page = _page(directory, approved=True)
            cli = Path(__file__).resolve().parents[1] / "cli" / "pageprogress.py"
            got = subprocess.run(
            ...
            for name in ("Context", "Structure", "Evidence", "Writing", "Check"):
