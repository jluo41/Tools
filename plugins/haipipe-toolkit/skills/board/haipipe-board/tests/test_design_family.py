...
def build(root: Path) -> Path:
    ...
    folder = board / "2-DS-design" / "DS01-patient-confirm-sms"
    folder.mkdir(parents=True)
    (board / "board.md").write_text("# Current DesignBoard\n", encoding="utf-8")
    (folder / "DS01-patient-confirm-sms.md").write_text(
        "# Current Design\n\nfolder-kind: design\n", encoding="utf-8"
    )
    return board
...
class DesignFamilyTest(unittest.TestCase):
    ...
    def test_d0_d5_thread_shape_is_rejected(self):
        with TemporaryDirectory() as td:
            board = build(Path(td))
            old = (
                board / "2-DS-design" / "DS01-patient-confirm-sms"
                / "design" / "DU01-old"
            )
    ...
    def test_rnn_design_ticket_is_rejected(self):
        with TemporaryDirectory() as td:
            board = build(Path(td))
            runs = board / "2-DS-design" / "DS01-patient-confirm-sms" / "runs"
            runs.mkdir()
