...
class RunsWorkbenchTest(unittest.TestCase):
...
"""
    ...
    def test_folder_shows_skill_as_an_explicit_outline_lane(self):
        ...
        (skill / "S-Test.md").write_text("- haipipe-page-structure\n",
                                          encoding="utf-8")
    ...
    def test_structure_must_close_before_numbered_paragraph_runs(self):
        runs = self.page.parent / "runs"
        runs.mkdir(exist_ok=True)
        (self.page.parent / "outline" / "S-Test-outline-v1.0.md").write_text(
            "# S-Test\n## C1 · Opening\n### C1.P1 · Opening\n- B1 · a\n"
            "### C1.P2 · Meaning\n- B1 · b\n",
            encoding="utf-8",
        )
        ...
        structure = writing_run("rp-struct-01", "Structure + P01..PN")
        ...
        self.assertIn("<h4>Structure</h4>", body)
        self.assertIn('<pre class="structure-text">C1 · Opening\n  C1.P1 · Opening\n  C1.P2 · Meaning</pre>', body)
        self.assertNotIn("Mermaid", body)
        self.assertNotIn("logic-steps", body)
    ...
    def test_runs_workbench_post_route_escapes_page_parameters(self):
