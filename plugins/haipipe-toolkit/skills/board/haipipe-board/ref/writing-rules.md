...
- `## Opening` = one lead sentence, then one plain paragraph
  The first paragraph is the actual question, written as a question, and stays in Opening. Write the remainder as **one flowing paragraph** covering the page's scope, why the question is hard, what breaks while it stays open, and what it affects downstream. build.py labels it "Why this matters" as a collapsed row inside Opening for Q and S. Do NOT use the old three-bullet form. There is no separate `## Boundary`; name the neighbouring page in this paragraph when it owns excluded work.
