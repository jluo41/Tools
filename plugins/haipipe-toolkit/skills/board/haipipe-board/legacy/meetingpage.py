...
Questions`), so the mapping below is a lookup rather than an interpretation:
    ...
    ### Action Items     -> ## Aims             SEEDED ONCE, then yours
    ### Open Questions   -> ## States           SEEDED ONCE, then yours
...
def render(board, note, note_path, stamp_date):
    ...
    return STUB.format(
        ...
        aims="\n".join(
            ...
            "- P1 · Route this meeting onto the pages it changed.\n"
            "  **Done when:** Every decision above has landed on its owning Q page.",
        aim_states="\n".join(
            f"- ⬜ P{i} · Imported from the meeting; not yet routed."
            for i, _a in enumerate(actions, 1)) or
            "- ⬜ P1 · Imported; routing has not started.",
        questions="\n".join(
