...
# /haipipe-page-for-meeting · talk is recorded here, and ruled elsewhere

**LOAD `haipipe-page` FIRST.** It owns the base frame. This file adds only what a meeting record needs, and it is short because a meeting page owns almost nothing.
...
**A page is a unit of work** (`QB6` §7). A meeting page reads what was collected in the room and hands each decision to the page that owns its subject.

```text
 📥 INPUT   <stage>/meeting/<page name>/     the raw material of the meeting
              the recording · the transcript · whatever was shown
              collected, not authored, exactly like any other input folder
 📤 OUTPUT  decisions ROUTED onto the page that owns each subject
              ▶ a decision spoken in a meeting is NOT ruled until it lands
                on that page; this page records it and never rules it
```

A meeting page is never counted in a board's settled total, like a mirror. It closes when every decision-shaped line either points at where it landed or is marked not-yet-routed.
...
Owns no scripts. The generator is `haipipe-board/cli/meetingpage.py`; the routing verb is `haipipe-board-routing`; the base is `haipipe-page`.
