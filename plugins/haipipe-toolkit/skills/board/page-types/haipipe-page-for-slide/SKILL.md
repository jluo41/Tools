...
# /haipipe-page-for-slide · one deck file, and every division shows its slide live

**LOAD `haipipe-page` FIRST.** It owns the base frame. This file adds only what a deck needs, and its acceptance model is `for-display`'s: a person accepts a RENDER, never a name.
...
**A page is a unit of work** (`QB6` §7). A slide page owns a unit folder on both sides, like a display, but ships a deck rather than a float.

```text
 📥 INPUT   <stage>/slide/<page name>/
              source/<deck>.template.html   carries NO DIGITS, on purpose
              source/build.py               fills them from a value record
            ↗ the value record it declares a need on
 📤 OUTPUT  <paper root>/slides/<page name>/deck.html      🤖 GENERATED
              ▶ each Content division embeds it live at ?preview=N
              ▶ a hand-edited slide is overwritten on the next build
```

The no-digits rule is what makes `for-value`'s discipline survive a deck. Numbers get typed into slide markup by hand and never traced again; if the template carries none, they cannot be.
...
Owns no scripts. The base is `haipipe-page`; the acceptance model is `haipipe-page-for-display`'s; the number rule is `haipipe-page-for-value`'s; the builder is `display/skills/html-ppt` (36 themes, 31 layouts, `?preview=N` single-slide mode, PNG export for non-iframe surfaces); the paper family's outline extractor is `display/skills/paper-slides`; the proving page is the boardform board's `QA-design/QA4-board-skillset.md`.
