...
description: >-
  The VARIANT contract for a Board's S-Family-unit stage pages, one Page per lifecycle stage of a paper or application across seed, resource, claims, venue, pitch, narrative, display, and section-edit. It resolves a Page to its own stage through the board_family and board_unit declared by stage.md and holds no per-stage rule itself. It loads haipipe-page for the base frame and adds the chain, managed Stage Contract, venue transfer tiers, venue-free versus venue-aligned split, and human gate semantics. Use when writing or fixing a stage Page, when its inputs or venue binding are wrong, when a paper retargets, or when a draft needs to know which venue rules bind it. Trigger: stage page, S page, S-Main, S-Venue, S-Seed, lifecycle stage, stage contract, requires, style-from, provides, venue contract, blueprint, retarget, section edit, gate, /haipipe-page-for-stage.
metadata:
  version: "0.6.0"
...
# /haipipe-page-for-stage · a stage page is the paper, one gate at a time
...
📊 A DASH PAGE        S-Main-Dash, S-Value-Dash, S-Display-Dash, S-Literature-Dash
                      → NO stage row, and it is not a stage of the paper. It is a
                        ...
                        → It declares `page-type: dash` and loads its own contract:
                          `haipipe-page-for-dash-{section,value,display,literature}`
                          in `paper/page-types/`, picked by the family in the filename.
                        → NEVER a human gate, and NEVER counted as settled: a dash
                          decides nothing.
                        → It DOES declare `requires: S-Open-Venue` (JL 260809).
                          This corrects the rule stated here until 260809, which
                          read "never give a Dash page a gate, requires:, or
                          provides:". That sentence conflated two different things:
                          a dash has no gate because it rules nothing, but it cannot
                          measure a family without the venue blueprint, which is the
                          only shared yardstick its members can be ranked against.
                          S-Display-Dash already declared it while three sibling
                          dashes declared nothing, so the rule was already broken
                          unevenly on the real board.
                        → `provides:` is allowed but rarely earned: a dash hands on
                          a reading, and a page that needs the reading reads the dash.
```

**A unit page may carry a more specific type.** An `S-Main-<n>` or `S-Appendix-<letter>` section unit declares `page-type: section` in its frontmatter and loads `haipipe-page-for-section` on top of this file; an `S-Display` unit page declares `page-type: display` and loads `haipipe-page-for-display`. The stage filename still says which family and unit the page belongs to; the `page-type:` key says which contract governs it, and the key beats the filename (base, type resolution step ③).
...
**A page is a unit of work** (`QB6` §7). A stage page reads its upstreams and hands on what it provides. Neither side is a folder.

```text
 📥 INPUT   ↗ elsewhere · the `### Provides` of every page named in `requires:`,
            and the prose rules resolved from `style-from:`
              the page keeps a RECORD of both, in its managed ## Stage Contract,
              and NEVER a copy: a copied rule goes a night out of date while
              the source moves
 📤 OUTPUT  its own `### Provides`, which the next stage reads the same way
```

The order of pages in `board.md` is never dependency inference. `stage.py sync` walks the explicit `requires` / `style-from` graph, and a stale contract span is reported until an explicit sync clears it.

## 📂 Files

```
page-types/haipipe-page-for-stage/
