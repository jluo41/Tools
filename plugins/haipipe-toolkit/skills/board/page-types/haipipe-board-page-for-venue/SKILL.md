...
description: >-
  The VARIANT contract for a Board's numbered QBv venue Pages, one Page per submission target such as a journal, agency, or patent office. It loads haipipe-board-page for the base frame and adds three figures in fixed order, venue-indexed Content divisions, the rule that the desk outranks the pack, five Files groups, provenance for every desk fact, and visible open rows for facts the pack cannot supply. Use when writing or fixing a venue Page, when a new outlet gets a Page, when the pack and desk disagree, or when a venue Page states a number with no source. Trigger: venue page, QBv, outlet page, journal page, desk, venue pack, playbook, submission rules, page limit, Venue-Structure, Submission-Rules, exemplars, section kinds, /haipipe-board-page-for-venue.
metadata:
  version: "0.1.2"
