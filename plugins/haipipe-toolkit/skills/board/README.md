...
reviewable set of Pages. Generic Boards hold question decisions and lifecycle
stages; an explicit `task-block` Board projects Block -> Job -> Task as Board
-> Group -> Page while leaving Run as an execution record.
...
- `page-types/` holds the one legacy variant this board family still owns.
  Migrated kinds live with their workflow phase: Paper under
  `paper/workflow-phases/`, and Application I0-I5/D0-D5 under
  `application/workflow-phases/`. Task is owned by `haipipe-task` and enters
  this family through the `task-block` Board adapter rather than a Page-type
  variant. Labeling remains with its owning plugin. The compatibility inventory is
  ...
  them until 260821. `design` (no speciality left), `meeting`, `skill`, and
...
- `page-workflows/` holds the page WORKFLOW: `haipipe-page-workflow` is its
  ...
  five host-agnostic phase contracts, in loop order: `haipipe-page-context`,
  `haipipe-page-outline`, `haipipe-page-evidence`, `haipipe-page-content`,
  and `haipipe-page-check`. Historical DRAFT/REVISE/COMPILE receipt tokens are
  interpreted by the lifecycle auditor; they do not have redirect skills.
...
│   │   └── value-rules.md           🧮 historical Probe receipt compatibility
│   ├── haipipe-page-creator-agent.md
│   ├── haipipe-page-approver-agent.md
│   ├── haipipe-board-reviewer-agent.md
│   └── haipipe-page-auditor-agent.md
├── haipipe-page/
├── page-plugins/        exactly five generic public skills; lane laws live
│   ├── haipipe-plugin-outline/       Context + Bullet + Evidence
│   ├── haipipe-plugin-studio/        Chat + Draw refs
│   ├── haipipe-plugin-runs/          Tickets + Results
│   ├── haipipe-plugin-delivery/      LaTeX + Word + Slide + Render refs
│   └── haipipe-plugin-folder/        roster meta-surface
├── page-types/          the ONE variant THIS skill set owns
│   └── haipipe-page-for-stage/
├── page-workflows/      the head skill, its five phase members, their agents
│   ├── haipipe-page-workflow/
│   │   └── ref/  page-run-contract.md · producer-contract.md ·
│   │             phase-cards.md · measured-cost.md
│   ├── haipipe-page-context/        00 · PREPARE
│   ├── haipipe-page-outline/        01 · SHAPE + SURVEY
│   ├── _old/haipipe-page-probe/     retired 260901 (SURVEY + LAND took its halves)
│   ├── haipipe-page-evidence/       02 · LAND + EMBED
│   ├── haipipe-page-content/        03 · WRITE
│   ├── haipipe-page-check/          04 · CHECK
...
└── haipipe-board/
    ...
    ├── src/             parser, renderer model, and `dialect_task_block.py`
    ├── live/
    ├── ref/
    ├── assets/
    ├── checks/
...
Regenerate the full view with `/workflow-table board`. The local closing gates
are the Board test suite, `haipipe-board/cli/pagetypes.py --check`, and
`haipipe-board/cli/foldercontracts.py --check`. Keep counts in command output
and changelogs rather than freezing a soon-stale snapshot here.
