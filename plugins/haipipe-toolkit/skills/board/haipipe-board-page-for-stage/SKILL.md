...
description: >-
  The VARIANT contract for a Board's stage pages: S-<Family>-<unit>-<slug>, one page per lifecycle stage of a paper or application, across seed, resource, claims, venue, pitch, narrative, display and section-edit. It RESOLVES a page to its own stage through the board_family and board_unit that stage.md declares, and holds no per-stage rule itself. It loads haipipe-board-page for the base frame and never restates it, then adds only what a stage page needs and a decision page does not: the CHAIN (requires / style-from / provides and the hash-managed Stage Contract span), the rule that ONE stage reads the venue page and every later stage reads that stage instead, the four transfer tiers deciding what crosses from a venue page into a draft (copy the shape, carry as a target, never copy, defer to deliver), the venue-free against venue-aligned split that decides what a retarget rewrites, and the human gate that closes the page. Use when writing or fixing a stage page, when a stage's inputs or venue binding are wrong, when a paper retargets to another venue, or when a draft needs to know which venue rules bind it. Trigger: stage page, S page, S-Main, S-Venue, S-Seed, lifecycle stage, stage contract, requires, style-from, provides, venue contract, blueprint, retarget, section edit, gate, /haipipe-board-page-for-stage.
metadata:
  version: "0.4.0"
  last_updated: "2026-08-03"
  summary: "Two fresh-agent tests: the blueprint binds row by row, and the Aims scheme is the q-consumer id because A<n> cannot key to a manuscript section."
...
Owns no scripts. The base frame is `haipipe-board-page`; the generator for the managed span is `haipipe-board/src/stage_contract.py` driven by `cli/stage.py`; the stage roster is `paper/1-lifecycle/haipipe-paper-stage/stages/index.yml`; the catalog this contract binds to is `haipipe-board-page-for-venue`; the phase workers that fill a stage are `haipipe-paper-draft`, `haipipe-paper-probe`, `haipipe-paper-revise` and `haipipe-paper-check`.
