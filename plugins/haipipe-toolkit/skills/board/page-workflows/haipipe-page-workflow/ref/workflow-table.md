...
| `03` | `haipipe-page-content` | `WRITE` | Page Content + declared delivery surfaces | Yes: Paragraph Writing Runs |
...
| `03` | `CONTENT / WRITE` | Realize the approved evidence-aware plan as Page Content and current delivery artifacts. | Fresh Context; approved folded plan; ready Evidence Results; Page Face owner structure; narrative/style policy and named exemplars; paragraph job/Bullets; applicable feedback/decisions; current Page and adjacent paragraph versions. | `haipipe-page-content → <Folder-owner> → <Page-Face-owner> → haipipe-plugin-outline/ref/plan-grammar.md → <narrative/style-policy> → haipipe-run + haipipe-writing (when selected by the owner) + <owner-selected revision/delivery-workers>` | Reads all three Outline workspaces; Page Content and Delivery are the produced surfaces. | Writes/promotes `<page>.md` Content paragraphs; updates authorized Opening/Aims/Log; regenerates declared `delivery/` projections; records writing receipts. | Normally `1 Page · Paragraph Writing Run × commissioned paragraph`; internal Draft/Revise/Build/Pre-check movements are not extra Runs. | Each paragraph Result passes trace/style checks and is promoted; division/Page continuity passes; delivery artifacts are current; fresh pre-check returns `ready`. | `CHECK`; loop `CONTENT`; route to `CONTEXT`, `OUTLINE`, or `EVIDENCE` by the broken authority. |
...
- `P` be Content paragraphs commissioned for writing.
...
Content work  = P Page Paragraph Writing Runs
Total         = Σ S_i + I + P
```
...
For CONTENT, `P` is the number of paragraphs actually commissioned, not the
number of divisions or all paragraphs already on the Page. Each target is
`C<n>.P<m>` and each Markdown Ticket embeds its prompt. Consume the paragraph
job, bounded claims, folded evidence, applicable feedback/decisions, shared
Context style and named exemplars, plus preceding prose and the next planned
handoff. Results are `paragraph.md`, `trace.md`, and `runtime.yaml`; promotion
changes only that paragraph. The division/Page continuity pass is internal
WRITE work and adds no umbrella Run. Later material revisions add new Runs;
shared Supporting Run identities in the evidence formula are counted once.
Exact writing contract: `../../haipipe-page-content/ref/paragraph-run.md`.
...
| CONTENT | paragraph candidate uses that folded result | `r04_page-writing_c02-p01` trace row names E01 Result |
