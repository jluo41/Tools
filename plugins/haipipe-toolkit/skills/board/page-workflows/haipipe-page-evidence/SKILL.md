...
description: >-
  ...
  Supporting Runs, validates zero-to-many exact PageX bindings, then executes
  exactly one local Page Evidence Item Run) and EMBED
...
workflow-phase skill supplies its semantic policy; this skill owns only the
Page EVIDENCE cycles.

## ⚡ Phase card

```text
READS    target Page · approved plan · outline/<stem>-evidence-items.md ·
         selected Run Tickets/receipts/Results · exact PageX authorities ·
...
EXITS    LAND: every make-item has valid supports/PageX bindings and one
...
PageX Binding       exact accepted cross-Folder source; not a Run or Result type
Local Input         one frozen envelope of Supporting Results + PageX/local sources
...
5. **Validate every PageX binding.** Resolve the exact repo-relative file or
   Result, confirm its named authority is accepted, and compare its version or
   hash with what SURVEY selected. A whole-Folder link, dangling path, merely
   similar Page, or superseded authority is not evidence and routes to SURVEY
   or HOLD. PageX is never counted as a Run or Result.
6. **Freeze one Local Input.** Materialize the SURVEY plan as one immutable
   envelope containing exact Supporting Result pointers/hashes, validated
   PageX bindings, plus any named pre-existing governed local artifacts. When
   PageX bindings exist, the envelope must name and freeze them. Zero supports
   is valid only when the planned PageX/local material or item contract is
...
declared Supporting Result and PageX binding to validate. This dependency is
...
or PageX authority after the fold makes the binding `stale` and reopens LAND
or EMBED as needed.

## 🔀 Routing

```text
LAND   item meaning/acceptance invalid                  → SHAPE
LAND   Run graph or PageX binding incomplete           → SURVEY
LAND   support/local Run truthfully failed or blocked  → LAND/HOLD with Run id
LAND   every make-item has an accepted local Result    → EMBED
EMBED  ready Result contradicts the outline            → SHAPE with D<nn>
EMBED  every ready Result folded                       → SHAPE with plan v<N+1>
```

EVIDENCE never routes directly to WRITE. SHAPE re-agrees the evidence-aware
...
pagex-bindings: n valid · n dangling/superseded/invalid
