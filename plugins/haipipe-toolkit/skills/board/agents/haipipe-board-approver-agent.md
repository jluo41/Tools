---
name: haipipe-board-approver-agent
...
4. Write the ONE field on the artifact, and nothing else in that file:

   ```text
   all pass          <verb>: ✅ auto <YYMMDD> · <file> R1-R<n> pass
   any fail          <verb>: ⬜ auto <YYMMDD> · R<k> fails: <the defect>
   any unevaluated   <verb>: ⬜ auto <YYMMDD> · R<k> unevaluated: <why>
   ```
...
```text
actor:      haipipe-board-approver-agent
