...
A flat page (no home folder yet) uses the board-level `<board>/<plugin>/` fallback, the same rule `live/deck.py` applies.
...
Server-side builders live as one `live/` mixin per concern and one `/_board/<plugin>` route, the shape `live/deck.py` set.
