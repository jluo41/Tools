...
## Opening
What must be defined about a Paper sentence before a stage can write or revise it reliably? A sentence carries its own evidence boundary and also lives among siblings inside a paragraph, and everything attached to it is bound by simple adjacency rather than new syntax.
...
```
 ...
 ├─ QB3a-sentence-citation.md   ## Content ◄ S1 S2 S3   only
 ├─ QB4a-sentence-value.md      ## Content ◄ S4         only
 ├─ QB5a-…-display-table.md     ## Content ◄ S5         only
 ├─ QB5b-…-display-figure.md    ## Content ◄ S6         only
 ...
 └─ _fixture/         ◄ EVERYTHING A MARKER RESOLVES TO. never prose.
    ...
    │    stock2002survey             the key QB3a's broken example SHOULD say
    │  ⚠️ stock2005testing is ABSENT, exactly as it is absent on the paper,
    │    which is what makes QB3a's broken example a real defect and not a prop
 ...
 TO MAKE A NEW STATE SHOWABLE
   ...
   Same human-only boundary QB3a states; it is the only reason a colour
 ...
 WHY THE FOUR TYPES NEEDED NO NEW SYNTAX
   QB3a citation · QB4a value · QB5a table · QB5b figure
...
`QB3a` to `QB5b` each carry their own sentences out of this paragraph, labelled by position, so the four type pages are four readings of one paragraph rather than four unrelated examples.
...
```
 ONE paragraph, FOUR kinds of thing hanging off it, ONE join key
   ...
   \citep{graziano1996perceiving}   ends in a .bib entry a HUMAN writes    QB3a
   \cite{TOADD} [Q-Seed-1]          owed; no key exists yet                QB3a
   +0.0009                          checked against the RUN                QB4a
   \ref{tab:descriptives}           rows in the panel; checkable on sight  QB5a
   \ref{fig:discretion-gradient}    two pictures; NOT checkable            QB5b
...
Six entries in the `.bib` is deliberate: only the keys these examples cite. Adding a key means copying the REAL entry out of that paper's bibliography, never writing one, which is the same human-only boundary `QB3a` states. To make a new citation state showable, copy in the entry (or the probe entry) that already produces it; do not invent one to make a colour appear.
...
- `ready`
  Something landed and the manuscript has not caught up, whether what landed is a probe answer or a rendered figure (QB5b).
- `parked`
...
- `stale`
  An asset older than its `source/`, so the manuscript is compiling the old one. Computed from mtimes, never declared (QB5a).
- `candidate`
  Retired into `ready` when QB5b was built: the same fact in two vocabularies.
