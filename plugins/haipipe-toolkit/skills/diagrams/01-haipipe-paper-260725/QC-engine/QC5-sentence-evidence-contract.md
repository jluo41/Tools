...
```
 ...
 ├─ QB12a-sentence-citation.md   ## Content ◄ S1 S2 S3   only
 ├─ QB12b-sentence-value.md      ## Content ◄ S4         only
 ├─ QB12c-…-display-table.md     ## Content ◄ S5         only
 ├─ QB12d-…-display-figure.md    ## Content ◄ S6         only
 ...
 └─ _fixture/         ◄ EVERYTHING A MARKER RESOLVES TO. never prose.
    ...
    │    stock2002survey             the key QB12a's broken example SHOULD say
    │  ⚠️ stock2005testing is ABSENT, exactly as it is absent on the paper,
    │    which is what makes QB12a's broken example a real defect and not a prop
 ...
 TO MAKE A NEW STATE SHOWABLE
   ...
   Same human-only boundary QB12a states; it is the only reason a colour
 ...
 WHY THE FOUR TYPES NEEDED NO NEW SYNTAX
   QB12a citation · QB12b value · QB12c table · QB12d figure
...
`QB12a` to `QB12d` each carry their own sentences out of this paragraph, labelled by position, so the four type pages are four readings of one paragraph rather than four unrelated examples.
...
```
 ONE paragraph, FOUR kinds of thing hanging off it, ONE join key
   ...
   \citep{graziano1996perceiving}   ends in a .bib entry a HUMAN writes    QB12a
   \cite{TOADD} [Q-Seed-1]          owed; no key exists yet                QB12a
   +0.0009                          checked against the RUN                QB12b
   \ref{tab:descriptives}           rows in the panel; checkable on sight  QB12c
   \ref{fig:discretion-gradient}    two pictures; NOT checkable            QB12d
...
Six entries in the `.bib` is deliberate: only the keys these examples cite. Adding a key means copying the REAL entry out of that paper's bibliography, never writing one, which is the same human-only boundary `QB12a` states. To make a new citation state showable, copy in the entry (or the probe entry) that already produces it; do not invent one to make a colour appear.
...
- `ready`
  Something landed and the manuscript has not caught up, whether what landed is a probe answer or a rendered figure (QB12d).
- `parked`
...
- `stale`
  An asset older than its `source/`, so the manuscript is compiling the old one. Computed from mtimes, never declared (QB12c).
- `candidate`
  Retired into `ready` when QB12d was built: the same fact in two vocabularies.
