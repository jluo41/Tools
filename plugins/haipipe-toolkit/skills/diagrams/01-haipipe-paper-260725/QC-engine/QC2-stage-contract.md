...
```
   ...
   ① THE SKILL · reusable, ships, never mentions a paper ────────────
      skills/paper/route/haipipe-paper-stage/
...
- [x] 📐 State the fields by measurement rather than by inference
      `../../paper/route/haipipe-paper-stage/stages/CONTRACT.md` names 24 required fields in 7 blocks, and all eight `stage.md` files carry all 24, checked field by field. `../../paper/route/haipipe-paper-stage/check-contracts.py`'s `REQUIRED` list is the same 24, so the document and the checker cannot disagree.
- [x] 📐 Declaring a stage costs four files and no more
      One `../../paper/route/haipipe-paper-stage/stages/index.yml` row, one `stages/<order>-<key>/` folder, its `stage.md`, its `template.md`. No new skill, no version bump, no router edit.
- [x] 🔧 Move every declared PATH into a program
      `../../paper/route/haipipe-paper-stage/check-contracts.py` resolves them all; 22 of 31 were dead the first time it ran. The eight contracts declare 28 such paths today, and exactly one carries `blocked_on:`, at `4-display/stage.md:30`.
...
- [ ] 🧠 Give every silent required field a named reader
      The 16 are `order phases gates probe_depth runs needs_paper sections formatting probes q_anchor upstream downstream handoff done_criteria closed_when exit_when`. Each must end up checked by a program, read by a human at a named moment, or deleted. Two live ways to get there: rule all 16 one at a time, or rule the three GRAPH fields below and declare the remaining 13 craft prose by default. `order:` is the free one: it is copied from the `../../paper/route/haipipe-paper-stage/stages/index.yml` row and nothing reads it out of `stage.md`.
...
- [ ] 🔧 Recount the stage-specific field census in `../../paper/route/haipipe-paper-stage/stages/CONTRACT.md`
...
- `../../paper/route/haipipe-paper-stage/check-contracts.py`
...
- Declaring a stage costs four files and nothing else: one row in `../../paper/route/haipipe-paper-stage/stages/index.yml`, one folder at `stages/<order>-<key>/`, its `stage.md`, its `template.md`. No new skill, no version bump, no router edit.
