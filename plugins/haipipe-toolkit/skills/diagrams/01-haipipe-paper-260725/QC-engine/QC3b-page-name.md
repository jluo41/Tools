...
Scope: This page covers How many pages a stage writes, which page each addresses, which side owns the filename, and what a contract stores instead of one. Neighbouring pages cover What the folders are is `QA6`; what a family means on the board is `QA7`; what shapes the page once it exists is `QC3a`; what happens when the page already exists is `QC3c`; what goes into it is `QC3d`.
...
```
   ...
   ── the one address that still dangles ────────────────────────────
      4-display   artifact: 0-lifecycle/3-display/4-display.md
                  ⚠️ not an S-face name at all, and declared
                     `blocked_on: QC3b`. Its family folder is right and
                  ...
                  ⚠️ and 4-display/template.md:2 ALREADY says to copy
                     ...
                     the template is ahead of the contract.  → QC3a
   ...
   ── WHO READS THESE FIELDS, AND HOW THEY FAIL ────────────────────
      ...
      to bind  the REOPENED half has no field at all. "a family with no
               ...
               It becomes checkable only by diffing `../../paper/route/haipipe-paper-stage/stages/index.yml`'s
...
The family with no stage is the sharp end, and it is one family, not two. Submission holds four pages (`S-Submission-0-reconcile` through `-3-submit`), is a real member of `FAMILIES`, and is written by nothing in `../../paper/route/haipipe-paper-stage/stages/index.yml`. So either the lifecycle is not the only thing that writes to the board, or Submission is a stage that was never declared.
