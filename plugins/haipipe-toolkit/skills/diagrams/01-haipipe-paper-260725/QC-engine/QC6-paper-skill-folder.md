...
- [ ] 🗣 Do the paper and application families still share one folder skeleton?
      `application/` received paper's buckets in a planned port and `../../application/SOP-paper-alignment.md` keeps them in step. 42 files across the toolkit name these paths. Nothing can move until this is answered.
      ...
      C · restructure both. Paper gets `s01…s10` from its board; application gets `SNN-` folders from ITS board, which has different groups. Largest change, and the two stay symmetric in RULE rather than in folder names.
      → CC recommends B, because the skeleton was shared to give application a head start, not to bind paper forever, and `§4.2` shows the group axis cannot be ported anyway. C is right only if application's board is already stable enough to name folders after.
...
### A4 · ⚠️ The coupling that blocks the move
- 🧠 A4.1 · Waiting on JL. Found 260803 during pre-flight, before anything was moved.
