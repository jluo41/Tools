...
## Boundary
- ✅ Covered here
  What a paper board holds, how its state is derived, and why nothing graduates out of it.
- ↪ Covered elsewhere
  The design board is the opposite object and is `QA3`; what else is in the paper folder is `QA4`; the grain that decides how many S pages a stage gets is `QB4`; who creates one is `QBc4`.
...
```
 todo.md          QBd2 already rules that ## Items to Finish IS the queue
...
## Where we are
Ruled and live. The MISQ paper carries 39 S faces across the seven families, its frontier is read rather than stored, and its S pages hold the paper's real Content.
...
One S page per independently gated unit is the claim this page rests on, and one of the seven families does not follow it. Display has eleven S pages on the MISQ paper while its stage contract still says `runs: once`, and the ruling that would reconcile them is `QB4`'s open "Apply it to display". So the grain is ruled in general and unruled for Display specifically.

The md-to-tex edge stated above has the same problem one level down: `QBa1`'s "Rule what sync reads" is open, so the direction is ruled and the mechanism is not.
