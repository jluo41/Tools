...
- `ready`
  Something landed and the manuscript has not caught up, whether what landed is a probe answer or a rendered figure (QC4).
- `parked`
...
- `stale`
  An asset older than its `source/`, so the manuscript is compiling the old one. Computed from mtimes, never declared (QC3).
- `candidate`
  Retired into `ready` when QC4 was built: the same fact in two vocabularies.
