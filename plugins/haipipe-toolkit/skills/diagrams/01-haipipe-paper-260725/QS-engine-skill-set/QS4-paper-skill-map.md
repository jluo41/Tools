...
## Boundary
- ✅ Covered here
  The first skill-page cohort for a Board-first paper lifecycle and venue-grounded scientific prose, and the criterion for adding another page.
- ↪ Covered elsewhere
  Board interaction belongs to `Q-Skill@boardform`; venue packs remain knowledge sources rather than callable skills. How a ruling becomes shipped text had a face of its own until 260727, when JL retired the three governance faces (graduation, versioning, per-skill status) into `_archive/`; the graduation rule itself still lives in `haipipe-board`'s own manual, which is where it was always stated.
...
### The first six pages
- **1. `haipipe-paper` → `Q-Skill-haipipe-paper`**
...
- **2. `haipipe-paper-lifecycle` → `Q-Skill-haipipe-paper-lifecycle`**
...
- **3. `haipipe-paper-stage` → `Q-Skill-haipipe-paper-stage`**
...
- **4. `haipipe-paper-draft` → `Q-Skill-haipipe-paper-draft`**
...
- **5. `haipipe-paper-probe` → `Q-Skill-haipipe-paper-probe`**
...
- **6. `haipipe-paper-revise` → `Q-Skill-haipipe-paper-revise`**
