...
## Items to Finish
- [ ] 💣 Syncing `S-Main-2` as written would STRIP EVERY CITATION from §1
      Measured 260727, and it is the sharpest argument this face has for why the extraction rule cannot stay a discipline. That page's `## Content` carries its 14 references as PLAIN-TEXT author-year. `sections/01_introduction.tex` carries them as real `\citep{}`. They agree key for key, so nothing looks wrong from either side.
      But sync runs `.md` → `.tex` one way. Run it on this page today and all 14 `\citep{}` commands are replaced by prose, and the section's bibliography silently empties. Nobody would notice until a compile, and the diff would look like ordinary prose editing.
      Two things follow. The extraction rule must say what a plain-text reference in Content BECOMES, and the answer cannot be "pass it through". And a generator needs a refuse-to-regress check: never emit a section with fewer resolving keys than the file it replaces.
...
- [ ] 🚨 Reconcile the TWO float homes, before anything else here
