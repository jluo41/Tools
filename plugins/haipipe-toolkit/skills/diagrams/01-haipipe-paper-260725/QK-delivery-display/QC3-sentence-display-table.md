...
- 260727 · The ruling landed on `QC4` and this side consumed it without a change of its own, which is the same evidence `QC1` recorded when `QC2`'s bracket resolver shipped: these are faces of one mechanism, not parallel ones. Four table chips moved `ok` to `owed`, all on `S-Display-4`, the main regression table.
- 260727 · JL asked whether a display panel could also point at that display's Stage page on the paper board. It already does, from earlier the same day, and verifying the count rather than the code turned up two things for this side. The chip census here was stale: 38 table chips now, not the seven recorded 260726, because the `> Display:` lanes on `S-Main-4` and `S-Main-5` landed in between. And a table chip goes green off disk while the unit's own S page may say it is blocked, which is the same defect `QC4` now carries the ruling for.
