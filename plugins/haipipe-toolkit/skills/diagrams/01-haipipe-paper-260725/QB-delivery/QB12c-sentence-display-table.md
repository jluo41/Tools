...
```
 ...
 WHY A TABLE IS NOT A FIGURE (and QB12d is a separate face)
 ...
 WHERE THIS PAGE'S EVIDENCE LIVES IN THE BOARD FOLDER
   the prose      QB12c-sentence-display-table.md  ## Content   (S5)
...
The paragraph's other sentences are the sibling pages': S1 to S3 the citations are `QB12a`, S4 the numbers is `QB12b`, S6 the figure is `QB12d`.
...
The finding is not on the board, it is in the audit, and it is the same shape as `QB12a`'s:
...
- 260727 · The ruling landed on `QB12d` and this side consumed it without a change of its own, which is the same evidence `QB12a` recorded when `QB12b`'s bracket resolver shipped: these are faces of one mechanism, not parallel ones. Four table chips moved `ok` to `owed`, all on `S-Display-4`, the main regression table.
- 260727 · JL asked whether a display panel could also point at that display's Stage page on the paper board. It already does, from earlier the same day, and verifying the count rather than the code turned up two things for this side. The chip census here was stale: 38 table chips now, not the seven recorded 260726, because the `> Display:` lanes on `S-Main-4` and `S-Main-5` landed in between. And a table chip goes green off disk while the unit's own S page may say it is blocked, which is the same defect `QB12d` now carries the ruling for.
...
- `haipipe-paper-revise-place`
  Turns a done DR row into `\input` + `\ref`. The `\input` PATH is the open question this face carries to `QB11a`.
- `4-display/`
  Owns the unit. This face only rules how a SENTENCE names it.

**Where the evidence lives** (live routes, repointed 260727 after the MISQ regroup; see `QB12d` for the translation table)
