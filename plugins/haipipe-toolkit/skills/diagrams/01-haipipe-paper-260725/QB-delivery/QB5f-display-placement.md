...
Scope: This page covers Which section a float lands in, what happens when two sections cite one unit, and whether the build reaches the float at all. Neighbouring pages cover What the caption and label SAY is `QB5e`. The folder the float lives in is `QB5c`. What a sentence citing it means is `QB5a` and `QB5b`. Float NUMBERING across the document is `QB9a`. The contract for how a selected unit reaches a reader-facing sentence is `QD3@display`, ruled and not re-argued here.
