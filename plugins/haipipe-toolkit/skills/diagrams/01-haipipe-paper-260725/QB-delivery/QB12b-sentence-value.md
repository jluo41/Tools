...
```
   ...
   the sentence   …at a mean absolute error of {VAL:? the deployed
                  model's MAE} [Q-Section-4]
                       ╰─ what is WANTED ─╯  ╰─ who owes it ─╯
                                │
                                │  ONE resolver, shared with QB12a
                                ▼
 ...
 WHERE THIS PAGE'S EVIDENCE LIVES IN THE BOARD FOLDER
   the prose      QB12b-sentence-value.md  ## Content   (S4)
...
The paragraph's other sentences are the sibling pages': S1 to S3 the citations are `QB12a`, S5 the table is `QB12c`, S6 the figure is `QB12d`.
...
So the resolver was built once, on the bracket, and both `QB12a` and `QB12b` consume it. Three citation chips that yesterday could only say "owed" now say the answer landed.
