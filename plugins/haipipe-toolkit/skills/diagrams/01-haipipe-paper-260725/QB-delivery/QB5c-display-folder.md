...
Scope: This page covers What the unit contains, what each part is for, and the authoritative-workspace versus submission-projection split. Neighbouring pages cover WHY a unit has both a page and a folder is `QA3@display`, ruled and not re-argued: "A page decides; a folder renders. Neither replaces the other." What the Intake inside `source/` must carry is `QB2@display`; how `candidates/` is promoted into `assets/` is `QB4@display`; who ASKED for the render that fills them is `QB5d`; the caption and label inside `float.tex` are `QB5e`; where the float lands is `QB5f`.
