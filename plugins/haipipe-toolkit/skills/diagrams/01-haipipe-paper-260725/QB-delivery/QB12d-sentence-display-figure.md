...
## Opening
When a sentence sends the reader to a figure, what is different from sending them to a table?
Mechanically nothing, and in what a reader can verify, everything. A table shows its numbers, so a wrong one is checkable on sight; a plausible-looking plot built from the wrong column is invisible.
...
```
 ...
 WHERE THIS PAGE'S EVIDENCE LIVES IN THE BOARD FOLDER
   the prose      QB12d-sentence-display-figure.md  ## Content   (S6)
...
The paragraph's other sentences are the sibling pages': S1 to S3 the citations are `QB12a`, S4 the numbers is `QB12b`, S5 the table is `QB12c`.

### Why this is not `QB12c` with a different file extension
Look at the two display chips in that one paragraph. `QB12c`'s table puts its ROWS in the panel, so a wrong number is checkable on sight. This one puts two PICTURES in the panel: you can see that both exist and which is live, and you still cannot tell from either image whether it was built from the right column. Same grammar, same resolver, same lane, and a reader's ability to check collapses. That asymmetry is the whole reason these are two faces rather than one.
...
```
 ...
 ⚠️ orphan            ❓ unowned         the id, or the \ref{} label, resolves
                                        to nothing
```
Same five rows as `QB12c` except `candidate`, which the table page has no equivalent for and which is the state most likely to produce a sentence that quietly stops being true. Two rows are worth reading twice: `promoted` and `candidate` collapsed into states `QB12b` had already built, for the reason below, and `folded` is still only a word on this page.
