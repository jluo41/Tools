...
**Where this page sits**: QB13 heads the float series, and `QA3@display` already ruled why a unit has both a page and a folder: a page decides, a folder renders, and neither replaces the other.
`QB2@display` owns what the Intake inside `source/` carries, `QB4@display` owns how `candidates/` is promoted, QB13b owns who asked for the render, QB13c owns the caption and label inside `float.tex`, and QB11c owns where the float lands.
...
- `QB9-build.md` · the concern whose G4 blocks on the one incomplete projection
