...
Scope: This page covers What a design board holds, what leaves it when a question closes, and why nothing may depend on it. Neighbouring pages cover The paper board is the opposite object and is `QA7`; the skill a ruling graduates into is `QA2`; where any board lives on disk, and its face grammar, are ruled on the boardform board at its `QC1@boardform` and `QA2@boardform`.
...
```
   ── which group lands where ────────────────────────────────────────
      ...
      QC3b-6  the page written create-page.py · stage.py  —
      QC4-11 the four phases  2-phase/ · ref/08-stage   —
...
```
 ...
 the wall                   NOT THIS FOLDER'S EDGE. A design board never asks
                            ...
                            asking, which is `QC4b`'s subject, not its own.
```
