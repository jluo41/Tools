...
```
   ...
   ruled by ②        user intent
                         │
                         ▼
   QA2 ──────────▶  haipipe-paper/   thin front door: resolve the paper, open
...
## Where we are
The ownership map, the anatomy, and now the per-stage run map are recorded. None of it has been applied: the current skills still duplicate several responsibilities, and no live paper skill has been reorganized in this Board pass.
