...
- ↪ Covered elsewhere
  What the page does with its dependencies is the Stage Contract block, `QBc1`; the ordering it implies is not Pages order, which the board already states.

## Diagram
```
 TWO DECLARATIONS OF THE SAME THING, AND IT IS NOT A TIE

  stage.md   read_order:            the page   requires:
  ┌────────────────────────┐        ┌────────────────────────────┐
  │ hand-maintained        │        │ rendered by sync           │
  │ an ORDER               │        │ a SET                      │
  │ "open the venue        │        │ names each source          │
  │  blueprint before      │        │ + ITS GATE STATE  ◄────────┼─ the
  │  the claims ledger"    │        │ + what it provides         │  difference
  │ craft guidance         │        │ AUTHORITATIVE              │
  └────────────────────────┘        └────────────────────────────┘

 WHY THE GATE STATE SETTLES IT
   a path says     "1b-claims exists"
   requires says   "1b-claims exists AND HAS NOT PASSED ITS GATE"
   the second is a different and more useful fact, and only one
   of the two forms can carry it.

 THE EVIDENCE
   the old inputs: on 5-section-edit listed z-structure,
   a dangling architecture path, wrong for weeks and noticed by nobody,
   because a hand-maintained list has nothing checking it.

 THE HONEST COST OF THE RULING
   an ORDER is not expressible as a SET. If read_order: dies, the craft
   guidance dies with it, so the ruling keeps it and strips its power:
   it states sequence and CANNOT create a dependency.
```
