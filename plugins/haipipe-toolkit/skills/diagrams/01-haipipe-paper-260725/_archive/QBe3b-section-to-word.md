...
Scope: This page covers What a citation, a value, a table, a figure, a heading, a lane and a why-comment become in a `.docx`; what tool performs it; and the ruling on an edit that comes back in the output. Neighbouring pages cover The LaTeX column is `QBe3a`. Which file is the paper is `QC3d`. The general one-source-many-projections model is `QD7` and the archived adapter contract is `QBe3c-format-adapters`. What each thing that hangs on a sentence MEANS is `QBe1a` to `QBe1d`.

## Diagram
```
 QC IS A MATRIX (JL 260726). QBe1a-QBe1d are ROWS, what hangs on a
 sentence. QBe3a and QBe3b are COLUMNS, where it is delivered.
 ...
 two rows flip the other way: a `> Value:` lane is DROPPED by QBe3a and
...
```
 QBe1b value        the number, inline                              ✅ free
 ...
 > lanes          the same, anchored to the NUMBER not the ¶      ✅ SOLVED
                  (QBe3a drops these; this column keeps them)
 the edit         partition returning comments by author          ✅ SOLVED
 QBe1a citation     baked author-year out of .board-refs.bbl,       ⚠️ ruling
                  which refs.py ALREADY generates with the
                  paper's own .bst; the key rides in a comment
 QBe1c table        a real w:tbl parsed from the unit's             ⚠️ ruling
                  assets/table-body.tex, so it stays EDITABLE
 QBe1d figure       assets/figure.png embedded; numbering is        ⚠️ ruling
...
```
Two audiences, one file, and the second one is invisible until a bubble is opened. That is the property `QBe3a` cannot offer: in LaTeX the apparatus is dropped at generate time, so a reviewer never sees it and neither does anyone else.
...
- [x] 🔧 An unresolved marker WARNS here and BLOCKS in LaTeX
      Not one ruling for both columns, which is how it was framed. The audiences differ: a reviewer must never see `[?]` in a results sentence, and a coauthor must see exactly where the paper stands, so a hidden hole is worse than a visible one. The export emits the marker, reports it, and continues. `QBe3a` should rule its own half the other way.
...
- [ ] 🚨 Eight of the nine `4-main` pages reference NO display in their prose
      Measured 260727 while exporting, and it is the same defect `QBe3a` found for citations on `S-Main-1`, one row down. Only `S-Main-4` carries a `\ref{}` in prose at all, and it carries three. Every other display pointer in the Main lifecycle lives in a `> Display:` lane, which both columns drop, so an export or a sync of any of those eight pages produces prose that points at no table and no figure.
...
- [ ] 📐 Reconcile the two other copies of the matrix
      The row-by-column table in this page's Diagram is duplicated in `QBe3a`'s Diagram and in `board.md`'s `QC` group intro. Two cells changed here on 260727 and those copies still read `> lanes DROPPED` and `%% {CC-*}: 🔴 NOWHERE TO PUT IT`. Three copies of one table is the drift `QC5` was split apart to stop.
...
- `displays/<unit>/float.tex`
  The `\label` and the unit's kind. Never guessed from the label prefix, per `QBe1c`.
...
- `_archive/QD4-format-adapters.md`
  The adapter contract this column is an instance of.

## Log
- 260803 · Left `QB · Delivery` for the new `QBe · Delivery Element` group, and `QB11b` became `QBe3b`, then took its place in the unit-size order ruled the same day (JL 260803: sentence, display, section); the old id resolves as a declared alias in `board.md ## Links`.
