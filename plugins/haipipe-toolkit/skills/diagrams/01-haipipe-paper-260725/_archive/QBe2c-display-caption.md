...
QBe3c owns where the float then lands, QBe1c and QBe1d own what a citing sentence means, and QBe3a owns float numbering across the document.
...
- `QBe2-delivery-display.md` · the series head, which owns why the float is its own unit
...
260803 · Left `QB · Delivery` for the new `QBe · Delivery Element` group, and `QB13c` became `QBe2c`, then took its place in the unit-size order ruled the same day (JL 260803: sentence, display, section); the old id resolves as a declared alias in `board.md ## Links`.
