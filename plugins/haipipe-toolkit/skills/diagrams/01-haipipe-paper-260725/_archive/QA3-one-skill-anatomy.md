...
## Diagram
```
   what a fresh agent loads, and in what order

   ① metadata           name + description        selects the skill
        ▼                                          cheap, always paid
   ② SKILL.md           the core loop, shortest complete procedure
        ▼                                          paid on every invocation
   ③ references/        loaded ONLY when the branch is taken
   ...
   the evidence that this is not theoretical:
     haipipe-paper/SKILL.md          556 lines   routing + UI + comments
                                                 + evidence + delivery
     haipipe-display-poster/SKILL.md 854 lines
```
...
## Law
Progressive disclosure. Metadata selects the skill; `SKILL.md` carries the shortest complete procedure and names exactly which conditional reference to read; `references/`, `scripts/` and `assets/` are loaded or run only when the branch is taken.

Design Boards, changelogs, feedback archives, migration narratives and status reports do not belong in the live invocation path. If history must be kept, keep it outside the callable skill folder.
