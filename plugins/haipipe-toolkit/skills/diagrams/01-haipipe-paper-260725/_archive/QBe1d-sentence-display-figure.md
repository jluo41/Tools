...
```
 ...
 WHERE THIS PAGE'S EVIDENCE LIVES IN THE BOARD FOLDER
   the prose      QBe1d-sentence-display-figure.md  ## Content   (S6)
...
The paragraph's other sentences are the sibling pages': S1 to S3 the citations are `QBe1a`, S4 the numbers is `QBe1b`, S5 the table is `QBe1c`.

### Why this is not `QBe1c` with a different file extension
Look at the two display chips in that one paragraph. `QBe1c`'s table puts its ROWS in the panel, so a wrong number is checkable on sight. This one puts two PICTURES in the panel: you can see that both exist and which is live, and you still cannot tell from either image whether it was built from the right column. Same grammar, same resolver, same lane, and a reader's ability to check collapses. That asymmetry is the whole reason these are two faces rather than one.
...
```
 ...
 ⚠️ orphan            ❓ unowned         the id, or the \ref{} label, resolves
                                        to nothing
```
Same five rows as `QBe1c` except `candidate`, which the table page has no equivalent for and which is the state most likely to produce a sentence that quietly stops being true. Two rows are worth reading twice: `promoted` and `candidate` collapsed into states `QBe1b` had already built, for the reason below, and `folded` is still only a word on this page.
...
## Log
- 260803 · Left `QB · Delivery` for the new `QBe · Delivery Element` group, and `QB12d` became `QBe1d`, then took its place in the unit-size order ruled the same day (JL 260803: sentence, display, section); the old id resolves as a declared alias in `board.md ## Links`.
