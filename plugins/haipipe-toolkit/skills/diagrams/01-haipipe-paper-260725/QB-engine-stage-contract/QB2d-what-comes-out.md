...
- ↪ Covered elsewhere
  What the page is called is `QB2b`; what a re-run does to it is `QB2c`; the sentence-level shape an extractor relies on is `QC0`; HOW a page becomes LaTeX is `QC5` and how it becomes Word is `QC6`.
...
## Items to Finish
- [~] ↪ MOVED to `QC5` · rule what sync reads · there is no generator · round-trip one section
      Three items left this page 260726 when JL opened the delivery columns. They are extraction and generation questions, which is what `QC5` owns; this face keeps the authority ruling, which is which of the two files is the paper.
- [~] ↪ MOVED to `QC6` · rule the external edit
