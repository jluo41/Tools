...
- ↪ Covered elsewhere
  What the contract declares is `QB1`; how the page gets created and named is `QB2b`; what a re-run does to a page already filled is `QB2c`; what DRAFT does with the shape is `QB3a`; what a `<!-- RULE -->` becomes once the prose is real is `QC0`.
