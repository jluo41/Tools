...
```
 ...
 ⚠️ MISQ ITSELF STILL HAS 0-displays/ AND 0-sections/
   It has not been through haipipe-paper-restructure. This fixture uses
   the layout QA6 ⑦ RULED, because a board should teach its own ruling.
   The resolver takes either: displays/ first, 0-displays/ as fallback.
   Measured 260726 — identical chip counts under both. Before that fix
   it knew only 0-displays/, and a conformed paper's \ref{} fell through
   to the "a \label that is not a display unit" branch and rendered
   GREEN, so every display reference on a migrated paper read as fine.
...
- S1, S2 · verbatim
  `0-sections/03_theoretical_framework.tex:9` and `:11`, which are also the prose of that paper's `S-Main-4-theory.md` P2. Both keys are in its `.bib`, so both chips are green.
...
- S5 · verbatim, LaTeX spacing normalised
  `0-sections/05-3_control_variables.tex:59` writes `Table~\ref{tab:descriptives}`. A stage page writes prose, not LaTeX spacing, so the `~` is dropped and the `\ref{}` kept, which is one of the three forms the resolver accepts.
...
- [ ] 🧩 Get the chip trigger out of `<summary>`, so the panel needs no script
      A chip inside a sentence drawer only opens because `board.js` asserts it; everywhere else the native `popovertarget` is enough. That is the one interaction on this board that regressed from script-free to script-assisted, and the fix belongs to `QA9`'s sentence-row mechanism on the boardform board, not to a paper ruling here.
...
> CC: Real, and it was the sentence drawer. A `<summary>` runs its own activation behaviour on a click anywhere inside it and consumes the event before a nested button's default action, which for these chips IS showing the panel. Measured on this page in one build: the three citation chips sit in plain paragraphs because S1 to S3 carry no `>` lane, and they opened; the eight chips in S4, S5 and S6 sit inside `<details class="sent"><summary>` because those sentences carry lanes, and every one of them was dead. That is exactly "citations work, values and displays do not". The embed had been hiding it, because `render_doc` builds no sentence drawers, so while the example came from an embed every chip sat in a plain paragraph. Fixed by asserting the panel explicitly one frame later, and only when the browser has not already opened it. `preventDefault` stays out of that file: it was the first attempted fix and it cancelled the panel too.
> CC: The honest cost is that this one interaction now needs the script, where a chip outside a drawer does not. The page still READS with JS off (the panel is real body text in the DOM, and the `title=` tooltip is the documented floor), but the click no longer works there. The script-free fix is to stop nesting the trigger inside `<summary>`, which is `QA9`'s mechanism on the boardform board rather than a paper ruling, so it is filed as an item and not done here.
