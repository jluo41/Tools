...
- ↪ Covered elsewhere
  Who may write which region is `QA8`; the collaboration as a whole is `QA4`. The live layer's mechanism, one session per question, the drawer, the terminal and in-page locking, is ruled on the tool's own board at `../01-boardform-260722/` (`QD1`-`QD3`, `QE4` THERE, not here).
