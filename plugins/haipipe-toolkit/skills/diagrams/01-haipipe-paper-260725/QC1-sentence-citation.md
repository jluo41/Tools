...
- ↪ Covered elsewhere
  The sentence itself is `QC0`; the rendering mechanism is `QA9`; the placeholder grammar shared with values is `QB8`; how a key becomes LaTeX or Word is `QD7`.
...
`\citep{stock2005testing}` sits in `0-sections/D_iv_analysis.tex` on this paper today and resolves to nothing, so it will compile to `[?]`:
