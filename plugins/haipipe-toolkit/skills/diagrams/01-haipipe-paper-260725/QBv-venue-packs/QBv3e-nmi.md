...
**Why this outlet is the tree's exception**: `QBv1a` records MISQ refusing method-as-contribution outright, and `QBv2a` records JAMA refusing an AI paper with no patient-outcome impact.
...
**Never transcribe the section norms**: word budgets live in `NMI/NMI-<section>/style.md` and are cited, never copied.
...
```text
  ...
  ❌ rejected for an unvalidated "human-level" claim
     ...
     ── this is a claim-scope error, the same class QBv3d
...
- `QBv3a-npj-digital-medicine.md` · where a dataset-only paper goes when the claim becomes clinical
- `QBv1a-misq.md` · the desk that refuses outright what this one accepts
