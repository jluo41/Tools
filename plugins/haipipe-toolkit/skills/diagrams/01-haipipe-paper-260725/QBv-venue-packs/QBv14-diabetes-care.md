...
RESEARCH DESIGN AND METHODS names the design in its opening sentence, `We conducted a <design> analysis using <data source>.`, and the pack's four exemplars differ only in which design word fills that slot.
...
The four labels are fixed text rather than paraphrases: `Why did we undertake this study?`, `What is the specific question we wanted to answer?`, `What did we find?`, and `What are the implications of our findings?`
The pack budgets them at 1-2 sentences, 1 sentence, 2-3 sentences carrying numbers, and 1-2 sentences (style.md lines 25-29).
Each answer is full narrative prose, where a JAMA Key Points box carries three labelled one-liners under Question, Findings, and Meaning, which is why a retarget cannot map one box onto the other.
...
The pack names eight failures here: opening OBJECTIVE with the study description before the clinical context, adding a JAMA IMPORTANCE heading, reporting P values without effect sizes, using causal verbs on an observational design, writing CONCLUSIONS past what the data show, burying the sample size in the middle of RESULTS, leaving an abbreviation undefined inside the abstract, and using any JAMA-style header.
...
That table hedges its own display cap with "verify current author guidelines", so the four-display figure is an expectation this page carries and not a gate it can enforce.
...
Each word figure is the pack's stated range followed by what the pack actually measured, which is how the Original Article number turns out to sit exactly at its cap while the Brief Report number sits inside its range.
...
🧭 Establishes the introduction as the one body section whose measured exemplars ran at half the budget the pack first stated, which is why its stated range is so wide.

#### 4.1 · Signature moves, as slots
(style.md "Paragraph-by-paragraph structure" lines 33-84)
P1 opens on the technology rather than on a burden statistic, `<Technology> has been shown to improve <outcome> and reduce <event> in people with <condition>.`, from Reaven 2026; the pack calls this technology-first framing and contrasts it with the JAMA habit of leading on prevalence.
...
P3 carries the pivot plus one of four stock gap phrases, `However, there are very limited data on <relationship>.` or `However, the application of <method> to <task> remains unexplored.`, from Reaven 2026 and Zheng 2025.
...
The pack names eight: a textbook-definition opening, more than two paragraphs of known material before the gap, a causal hypothesis on observational data, methods detail beyond a brief design mention, a gap sentence with no adversative, a results preview, an omitted ADA Standards citation where the topic touches a recommendation, and a passive aim statement.
...
The pack already reconciled this budget once, cutting the stated Original Article range from 600-1000 words to 250-1000 after both exemplars measured under it, and that correction is why the range reads unhelpfully wide today.
...
The citation row is the only one here a writer can feel while drafting, since about 0.6 markers per sentence in an Original Article against one marker in a whole Brief Report is a visible difference on the page.
...
The other five are readings: a statistical method with no specific model named, a missing software version, results leaking into methods, future tense in place of past, and a missing trial registration number on a registered study.
The pack also records a JAMA contrast worth carrying, that JAMA methods almost always name a reporting guideline in their opening sentences while only one of the ten Diabetes Care exemplars named one at all.
...
A Brief Report does not write shorter methods so much as relocate them: Galindo 2026 fits 500 to 800 words by pushing the full inclusion and exclusion criteria and the self-adjustment guidelines into the Supplementary Material.
...
The citation row is what separates this section from the introduction, since a methods marker names an algorithm or a prior methods paper rather than supporting a finding, and a Brief Report can legitimately carry none at all.
...
The two-hyphen form is how the pack itself writes it on disk and how this page must write it; the printed journal sets the same separator as a dash, which is the character a repo-wide rule forbids in prose.
...
The opening paragraph reports the analytic N first, then demographics, then its cross-references, `Among <N> participants enrolled, <n> were included in the analysis.` followed by `The sample's mean age was <M> years, <p>% (n = <n>) were women (Table 1).`
Numbers take fixed forms: a mean and SD as `<M> ± <SD>`, counts as `<n> (<p>)`, P values italic and without a leading zero as `P = 0.28`, and intervals as `95% CI <lo>-<hi>` or the comma variant `(95% CI <lo>, <hi>)`.
Null results carry their interval or P value and no softening, so a reader judges precision rather than tone.
The hero display for a CGM paper is a stacked bar of time in each glycemic range, Very Low under 54, Low 54 to 70, Target 70 to 180, High 180 to 250, and Very High over 250 mg/dL, in the standardized colors.
...
The pack names eight: interpreting inside Results, saying significant with no number attached, P values with no effect size, methods statements in Results paragraphs, causal verbs on observational data, spinning a null result as a trend, burying the primary outcome behind secondary ones, and dropping the CGM threshold ranges.
...
The article type caps the displays this section may commission: an Original Article gets four tables and figures, a Brief Report up to three in total, and an e-Letter or Observation one or two.
...
A Diabetes Care Results section is among the most value-dense prose this whole tree covers, since one result per sentence means an estimate with its interval or its P value in almost every line, and the CGM metrics TIR, TBR, TAR, GMI and MARD each drag their thresholds along behind them.
The pack counts words per sentence and citation markers per sentence for exactly this section and never counts the numbers, and no style.md in the venue tree does either, all 95 of them read on 260802.
...
The flat format runs unlabeled paragraphs and closes on `In conclusion, ...`, which is what Galindo 2026 does across six paragraphs.
The subsectioned format gives the same arc named subheadings, and Reaven 2026 uses five of them: effects on the primary outcome, effects on interim outcomes, potential mechanisms of protection, limitations, and conclusion.
...
Nine of them, and three are string tests: a heading reading DISCUSSION, `In conclusion` or `In summary` opening a paragraph that is not the last, and a missing ADA Standards connection where the topic touches a recommendation.
The other six are readings: opening on a mechanism before the principal findings, causal verbs on observational data, new analyses appearing here for the first time, limitations written as one run-on sentence, an omitted design name in the closing paragraph, and overstated implications on a null result.
...
An e-Letter or Observation has no separate section at all, and its roughly 200 to 400 words of interpretation sit inside the running body, which is why a heading check has to know the article type before it may fire.
...
The citation row is the only metric on this page with a zone inside it, since markers cluster in the comparison and mechanism paragraphs and fall to zero in Limitations and in the closing paragraph of both measured papers.
...
No package number exists anywhere, which is the whole difference from the JAMA convention of Supplement 1, Supplement 2, and Supplement 3 carrying protocol, data, and data sharing.
...
One item that looks like it belongs here does not: the data-sharing statement is the Data and Resource Availability subsection closing RESEARCH DESIGN AND METHODS, and the two are routinely confused because both concern data availability.
...
Two labels are ADA-specific and arrive wrong on any manuscript coming from the JAMA portfolio: Duality of Interest is what JAMA calls Conflict of Interest Disclosures, and Handling Editors has no JAMA counterpart at all.
Author Contributions must carry the guarantor sentence, naming one author as having had full access to the study data and taking responsibility for its integrity and for the accuracy of the analysis.
Order, label text, and the guarantor sentence are all string-level facts, which makes the back matter the second most checkable block in the manuscript after the abstract.
...
What those counts do say is that the load tracks the study design rather than the article type: a cost-effectiveness Original Article ran 17 supplementary tables while an NLP Original Article ran one, and one e-Letter carried nothing at all.
...
Three empty rows here are the pack drawing a boundary rather than leaving a hole, since it says outright that prose metrics do not apply to a supplement and reports item counts in their place.
...
Everything else this section owns is printed as unattributed template text: the guarantor sentence carries a bracketed initials placeholder, and the clean Duality of Interest form and the figshare hosting line name no paper at all.
...
260802 · Added one Content division per section kind (3 abstract, 4 introduction, 5 methods, 6 results, 7 discussion, 8 appendix), each carrying its arc, its sourced budget, its slot patterns, and the pack's own anti-patterns; folded the retired pack head page's ADA-versus-JAMA delta and article-type table into them; relaxed the Writing Style bullet from never transcribing a number to always sourcing one.
260802 · Added an Authority block to Files from the Diabetes Care instructions for authors, fetched that day, which closes the expectation-versus-gate hedge for Original Articles and corrects three of the pack's other types. The Original Article limits are published and match the pack exactly: 4,000 body words, 250-word abstract, 40 references, 4 tables and/or figures. Brief Reports do not: 1,500 words against the pack's about 2,500, a 150-word abstract cap, and 4 displays against the pack's up to 3. Letters are two limits and not one, 500 words for comments and responses against 750 for Observations, both under the about 1,000 the pack carries. Article Highlights print the second question with a plural marker the pack drops and carry a 75 to 130 word cap it does not record. The caption separator is nowhere in the instructions, so 6.1's form is house typesetting rather than a published rule.
260802 · Added two subsubsections per section division: a `Format values` block carrying WORDS, CITATION DENSITY, VALUE DENSITY and DISPLAYS with each value sourced to a pack line, and a quotation set from the pack's own exemplar sentences. VALUE DENSITY came back empty in all six, since no style.md in the venue tree counts numeric values per sentence, all 95 read on 260802; the appendix additionally records no word and no citation figure by the pack's own statement. Labelled every word budget and every display cap an expectation rather than a gate, because the README hedges its cap with "verify current author guidelines" and style-profile.md still leaves "Exact word limits per article type" unchecked.
