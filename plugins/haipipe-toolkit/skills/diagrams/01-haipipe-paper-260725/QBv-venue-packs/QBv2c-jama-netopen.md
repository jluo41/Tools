...
state: 🟡 PARTIAL · 3 exemplars filed under the flagship · 6 sections · taste ✓ · the only outlet with no examples folder
owner: JL
...
**What is wrong with its folder**: it has no `examples/` at all, and its three exemplars sit under `jama-flagship/examples/`. It also carries six section folders where both siblings carry seven, with no `jno-letter/` and no note saying why.
...
**Never transcribe the section norms**: word budgets live in `jama-netopen/jno-<section>/style.md` and are cited, never copied.

**Never call this outlet a fallback**: `QBv2` ruled the descent a planned route, and fallback language contradicts the ruling on the page that instantiates it.
...
```text
  ...
  ⚠️ FOLDER DEFECTS
     no examples/ ── its 3 exemplars are under jama-flagship/
...
```text
  📂 exemplars ── 0 here, 3 under jama-flagship/examples/
...
- `QBv2a-jama-flagship.md` · the folder currently holding this outlet's exemplars
- `QBv2b-jama-im.md` · the outlet the descent to here is planned from
