...
**Where this page sits**: it is the only outlet under `QBv4`, and `QBv4` owns the pack-level facts about `playbook-pnas`.
...
`QBv0` A3.1 owns that placement split; this page is the outlet where it is most confusing, because `playbook-pnas/pnas/` doubles the name.
...
**Never transcribe the length numbers**: they live in the pack and are cited, never copied.
...
🚪 Establishes the split as the outlet's own defect, distinct from the pack-level placement question `QBv0` owns.
...
`QBv4` records that this test needs a sentence rather than a study, which makes it usable on the venue stage's `--no-pin` path.
...
- `QBv4-pnas.md` · the pack this outlet is the whole of

<!-- exemplars:begin -->

📚 **Exemplars** · 34 papers on disk, regenerated by `_tools/sync-exemplars.py`

Filed at FAMILY level under `../../paper/venue/playbook-pnas/examples/`, not under the outlet (QBv0 A3.1).
