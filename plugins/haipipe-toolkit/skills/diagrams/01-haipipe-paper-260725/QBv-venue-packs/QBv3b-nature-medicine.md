...
state: 🟡 PARTIAL · 25 exemplars · 7 sections · taste ✓ · the validation bar is a study-design property no retarget can supply
owner: JL
...
`QBv3` records this as one of two Nature outlets decided before venue runs; `QBv3a` is the other.
...
**Never transcribe the section norms**: word budgets live in `nature-medicine/natmed-<section>/style.md` and are cited, never copied.
...
```text
  ...
  📊 25 exemplars ── the largest base in the whole tree
```
...
⏳ Establishes this outlet as a task-layer decision, which is the class `QBv3` names and no stage currently enforces.

#### 1.1 · The same trap exists at ISR, in a different field
(so it is a tree-wide pattern rather than a Nature quirk)
`QBv1b` records that a clean identification strategy is likewise unacquirable at revision.
...
### A1 · ⏳ The bar is set before the paper exists
- ⬜ A1.1 · Not started, and shared with `QBv3a` and `QBv1b`.
...
- `QBv3a-npj-digital-medicine.md` · the sibling this outlet is decided against
- `QBv1b-isr.md` · the same unacquirable-property trap in the IS family
