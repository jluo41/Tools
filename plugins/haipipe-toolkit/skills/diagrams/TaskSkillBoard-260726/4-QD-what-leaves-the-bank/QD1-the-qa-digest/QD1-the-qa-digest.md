...
The contract for that line belongs to `haipipe-probe` and is linked rather than repeated here,
because two places stating one contract is how the two versions start to differ.
It was argued on the probe design board until that board retired on 260804 and the ruling shipped into the skill.
