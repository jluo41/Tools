...
## Boundary
- ✅ Covered here
  The test that decides whether something is a probe at all, and where a non-probe question goes.
- ↪ Covered elsewhere
  The skill's four verbs, as part of what the shared model ships, are `QA2`; what `status` derives is `QC3`.
  The loop a real probe enters is `QB1`.
