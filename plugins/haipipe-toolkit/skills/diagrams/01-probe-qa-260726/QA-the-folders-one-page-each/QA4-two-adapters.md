...
## Diagram
```
   ③ paper/2-phase/1-probe/haipipe-paper-probe/          v0.7.0
...
- [ ] 🐛 The cwd-dependent `deferred-undeclared` is fixed in `③`
      One test in the PASS 1 loop re-opens the file by its display name instead of the loop variable, so it false-fails every `deferred` entry unless cwd is the paper root.
      1096 lines reconciled by hand is exactly the condition under which a defect like this survives; `QC2` owns the fix and it is one word.
