...
- [x] 🏃 The checker has been run on a real consumer
      Run 260726 against the MISQ paper: **exit 1, 12 FAIL, 13 PASS**.
      It does not pass, which is the answer this item wanted; what it fails on is now known rather than guessed.
- [ ] 🧯 The 12 real failures are fixed or triaged
      5 `deferred-undeclared`, 5 `answered-not-read`, 2 `commissioned-target-answered`.
      None is a checker bug: each names a probe entry whose state on disk contradicts what it claims.
...
## Where we are
The checker exists, is tested against fixtures, and enforces three times what the manual says it does.
