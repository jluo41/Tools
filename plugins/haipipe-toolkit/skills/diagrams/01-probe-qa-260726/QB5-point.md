...
```
   ...
   commissioned-target-answered the answer landed, entry still waiting
     together: 7 of the 12 real failures on the MISQ paper
```
...
(`answered-not-read` 5 times and `commissioned-target-answered` twice, on the only real consumer)
...
## Where we are
Newly separated on 260726, and it was the step most worth separating: it had no page, it owns five of the checker's codes, and 7 of the 12 real failures on the MISQ paper are its.
