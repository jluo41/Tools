...
owner: JL
method: QC3's Law verbatim, a mechanical move under a response-identical gate; mixins so no signature changes; the areas that stopped moving go first
...
This is `QC3`'s deferred half, not a new idea.
...
```
   ...
   src/            the RENDER half           live/activity.py     the focus-time DB
     ...
     over 8 modules (QC3, shipped 260724)
                                            src/common.py stays the one shared floor
   one process · one port · 5599 only       both halves, no duplication (QC3 Law)
```
...
`QC3`'s Law rules this without needing a new one: refactors move code under a byte-identical gate, and features never ride along in the same step.
...
The board's own word for that half is the live layer (`QC3`), so `live/` uses vocabulary the board already speaks.
The modules are named for what they SERVE, mirroring `QC3`'s rule that render modules are named for what they render.
...
- [x] 🧪 Build the gate before moving anything
      ...
      `QC3`'s Lesson repeated itself on the first run: the gate caught `@staticmethod` decorators being dropped by the slicer, which would have shipped eight silently broken methods.
...
- 260731 CC · 🧭 This is QC3's deferred half, and its Law already governs it
  `QC3` shipped the render split on 260724 and recorded that it left the live layer alone on purpose, because that layer was still forming.
  ...
  What this page adds is the part `QC3` could not answer at the time: which areas have since stopped forming, and how a handler class splits when its methods share `self`.
...
- [x] 🔧 Rule the split mechanism
      ...
      → CC's proposal: A now and B never as one step; `QC3`'s Law says a refactor moves code and features never ride along, and B changes every signature.
- [x] ⏱ Rule the order
...
- [x] 🏷 Rule the naming, in JL's own words against the board's
      ...
      A · a `live/` package (`live/chat.py`), which mirrors the shape `QC3` shipped for the render half and keeps the skill root at its current 9 scripts.
      B · flat `server_*.py` files beside serve.py, exactly as JL named them, which costs nothing if the flat names read better.
      → CC's proposal: A; it is the same shape `QC3` shipped for the render half, and the root stays scannable, but B costs nothing if the flat names read better to JL.
...
260731 · Opened from JL's ask ("could we separate them?"), inheriting QC3's Law; modules-not-servers, mixins, and the settled-areas-first order proposed as three Decision Now rows
