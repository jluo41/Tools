...
## Opening
Does `haipipe-board` give a newcomer one reliable way to open, run, update, and close a Board?

This is the public door that connects the source folder, generated site, live tools, and write-back discipline.
Its challenge is staying small while the detailed page, sentence, routing, and index contracts continue to change.
Every Board session depends on this entry point choosing the right action and handing work to the right unit.
It is healthy when a fresh agent can complete the workflow without hidden context or duplicated rules.

## Diagram
<!-- haipipe:skill:tree:start 9b88974a640649e8 board/haipipe-board -->

```
haipipe-board/
  assets/
    css/
      00-base.css              283 ln  palette + type follow html-ppt's academic-report; layout is body.single
      10-focus.css             337 ln  单题聚焦：纯 CSS，零脚本
      20-drawer.css            412 ln  每题一个对话窗（QD1）
      ...
      70-page list.css           131 ln  Pages page list (JL 260731): Index → group → page, hideable
    ...
    js/
      10-drawer/
        ...
        20-chat/
          ...
          10-sessions.js       461 ln  session 拣选器（QD1 Law 修正 260731：一题多 session，一个 current）
          ...
          50-prefs-paste.js    653 ln  模型 / effort / 权限档 记在本机；default Opus 5 · high · full·ask
        00-open.js              33 ln
        30-terminal.js         469 ln  ⌨ 真终端：同一个 session 换个窗口
      ...
      40-sentence/
        00-apparatus.js        235 ln  ➕ sentence apparatus add (QA8, JL 260725): click a bare prose sentence, or the
      ...
      20-live-refresh.js       260 ln  live refresh (QD6, JL 260724)
      30-expand-all.js           6 ln  section「expand / collapse all」
      50-activity.js           282 ln  QD8 · activity timing and layout
      60-page list.js            210 ln  Pages page list: toggle, per-board persistence, active row (JL 260731)
  ...
  checks/
    ...
    splitshell.mjs             315 ln
    termnav.mjs                185 ln
  cli/
    build.py                   164 ln  Board source folder -> generated board/ site.
    check.py                   985 ln  check.py — the structural half of QA9, run against one board.
    ...
    serve.py                   494 ln  Serve boards AND accept comment writes — from the machine the files live on.
    skillpage.py               755 ln  One skill folder -> one Q page on a board (QC5, opened by JL 260726).
    stage.py                   421 ln  Create and synchronize lifecycle S pages with explicit inherited contracts.
  ...
  live/
    ...
    chat.py                   1303 ln  QC8 · the Claude Code bridge (QD2): rules, prime context, the SDK turn.
    home.py                    238 ln  The read-only SPACE-level Board Home.
    shell.py                   743 ln  QD5 · the operating shell: index · page · chat, three frames that ignore each other.
    ...
    write.py                   337 ln  QC8 · the write path (QC7, QB5): a typed line under its anchor sentence.
  ...
  ref/
    ...
    board-form.md              436 ln  Board form: full specification
    page-template.md           358 ln  Short title in sentence case: say what this page is FOR
  ...
  src/
    ...
    body.py                   1294 ln  The board's body grammar -> html (ref/board-form.md §5): inline marks, link
    ...
    page_board.py             1025 ln  The cover + index + whole-page assembly (QB5: render(), asset inlining,
    ...
    parse.py                   374 ln  md -> data (QB5): board.md, Q files, folder discovery, legacy blocks.
  ...
  tests/
    ...
    test_home.py               131 ln  Read-only discovery and rendering checks for the SPACE Board Home.
    test_quality_check.py       64 ln  Executable safety checks for the one-click, read-only Quality Check.
  ...
  vendor/
    xterm/
      ...
      xterm.min.js               2 ln
  CHANGELOG.md                1743 ln  haipipe-board — Changelog
  SKILL.md                     699 ln  /haipipe-board: one topic, one source tree, one generated Board site
  status.py                    352 ln  Render the three-line closing block for one Board-attached session.
```
...
## Content
<!-- haipipe:skill:body:start 9b88974a640649e8 board/haipipe-board -->

**haipipe-board** · `0.104.1` · last shipped 2026-08-02
...
- 1 · 👪 The family: one door, three specs, one verb (QC1b §2, shipped 260731)
...
- 4 · 🗣 A decision lives on the page
...
- 5.1 · view · open an existing board (the most common case)
...
- 5.4 · stage · create or refresh a lifecycle page
...
- 5.6 · the round trip and its unit
...
- 5.7 · serve · bring the board alive (the live layer)
...
- 5.8 · excalidraw · one scene per board, one frame per question
...
- 5.9 · comment / edit · comments and edits on a sentence (needs serve.py running)
...
- 5.10 · sync · when the work is done, write it back to that question in the same round
...
- 5.11 · link · connect the board to its products
...
- 7 · ✍️ Writing (the part most often skipped)
...
- 9 · 📖 The board ↔ SKILL.md: how they stay in sync
...
107 files besides `SKILL.md` and `CHANGELOG.md`, each with the purpose it states about itself. They are described here, not reproduced: the folder is the copy.

```
assets/board-mark.svg               44 ln
assets/css/00-base.css             283 ln  palette + type follow html-ppt's academic-report; layout is body.single
assets/css/10-focus.css            337 ln  单题聚焦：纯 CSS，零脚本
assets/css/20-drawer.css           412 ln  每题一个对话窗（QD1）
...
assets/css/70-page list.css          131 ln  Pages page list (JL 260731): Index → group → page, hideable
...
assets/js/10-drawer/10-comment/00-highlight.js   123 ln  highlighting
...
assets/js/10-drawer/20-chat/10-sessions.js   461 ln  session 拣选器（QD1 Law 修正 260731：一题多 session，一个 current）
...
assets/js/10-drawer/20-chat/50-prefs-paste.js   653 ln  模型 / effort / 权限档 记在本机；default Opus 5 · high · full·ask
assets/js/10-drawer/30-terminal.js   469 ln  ⌨ 真终端：同一个 session 换个窗口
assets/js/10-drawer/35-imagepick.js    99 ln  🖼 Attach an image WITHOUT a clipboard (QD14, JL 260801: "然后我们手机上的话，
assets/js/10-drawer/40-follow.js    79 ln  面板跟着题走（JL 260723）
assets/js/10-drawer/50-structure.js   317 ln  index structure controls (QC2, JL 260724): add / archive groups and
assets/js/20-live-refresh.js       260 ln  live refresh (QD6, JL 260724)
assets/js/30-expand-all.js           6 ln  section「expand / collapse all」
assets/js/40-sentence/00-apparatus.js   235 ln  ➕ sentence apparatus add (QA8, JL 260725): click a bare prose sentence, or the
assets/js/40-sentence/10-address.js   221 ln  Automatic Content addresses + sentence-specific chat.
assets/js/40-sentence/20-breadcrumb.js   224 ln  Section and subsection breadcrumbs (QB5d)
assets/js/50-activity.js           282 ln  QD8 · activity timing and layout
assets/js/60-page list.js            210 ln  Pages page list: toggle, per-board persistence, active row (JL 260731)
assets/js/65-section-matrix.js      22 ln  SECTION MATRIX cells open their page at the named section (QB2).
assets/js/70-router.js             145 ln  board/ tree navigation (QC9, JL 260731)
assets/js/80-restore.js            103 ln  the reload must not cost you your place (QD2/QD6, JL 260731)
assets/xcal-boot.js                264 ln  Injected by serve.py into the proxied Excalidraw app (QA4a).
checks/chatui.mjs                  335 ln
checks/guichat.mjs                 231 ln
...
checks/splitgaps.mjs               228 ln
checks/splitgaps.py                174 ln  QD5 · the gap checks: everything checks/splitshell.mjs does NOT cover.
checks/splitshell.mjs              315 ln
checks/termnav.mjs                 185 ln
cli/build.py                       164 ln  Board source folder -> generated board/ site.
cli/check.py                       985 ln  check.py — the structural half of QA9, run against one board.
...
cli/serve.py                       494 ln  Serve boards AND accept comment writes — from the machine the files live on.
cli/skillpage.py                   755 ln  One skill folder -> one Q page on a board (QC5, opened by JL 260726).
cli/stage.py                       421 ln  Create and synchronize lifecycle S pages with explicit inherited contracts.
...
live/activity.py                   446 ln  QC8 · focus time (QD6): the SQLite span store and its aggregates.
live/base.py                       260 ln  QC8 · shared state, the request floor, and the helpers every mixin calls.
live/chat.py                      1303 ln  QC8 · the Claude Code bridge (QD2): rules, prime context, the SDK turn.
live/home.py                       238 ln  The read-only SPACE-level Board Home.
live/shell.py                      743 ln  QD5 · the operating shell: index · page · chat, three frames that ignore each other.
live/structure.py                  270 ln  QC8 · index shape (QB2): add a question, add a group, archive.
live/term.py                       857 ln  QC8 · the terminal (QD3): PTY, ring buffer, the /_term WebSocket terminus.
live/turnring.py                   171 ln  QD2 §9 R1 · the turn ring — what QD3's terminal has had all along.
live/write.py                      337 ln  QC8 · the write path (QC7, QB5): a typed line under its anchor sentence.
...
ref/board-form.md                  436 ln  Board form: full specification
ref/page-template.md               358 ln  Short title in sentence case: say what this page is FOR
...
src/body.py                       1294 ln  The board's body grammar -> html (ref/board-form.md §5): inline marks, link
...
src/page_board.py                 1025 ln  The cover + index + whole-page assembly (QB5: render(), asset inlining,
...
src/parse.py                       374 ln  md -> data (QB5): board.md, Q files, folder discovery, legacy blocks.
...
tests/test_home.py                 131 ln  Read-only discovery and rendering checks for the SPACE Board Home.
tests/test_quality_check.py         64 ln  Executable safety checks for the one-click, read-only Quality Check.
...
<!-- haipipe:skill:log:start 9b88974a640649e8 board/haipipe-board -->

Converted from the skill's own `CHANGELOG.md`: 138 releases.
...
260801 · `0.94.0`
      ...
      **Navigation stopped re-downloading a page list it throws away.** JL: "why I feel it will have a long time to navigate to different pages?" Measured: the server answers in 8ms and re-wiring a swapped page costs 4ms, so the fetch was the whole cost. Every page file carries the complete 112 KB page list and the router swaps only `div.wrap`, so 82% of a median 136 KB page is discarded on arrival, and both the fetch and the server said `no-store`, which re-downloaded it every visit. `no-store` was the wrong instrument for the guarantee it protected (JL 260726, "why now I cannot open them"): that requirement is never serve a page from before the last build, and it is spelled `no-cache`, meaning REVALIDATE BEFORE USE. Both sides now say it, a revisit costs 0.3 KB instead of 119 KB, and the staleness guarantee was re-tested by editing a page on disk and watching the next fetch come back full-size with the new bytes. The 112 KB duplicated into every page file is recorded as an open decision on QC4.
      **Two smaller repairs.** The page list's drag handle is `position:fixed` and placed off `--sbw` alone, so with the page list collapsed it stood 238px into the page as a bar across the text (JL screenshot); it now carries the page list's own two visibility conditions and moved from `80-matrix.css` to `70-sidebar.css` beside what it resizes. And `check.py` called `fig/image copy.png` a dead link because it compared a URL-encoded `%20` against a filesystem path; it unquotes first. Recorded on QB5c, QC4 and QB2a.
260801 · `0.93.0`
...
260731 · `0.86.0`
      **The page yields exactly as much as the page list takes, in both packagings.** JL: "when I drag the left panel the body text does not follow; board.html does, can you unify them." The page list's width had become `--sbw` in 0.85.0, but the body's `padding-left` was still hard-coded at `238px`, so widening the page list slid it OVER the text instead of pushing the text along. Both now read the same variable, which is exactly what `--chatw` already does for the chat drawer and the body it displaces: one variable, two consumers, no way for them to disagree. Verified over CDP by dragging in a real browser, and the two packagings return the same three numbers: page list 238 to 460, content left edge 369 to 480, content width held at 1000, identically on a `board/` tree page and in `board.html`. Recorded on QB2a.
260731 · `0.85.0`
      **A tree page is a FOCUSED page, and the page list can be dragged.** JL sent a screenshot of `board.html` in focus mode: "this is the format I want, look how wide it is, no boundary." A page in the `board/` tree is alone in its document, so it is permanently the focused page, and rather than inventing a second look for it (which is how the tree drifted three times today) all 79 `:target` focus-mode rules are now mirrored onto `body.split`. The card's coloured left bar, the Opening's accent bar, and the 820px width are gone on tree pages for the same reason they are gone in focus mode, and the index keeps them because it is not focused. The naive mirror needed two corrections found in the browser: it produced `body.split .q:target`, which a tree page never matches, and a `body.split .q{display:none}` that hid every page.
      **Page list drag (`--sbw`).** JL: "can the left panel be dragged, it feels fixed." One CSS variable, a handle that sets it, width remembered per machine: the same shape `--chatw` already uses for the chat drawer. Range 150px to 60% of the window, double-click the handle to reset. The handle had to become a FIXED strip OUTSIDE the nav, because the page list is `overflow-y:auto` and clipped an absolutely-positioned handle so thoroughly that `elementFromPoint` returned BODY. Verified by dragging it over CDP: 238 to 440, then 440 to 180, with 180 surviving a reload, a jump to another tree page, and the single-file board.
      **Two reversals, both mine.** Reading "I do not want boundary" as the `## Boundary` SECTION, CC removed it from 47 pages; JL meant the coloured bar down the left of the card. 41 were restored from git HEAD; 6 pages born today have no committed version and their Boundary text is lost. CC also removed the left page list entirely on a second misreading, and put it back.
260731 · `0.84.0`
      **The tree index and page list stop being a second implementation.** JL: "the ASCII here has not become real ASCII", then "compare your configuration with the original .md one, there are big differences, look carefully." Both symptoms had one cause: `render_tree` hand-wrote the index listing and the page list instead of reusing what `render()` already built. That silently dropped every `.gi` group-intro block, every `.gib` body, and all six `.gidia` figures (the per-group lane diagrams), so the ASCII was not failing to render as monospace, it was not being emitted at all; the page list lost its per-page section outline the same way, 54 `.sb-out` blocks and 298 `.sb-s` rows. Fixed structurally: the index loop is now `index_rows(meta, qs, href_for, group_href)` and the page list loop `sidebar_rows(qs, href_for, group_href)`, each parameterised only by how a link is spelled, and both packagings call the same function. Verified by a class-by-class diff of the two indexes: the only remaining differences are the progress bar and the ALL PAGES hint (both deliberate under JL's "only Map, Matrix and Activity" ruling) and one page list row, because the tree's Index is its own document. `pre.gidia` confirmed in the browser as `ui-monospace` with `white-space: pre`. This is the third time in one day that this family's own "one grammar, never two implementations" law caught its author, which is the argument for the law. Recorded on QB2.
260731 · `0.83.0`
...
260731 · `0.66.0`
      **The page list gains a per-page section outline, accordion enforced.** JL, pasting the Structure rows ("🎯 Items to Finish · 2 done · 1 open …"): should the sections be in the left panel too, "and make sure that everytime, only one pages's section and subsection can be opened". The Structure map's counting logic moved out of `render_structure()` into a shared `structure_rows()` (page_question.py), and the page list now renders those same rows — section emoji + name + meta, plus one indented row per Content division — under every page, hidden by `.sb-out` until that page is the open one; `mark()` collapses every other page's outline on navigation, so exactly one page's sections show at a time. Clicking an outline row navigates, then opens the target `<details>` and scrolls to it (division rows open Content first). Because drawer and page list read one source, they can never disagree. Owned by the new design-board face QB2a-page list.md, opened on JL's ask this round.
260731 · `0.66.0`
...
260731 · `0.61.0`
      **A hideable pages page list.** JL: "I also think to added the page list so I can choose the pages more easier ... like the side bar, and then index, QA, QA1, QA2, etc ... and that page list can be hidden as well." `page_board.py` now renders a fixed left page list from the same listing as the index rows — `🗂 Index`, then each group heading, then each page as state emoji + id + title — so the page list exists with JavaScript off and never drifts from `## Pages`. It sits OUTSIDE `.wrap`, untouched by the `:target` show/hide rules, so it stays up in both the Index view and an open page; a group link re-targets `#group-…`, which also restores the Index. `board.js` adds the ☰ toggle (state persisted per board in localStorage), the active-row highlight on hashchange, and the narrow-screen behavior (overlay, closed by default, a jump closes it). CSS default with no saved choice: open at ≥1150px, hidden below; the page list is print-hidden. Recorded on the design board's QB2.
260731 · `0.60.0`
