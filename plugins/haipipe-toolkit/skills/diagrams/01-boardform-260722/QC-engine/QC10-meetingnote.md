...
- ↪ Covered elsewhere
  How a folder is mirrored into a page at all: `QC5`, which already does exactly this for a skill folder.
...
`QC5` already settled this exact managed/authored split for a skill folder, and `skillpage.py` already implements it.
...
```text
   ...
   read the summary, not the transcript
        │
        ├─ "we decided the tree replaces board.html"   → QC9 · Where we are
...
Re-transcribing or re-summarizing therefore rewrites one section and leaves the rest alone, which is the same discipline `QC7` settled for this board's own write path: land at a section boundary, never at a byte offset.
...
- [x] 🗂 Define the Meeting page kind
      Three managed spans (`head`, `diagram`, `body`) and everything else authored, which is `QC5`'s split with the seed line drawn where ticking happens.
...
- [ ] 📼 Rule what happens to the transcript
      A · keep it inside the meeting page, folded, so one file holds everything.
      B · keep the page to the summary and link back to the Obsidian note for the transcript and the recording.
      → CC recommends A for a board that must survive off the vault, and notes the cost honestly: it is 1,045 lines, and this board's largest page today is a quarter of that.
