...
## Boundary
- ✅ Covered here
  What any agent that could act on a face is actually handed, and what it should be handed: the count, the file, the apparatus, or nothing.
...
## Diagram
```
  WHO COULD ACT ON A FACE, AND WHAT EACH IS HANDED
  ──────────────────────────────────────────────────────────────────────────
  ① the in-page drawer / terminal            (serve.py prime_context)
     ✓ board title and folder
     ✓ face id and title
     ✓ the face's path, relative to cwd (= the repo root)
...
`prime_context` assembles the board title and folder, the face id and title, the face's path relative to the repo root, the first 280 characters of `## Question`, and one line naming a count.
...
It has no board orientation, no counts, no instruction to read the face, and nothing in `CLAUDE.md` tells it that a `>` line under a sentence is addressed to it.
...
This is the channel doing most of the work, so it is odd that it is the one told least, and it is the reason this face is about any agent rather than about the drawer.
...
On a normal face most unchecked boxes are Items to Finish, so the agent is routinely told there are eleven unresolved comments on a page that has none.
...
(`CHAT_RULES` describes a face that no longer exists)
The rules block lists the face's sections as `## Question / ## Diagram / ## Done when / ## Now / ## Why here / ## Lesson / ## Glossary / ## Discussion / ## Comments / ## Log`.
...
(measured 260726 by firing all four endpoints at a throwaway face and diffing the file)
...
Commenting on the same sentence writes somewhere else entirely, appending a block to `## Comments` and creating that section when the face has none:
...
Marking it solved is a one-character edit in place, `- [ ]` to `- [x]`, and attaching a canvas inserts `## Diagram` before `## Content`, which is where the fixed on-stage order puts it.
...
Because a lane is just a line in the file, an agent that reads the face after the write sees it, with no protocol between them and nothing to keep in sync.
...
A `SessionStart` hook could print the same orientation the drawer gets, or a line naming the faces that carry unresolved attachments.
...
- [x] 🔢 The count says what it counts
      ...
      Fixed 260726: `prime_context` counts the `## Comments` boxes and the rest separately and names each, so a face with one comment and one open item now reports one of each instead of "2 unresolved comments".
...
- [x] 📖 CHAT_RULES describes the face as it is now
      ...
      This does not decide what the agent is HANDED, which is the ruling below; it only stops the rules describing a face that no longer exists.
- [ ] 🧭 The plain workspace session is either oriented or ruled out of scope
      ...
      This closes either way: something orients it, or the face says plainly that nothing does and why.
...
- [x] 🧪 A cold agent is asked what it sees
      Run 260726 on a throwaway face carrying two lanes, one open comment, one open item, and a second sentence with nothing attached as a control.
      ...
      So the current pointer IS sufficient for a fresh session that is asked directly, which is the honest answer to the question this face opened with, and it narrows what is left to the mid-session case below.

## Where we are
Nothing is ruled, and three of the five items are now closed by measurement rather than by argument.
The agent still gets a pointer, a count and an instruction and never the attached text, which is `P6`'s open ruling. What changed is that the count is now correct, the rules describe the face that exists, and "usually reads, usually sees" has been replaced by a result: asked directly, a cold session found every attachment, both before and after the fixes.
...
- 260726 JL · 🧭 It was never only the drawer
  ...
  Correct, and the second channel is the worse-served one: `prime_context` runs for the drawer and the terminal, and a plain Claude Code session started in this repo gets no orientation, no counts and no instruction to read the face.
  ...
  This session is an instance of it. Every attachment JL made today arrived because JL typed it in chat; a `> JL:` lane written on the page would have reached me only if I were told to read that face.
  The face is now about any agent that could act on a sentence, its title and Boundary say so, and the ruling below has two channels to answer for rather than one.

- 260726 CC · 🧪 The measurement, and what the agent actually said
  A throwaway face carried `> Check:` and `> JL:` under one sentence, an open comment quoting that sentence, an open item, and a bare second sentence as a control. One scoped turn, the same question both times: what is attached to this face, and what does any of it want from you.
...
- 260726 JL · ❓ Opened from QA8, after the ➕ canvas landed
...
## Glossary
orientation: the text `prime_context` prepends to a chat or terminal session so it knows which board and face it is attached to.
