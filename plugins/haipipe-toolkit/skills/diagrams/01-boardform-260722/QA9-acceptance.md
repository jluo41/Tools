...
- [x] 🖱 A construct that RENDERS but cannot be USED is reported
      A third failure mode, found 260726, that neither of this page's two instruments can see. The page rendered correctly and read correctly, and every chip on it was dead, because a bare `.fig{}` rule also matched every chip PANEL (`class="chipcard <kind> <state>"`) and its `display:block` beat the UA rule hiding a closed popover. Five invisible full-width panels lay over the page and swallowed every click.
      ...
      The instrument lesson is the more valuable half. The first diagnosis was wrong because it was measured with `element.click()`, which SKIPS HIT-TESTING and so reports success on an element no mouse can reach. `document.elementFromPoint` at the element's own centre found 11 of 11 unreachable and named the covering element in one line.
- [ ] 🎯 An interaction check that a static reader cannot fake
      `check_css` catches this one collision class. It does not catch an element covered by anything else, an off-screen panel, or a control whose click does nothing. The instrument that would is a headless browser asking `elementFromPoint` at each control's centre and then asserting the panel opens INSIDE the viewport; it was written ad hoc for the 260726 bug, on a stdlib-only CDP client, and never kept.
