...
```text
  ...
  Meeting         Meeting-<n>-<slug>     it records what was said   none
  Literature      register marker +      every row supported,       for-literature
  topic           outward route          deferred, or withdrawn
  Value topic     register marker +      every claim bound to a     for-value
                  inward route           run, deferred, withdrawn
...
#### 1.1 · Two types ship without a contract
(the Q page leans on the base alone, and Meeting has none anywhere)
A Q page is the base's home case, so the base doubling as its contract is a choice rather than an accident.
`Meeting-<n>` is generated by `cli/meetingpage.py` and no skill rules it; the base names it only so the kind is discoverable.
Neither gap blocks the list question, and both are recorded here so they stop being invisible.
...
#### 3.2 · Display waits on one prior question
...
The mirror question is small and should be ruled first.
