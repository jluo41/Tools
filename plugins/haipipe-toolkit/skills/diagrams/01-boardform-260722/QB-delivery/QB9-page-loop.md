# One Page through four phases: draft, probe, revise, and check
...
├── 🔀 routes    close · REVISE · PROBE · new DRAFT
...
#### 4.2 · CHECK has four meaningful outcomes
...
DRAFT, PROBE, REVISE, and CHECK remain divisions of QB9 because their meanings depend on comparison and transition.
...
It must need its own Aims and States rather than borrowing QB9's records.
It must have an independent closing gate and its own small continuation map in Files.
If any test fails, the material remains a division or paragraph on QB9.
...
If a phase later passes the split test, QB9 keeps the shared boundaries and transitions while pointing to the new Page for that phase's independent question.
...
- 260804 JL · 📄 A phase gets a Content division before it gets a Page
      QB9 remains the shared home for the four phase definitions, their boundaries, and their transitions.
...
260804 · JL agreed that DRAFT, PROBE, REVISE, and CHECK remain Content divisions of QB9 by default. Added division 7, Aim A7.1, its completed State, and the dated Law that requires four tests before a phase becomes its own Page.
