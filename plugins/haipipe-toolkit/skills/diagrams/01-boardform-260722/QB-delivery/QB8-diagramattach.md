...
## Opening
How can a reader attach, replace, or remove an Excalidraw from Diagram without editing Markdown by hand?

This page defines the write control that turns an in-page drawing action into ordinary `## Diagram` source.
The hard part is preserving the ASCII figure, handling a missing section, and refusing unsafe URLs without inventing hidden metadata.
If the route leaves the page for a text editor, collaborative drawings are too costly to add during the discussion that needs them.
The design succeeds when every page can round-trip a drawing through its source without losing prose or creating special syntax.
