...
state: 🟡 PARTIAL · the order and identity are settled; the section-face carve awaits JL
owner: CC
...
intent first, status second.
  ...
  🎯 Items to Finish    ☑ ☑ ☑ ☐   7/9      — testable gap             ← intent
       context names, e.g. "Choose the skill family" · "Prove routing"
  📍 Where we are       the honest present                            ← status
       context names, e.g. "Roster awaiting JL" · "Current family"
  📎 Files              the action map
...
### 5 · Sections own optional subsections and copyable paths
Every visible page section keeps its fixed place in the shared frame, but its owning face may define optional `###` subsections beneath it.
The frame therefore stays stable while Items to Finish, Where we are, and Files can group records around the page's actual subject.
Names such as Decisions, Build, Current decision, Previous decisions, Sources, and Outputs are examples of roles, not reserved subsection names.
...
- 260730 JL · 🧭 The fixed sections gained optional internal structure
  ...
  The subsection roles shown in the first sketch are examples rather than a fixed vocabulary: every page uses contextual headings, Files gains the same internal structure, and every section/subsection heading receives a generated copyable path that can focus the existing page Chat.
