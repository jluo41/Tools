...
## Files
- `src/common.py`
  `ALIAS` (moved out of build.py in QB5's src/ split) decides whether old section names are still recognized; whether an old board regenerates untouched depends entirely on it.
...
## Log
260725 1145 · QB alignment pass: the ALIAS pointer repointed from build.py to src/common.py (where it lives since QB5)
