...
/_excalidraw/?board=Tools/plugins/haipipe-toolkit/skills/diagrams/01-boardform-260722/board.excalidraw&frame=QD4
...
- [ ] 📦 Swap only the page that changed, not all 53
      ...
      Today any write to any page rebuilds `board.html`, whose Last-Modified changes, so every open tab replaces the whole of `div.wrap` even when the page it changed is one the reader is not looking at.
