...
state: 🟢 built 260801-02 and driven end to end in a real browser; 10 of 13 Aims ✅; A2.2 is the real gap, A2.4 and A3.2 are halves
owner: CC
...
```
  TODAY                                     PROPOSED
  one document that swaps its own middle    three frames that ignore each other
  ─────────────────────────────────────     ─────────────────────────────────────
  ┌─────────────────────────────────────┐   ┌─ shell.html ───────────────────────┐
  │ sidebar    94,942 B    75% of page  │   │ ┌───────┬────────────┬───────────┐ │
  │            53 pages inlined, again  │   │ │ index │ page       │ chat      │ │
  │ ┌─────────────────────────────────┐ │   │ │ frame │ frame      │ frame     │ │
  │ │ div.wrap                        │ │   │ │       │            │           │ │
  │ │  router swaps THIS on a click   │ │   │ │ loads │ re-fetches │ nothing   │ │
  │ │  refresh swaps THIS on an edit  │ │   │ │ once  │ ITSELF     │ else can  │ │
  │ └─────────────────────────────────┘ │   │ │       │ on an edit │ reload it │ │
  │ chat and terminal sit on <body>,    │   │ └───────┴────────────┴───────────┘ │
  │ outside the swap, so they survive   │   └────────────────────────────────────┘
  └─────────────────────────────────────┘
  held together by guard code: deferred     a frame reload is scoped to one frame,
  swaps, asset stamps, PTY parking, and     so there is nothing left to guard
