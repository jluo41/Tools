...
- A2.2 · The index is loaded once per session, not once per page.
  **Done when:** the page list is its own frame and no page's html carries `sb-out` blocks for the other 52 pages.
...
- ❄️ A2.2 · HELD, and handed to `QD8` A2.4, which owns the page list's 67% for both doors. Half of this Aim is met and the half that is not was never the split's to finish: the index IS loaded once per session because it is its own frame, but every page still SHIPS the 53 `sb-out` blocks and the pane only stops drawing them. Deleting them is a `build.py` change that takes the page list off a page opened on its own, so it is a decision about what a standalone page can do, not about panes, and it blocks nothing here.
...
- ✅ A5.2 · Both side frames ship as `data-src`; opening a page is 157 KB and 4 requests, the page list costs 34 KB when first shown, the chat 228 KB, and toggling either afterwards is 1 to 3 ms and no request at all.
