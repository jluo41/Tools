...
state: ✅ SETTLED · built 260801-02 and driven end to end in a real browser; 14 of 16 Aims ✅, and the two that are not are explicitly held with their reasons
owner: CC
...
The page pane is this page's html, the index pane is the board's sidebar, and the chat pane is whatever you talk to Claude through, today a terminal running the CLI.
...
```
  ...
  ② ☰ Index            the sidebar, to find the   34 KB ·  30 ms
  ...
  ⑤ repeat ③④         the sidebar keeps its scroll, the chat keeps its
...
```
  ...
  the sidebar bytes  67% of every page, measured     ❄️ A2.2 held
                     on QD8; three copies when all       handed to QD8
...
This is orthogonal to `cause 2`: compression makes the repeated sidebar cheap to send, and `A2.2` is what stops sending it at all.
...
So the shell carries a 30-pixel strip of its own: 🏠 back to `/boards`, ☰ and 💬 to put the sidebar and the chat away, the board folder's name, the current page's title, and `↗ plain` to leave the split for that page on its own.
...
- ❄️ A2.2 · HELD, and handed to `QD8` A2.4, which owns the sidebar's 67% for both doors. Half of this Aim is met and the half that is not was never the split's to finish: the index IS loaded once per session because it is its own frame, but every page still SHIPS the 53 `sb-out` blocks and the pane only stops drawing them. Deleting them is a `build.py` change that takes the sidebar off a page opened on its own, so it is a decision about what a standalone page can do, not about panes, and it blocks nothing here.
...
- ✅ A5.2 · Both side frames ship as `data-src`; opening a page is 157 KB and 4 requests, the sidebar costs 34 KB when first shown, the chat 228 KB, and toggling either afterwards is 1 to 3 ms and no request at all.
