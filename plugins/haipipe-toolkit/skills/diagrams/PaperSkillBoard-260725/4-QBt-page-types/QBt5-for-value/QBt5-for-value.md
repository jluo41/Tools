...
Nine pages out of 146 declare a type key, six percent of the estate. All nine are on this board and eight of the nine are the `QBt` specimens themselves, so the type system is today a design with one worked example set rather than a property of the estate: `PaperSkillBoard-260725` and `TaskSkillBoard-260726` carry no typed page at all, and `QB6 §5.1`'s checker rules would report nothing on either. One limit travels with it: this counts pages that DECLARE a key, not pages that declare the right one.
