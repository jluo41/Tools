...
### Unit shape and migration
The target bundle is `README.md`, `intake/`, `recipe/`, `float.tex`, `preview.tex`, `preview.pdf`, `assets/`, `candidates/`, and `versions/`.
`float.tex` references only a promoted asset; `preview.pdf` is the inspection wrapper, not a substitute for the live artifact.
`versions/` keeps saved outputs and editable sources; `candidates/` keeps unpromoted alternatives. Neither directory selects what the manuscript prints: only `float.tex` does. A later `versions/manifest.yaml` may record author, date, rationale, and supersession, but a filename alone never establishes any of those facts.
An existing `source/` directory is a declared legacy state, not an alias for `recipe/` or `intake/`.
Migrate one unit only after its values provenance can be recorded, its rebuild material can be separated into `recipe/`, and the current live asset can remain unchanged through the migration.
