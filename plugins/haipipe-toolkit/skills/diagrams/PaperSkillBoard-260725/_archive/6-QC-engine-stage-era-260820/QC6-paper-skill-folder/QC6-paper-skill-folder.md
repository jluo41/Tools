...
The naming instead copies the BOARD's own folder rule exactly, `1-QA-design` and `S01-opening`, so a group folder is spelled the same in the skills tree and in every paper.
...
The board spelled its own group folder `QA-design` when this was written and added the leading number on 260816, for the same reason the S folders carry one: a listing that sorts by letter contradicts the order the board declares.
...
- [x] 🗣 The thin paper: does `haipipe-paper-stage` fold into the door, and does `phase/` dissolve into flat workers?
      ...
      ✅ `Phase 2 EXECUTED 260805` under a SHARPER ruling that supersedes the "flat workers/" half of A. JL, same day: "we just remove all the workers/... or the worker will go to the haipipe-board." Workers went to ZERO skills, and NOT into the board (which stays family-blind): page rules were already the board's (for-literature/for-value/for-display own the bindings), the LaTeX craft became stage DATA files (citation-craft.md in S03, values-craft.md in S04, draft-craft.md in S05, the revise/check trio in S06-main/section-edit, proof-checker pack in S09-build), and the checkers became scripts a stage.md declares. Two new stage.md contract fields carry this: `checker:` and `craft:`, honored by the phases per for-stage 0.5.0. The door fold landed the same pass: enter + lifecycle + stage absorbed into haipipe-paper 0.5.0 (682 lines, one door), stages/ + create-page.py + probe tooling moved under it, workers/ and the three routers retired to _old/. Verified 4/4: install --global (20 stale links pruned, plus a fix: the global branch never pruned stale links at all), check-contracts (8 contracts, hard-FAIL-on-zero kept), both design boards rebuild clean, final reference grep clean. Registered paper skills 21 → 10.
...
### A4 · ⚠️ The coupling that blocks the move
- ✅ A4.1 · Ruled C on 260804: the two families share a CONTRACT, not folder names. Four host-agnostic phase rulebooks now ship under `board/page-phases/` as `haipipe-page-draft`, `-probe`, `-revise`, and `-check`, registered recursively by `install.sh --global` and resolving in the roster. Paper and application keep their family worker folders and load the shared Page Phase before applying artifact-specific rules.

## Files

📋 **Contracts** · what carries this page's rule to somewhere else

- `../QC1-delivery-skill-map/QC1-delivery-skill-map.md` · becomes a listing rather than an essay if this lands
- `../QC2-stage-contract/QC2-stage-contract.md` · owns what a stage declares, and gains the resolve-to-a-live-page rule
...
- 🔑 **A folder named `SNN-<group>` is a delivery group; anything else is an axis.** It is the board's own rule, the one that spells `1-QA-design`, applied unchanged, so the skills tree and every paper's `0-lifecycle/` name a group the same way (JL 260803; the board added its own leading number on 260816, which the padded `SNN-` already carried). The rule needs no punctuation to carry it, and a leading underscore must not be used, because `install.sh:143` already prunes `_archive` and `_paper-writing-backup` by name and the mark therefore reads as "not shipped".
...
## Log
- 260806 0800 · [REVISE-CC] PHASE 3 EXECUTED and verified 4/4: the paper family registers exactly ONE skill (door 0.6.0, line 16 states it); folder/conform and the five build tools live on as fn/ verb procedures + scripts/ under the door; round and rebuttal became S10 stage DATA (stage.md with board_family: Round, template proven by a scratch create-page run, rebuttal-craft on the craft: list, index.yml row 6); nine folders retired to _old/phase3-260806/, install prunes 9 stale links, check-contracts now reads 9 stage contracts, three boards rebuild clean, zero live references. QCskill roster on this board is now ONE card, Skill-0, resynced to 0.6.0; Skill-14 archived beside the other retirees.
- 260806 0640 · [REVISE-CC] PHASE 3 RULED AND DISPATCHED, JL: "我们是不是可以先只保留一个 skill,就是 haipipe-paper... 你现在就开始做吧,不要等待了." The paper family collapses to ONE registered skill: folder and conform fold into the door's fn/, the five build tools become door verbs backed by fn/ files and scripts, round and rebuttal become S10 stage DATA (stage.md + template + rebuttal-craft, new index.yml row), nine skills retire to _old/phase3-260806/. for-round as a new Page Type was considered and deferred: S-Round pages ride for-stage plus S10's stage.md until a live rebuttal round proves a typed record no existing type carries. Execution agent running; commit lands when it and the S03/S04 template agent both verify.
- 260806 0450 · [REVISE-CC] the seven ⚫ RETIRED mirror cards archived to _archive/QCskill-retired-260806/ on JL's "we just remove them" (roster now shows only live units: Skill-0 door, Skill-6/8/9/10/11 board family, Skill-14 round); their Q-Skill link aliases repointed at the archive so old cross-board links still land. JL's companion idea, a new haipipe-paper-page wrapper skill, assessed NO: it would re-create the packet compiler folded into the door 24 hours earlier, and the paper-page specialization already has one owner, haipipe-page-for-stage; the visible-bridge want is served instead by a future Skill-1 MIRROR of for-stage, gated on QB6's nesting row.

- 260805 2330 · [REVISE-CC] Phase 2 executed by a restructure agent and verified 4/4 (install prune, check-contracts, both board rebuilds, reference grep). Door fold: enter + lifecycle + stage absorbed into haipipe-paper 0.5.0, one skill at 682 lines, with stages/, create-page.py, probe/ tooling and the merged ref/ under it. Workers DISSOLVED to zero skills per JL's sharper same-day ruling ("we just remove all the workers/... or the worker will go to the haipipe-board"): craft became stage data files, checkers became stage.md `checker:` declarations, page rules were already the board's; the board itself took no LaTeX (family-blind is what lets application rent it). New for-stage 0.5.0 fields `checker:`/`craft:`; base and three phase contracts reworded "family worker" to declared family craft. Registered paper skills 21 to 10. Fresh-context door validation and the Log-format pass dispatched the same night; QC6's application row now unblocks (sequenced behind exactly this fold).
260804 · A4.1 RULED C, and the ruling came from a measurement rather than an argument. The blocker had been stated as a folder-name coupling: `application/` was given paper's buckets in a planned port, 42 files name those paths, and a SOP exists to keep the two in step. What nobody had counted was what the two families actually duplicate. They each ship their own DRAFT, PROBE, REVISE and CHECK hub, 1,263 lines against 531, and NOT ONE of the eight loads `haipipe-page`; every one of them had copied the page grammar from memory, which is why `haipipe-paper-draft` still named `## Items to Finish` five times on the morning that section was renamed. So the thing worth sharing was never the bucket names, it was the four-phase loop, and B would have thrown away the valuable half to keep the worthless one. Built the same day: four host-agnostic phase contracts in the Board family, `-for-stage-draft`, `-for-probe-entry`, `-for-stage-revise`, `-for-stage-check`. PROBE is deliberately not named `-for-stage-probe`, because it is the one phase that writes a page it was not invoked on (the topic register plus a nested entry, both in another group), so it is contracted as a PAGE KIND instead. `install.sh --global` re-registered cleanly and all four resolve in the roster.
