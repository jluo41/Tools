...
Scope: This page covers What a re-run does to an existing page, what is protected, and who decides. Neighbouring pages cover Who created the page is `QC3b`; what the phases write is `QC4a` to `QC4d`; who may pass the gate a second time is `QC4d`; ownership of regions on a shared page is `QA8`.
...
```
   ...
   ② ref/09-stage-illuminate.md:25
        ...
        📍 IT MOVED, AND IT WAS PROMOTED. This began as the
           ...
           and the rule now sits in a ref doc that `../../paper/haipipe-paper-stage/ref/08-stage-gate.md`
   ...
   ⑥ ../../paper/S06-main/section-edit/stage.md:272
        "a second backward-fill would overwrite authored prose with
         a build product"

   ⑦ ../../paper/S06-main/section-edit/stage.md:301            ◀ FOUND 260727
...
- [x] 🔍 Re-measure the scattered rules against disk
      Done 260727. Eight rules, not six: `../../paper/S06-main/section-edit/stage.md:267` and `2-phase/REF/prose-quality.md:34` were missing from the inventory, and both are the human-comment protection this face was asking for. The 2026-06-22 feedback note no longer exists anywhere in the tree; its rule was promoted into `ref/09-stage-illuminate.md:25`. Four of the six cited line numbers had drifted.
- [x] 🧠 Rule the second run, and where it is DECLARED
      Ruled 260727: **Ⓒ protected regions, with Ⓑ as the behaviour for everything outside them**, declared as `on_rerun: diff-and-ask` in all eight `stage.md` contracts and specified once in `../../paper/haipipe-paper-stage/stages/CONTRACT.md` under "The second run". Both halves of the two-decision shape are answered: the behaviour is Ⓑ, which had been the written rule in `ref/09-stage-illuminate.md:25` and never built, and the declaration is a stage field rather than a page marker, because the page-marker version already exists in 96 files and reaches 0 pages.
...
- [ ] 🔧 Retire or generalise the local rules the ruling replaces
      Seven files hold the eight: `create-page.py`, `../../paper/haipipe-paper-stage/ref/09-stage-illuminate.md`, `stages/5-section-edit/template.md` (plus 95 venue copies), `../../paper/haipipe-paper-stage/ref/08-stage-gate.md`, `haipipe-paper-rebuttal/SKILL.md`, `../../paper/S06-main/section-edit/stage.md` (two of them), `../../paper/phase/REF/prose-quality.md`. A local rule that survives the ruling is a local rule that will contradict it later.
...
## States
Ruled and declared 260727. `on_rerun: diff-and-ask` is in all eight `stage.md` contracts, the rule is specified once in `../../paper/haipipe-paper-stage/stages/CONTRACT.md` under "The second run", and the two preservation rules now match the lane shape `> <ACTOR>:` instead of the one id that was never on a page.
...
- `../../paper/haipipe-paper-stage/ref/09-stage-illuminate.md`
...
> CC 260727: one judgment is open that the fork does not contain, and it blocks every option in the fork. The token fix has two shapes and they cost differently. Teaching both rules to match `> USER:` or `> JL:` is two string edits and leaves two names for one thing, which is how `output:` and `generated:` ended up naming one law on `QC3d`. Rewriting the 45 `> JL:` lines on the MISQ pages to `> USER:` makes one name true everywhere, and it does so by editing human comment lines, which is the single edit `board SKILL.md:475` exists to forbid. I recommend the first, and would spend the tidiness on the checker instead: one assertion that reports either token. The cost of the first is that the vocabulary stays double, so every future check has to remember both.
