...
And whether `sections:` in the contract and the divisions in the template must agree, and which one wins if they do not. Today both exist, `../../paper/haipipe-paper-stage/check-contracts.py` requires `sections:` to be present, the parser reads only the template, and nothing compares them.
