...
The run-time rule is already good: announce the boundary, log the verdict, mark the phase line. Its weakness is reach rather than homelessness. The router that walks the list cites `../../paper/haipipe-paper-stage/ref/08-stage-gate.md` by name at its Step 3, naming the Phase Transition Contract specifically, and the CHECK worker cites it twice. DRAFT, PROBE and REVISE cite it nowhere, and each writes its own `[PHASE]` entry regardless, which means the rule is being obeyed by convergence rather than by contract.
...
- [x] 📐 A shorter declared list is legal, and venue says why
      Ruled in three places, so the 🧠 this face carried was already answered on disk: `stages/CONTRACT.md:29`, the router's Step 4 ("`phases:` is a LIST, not a type ... never pad a list to four"), and `../../paper/S01-opening/venue/stage.md:13-21`, nine lines of comment saying venue produces a contract rather than prose.
...
- [ ] 🔧 Cite the Phase Transition Contract in three worker skills
      `haipipe-paper-draft`, `-probe` and `-revise` SKILL.md contain zero references to `../../paper/haipipe-paper-stage/ref/08-stage-gate.md`, though each writes its own `[PHASE]` entry (draft:246, probe:121, revise:25). The router (`haipipe-paper-stage/SKILL.md:69`) and CHECK (`haipipe-paper-check/SKILL.md:87,292`) already cite it, so this is three missing pointers, not a homeless rule.
...
- [ ] 📐 State what a skip means for a per-unit stage
      The router's Step 4 says the whole list runs once per unit; nothing extends that to a skip. `../../paper/S06-main/section-edit/stage.md:29` is the only `runs: per-unit` contract, so it meets this first.
...
- `../../paper/S01-opening/venue/stage.md`
