...
```
 ...
 WHERE THIS PAGE'S EVIDENCE LIVES IN THE BOARD FOLDER
   the prose      QBe1a-sentence-citation.md  ## Content   (S1 S2 S3)
...
The paragraph's other three sentences are the sibling pages': S4 the numbers is `QBe1b`, S5 the table is `QBe1c`, S6 the figure is `QBe1d`.
...
## Log
- 260803 · Left `QB · Delivery` for the new `QBe · Delivery Element` group, and `QB12a` became `QBe1a`, then took its place in the unit-size order ruled the same day (JL 260803: sentence, display, section); the old id resolves as a declared alias in `board.md ## Links`.
