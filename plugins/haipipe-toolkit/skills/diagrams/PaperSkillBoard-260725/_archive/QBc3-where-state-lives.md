...
## Diagram
```
 A HAND-WRITTEN POINTER DISAGREED WITH THE RECORD IT POINTED AT

  BEFORE  STATUS.md decides
  ┌───────────────────────────────────────────────┐
  │ current_layer: 3-narrative     hand-written   │
  │ gate ledger:   …4-display passed…  hand-written│
  │        ▲                    ▲                 │
  │        └──── DISAGREED ─────┘            ⚠️    │
  │ a whole board face existed for MONTHS to rule │
  │ which of two fields in ONE file was true      │
  └───────────────────────────────────────────────┘

  AFTER  the frontier is READ, never stored
  ┌───────────────────────────────────────────────┐
  │ every page carries its own  state:            │
  │                                               │
  │  S-Seed-0    ✅   S-Work-1  ✅   S-Venue-2 ✅   │
  │  S-Display-0 ✅   S-Main-2  🟡  ◄ THE FRONTIER │
  │  S-Main-4    🔴   S-Main-6  🔴                 │
  │                                               │
  │  frontier = the EARLIEST page whose gate      │
  │             has not passed. Derived, so it    │
  │             cannot disagree with itself.      │
  │  STATUS.md now MIRRORS the board.             │
  └───────────────────────────────────────────────┘

 WHAT IS UNSETTLED  ⚠️
   the ruling was made on ONE paper. The skill still tells a stage
   worker that STATUS.md is the machine state it must update, so a
   worker following its contract writes a field that decides nothing.
   Either the skill adopts the ruling, or the paper is out of step
   with its own skill.
```
