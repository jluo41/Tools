...
**The board door beside the paper door** (JL 260806, the companion to Skill-0's Diagram): first this door's own flow, then the two doors compared.

```
[1/2] 🔌 THE WIRING · every arrow is a real call or read

 👤 JL ── "/haipipe-paper claims" ──▶ 📄 haipipe-paper (DOOR)
  │                                     │ reads stages/index.yml → stage key
  │ "/haipipe-board work on <page>"     │ reads ONE stage.md ────────────┐
  │ (any board, any page)               │ page absent? create-page.py    │
  ▼                                     │   └─▶ cli/stage.py writes ─┐   │
 🧩 haipipe-board (DOOR)                │                            ▼   │
  │ scope=sentence ▸ haipipe-board-     │                 📝 S-Work-C-claims.md
  │   sentence (comment · edit · card)  │                    the PAGE on disk
  │ scope=board   ▸ build/serve/check   │                            ▲   │
  │ scope=page ─────────────┐           │                            │   │
  ▼                         ▼           ▼                            │   │
              ⚙️ haipipe-board-page (ENGINE) ◀── hands the page ─────┘   │
                │ TYPE  ◀── filename S-… → for-stage ── reads ──────────┘
                │           (register route: → for-lit/-value ·          (chain,
                │            page-type: → for-display · …ten)             gate,
                │ PHASE ◀── authority test: promise/unknown/realize/judge venue)
                │ loads base → type → phase → the stage's craft: files
                ▼
   ✏️ DRAFT ──▶ 🔍 PROBE ──▶ 🔧 REVISE ──▶ ⚖️ CHECK
               │  q-executor ▲ A-consumer      │ runs stage.md checker:
               ▼             │ written back    │ fresh judge routes
        probes/L|V entry ────┘                 │ CLOSE/REVISE/HOLD
               │ dispatch (stake stripped)     ▼
               ▼                          👤 [CHECK-JL] the gate
   🏦 task/discovery orchestrator agents       │ state ✅ · Log line
               │ run the bank's lifecycle      ▼
               ▼                     🔨 build.py rebuilds board/ site
        QA/<n>-<slug>.md                       │
        pointed at, never copied               ▼
                                     🌐 JL's browser (serve.py live:
                                        a click or comment lands back
                                        on the .md, loop restarts)
```

```
[2/2] ⚖️ THE DIFFERENCE · two doors, two altitudes

              📄 /haipipe-paper                🧩 /haipipe-board
  ──────────────────────────────────────────────────────────────────
  owns        ONE family's intent:            the MEDIUM every family
              stages S01-S10, venue,          writes on: boards, pages,
              probe spend, claims             sentences, the site
  knows       what a PAPER needs              what a PAGE is · never LaTeX,
                                              never a stage's meaning
  routes by   verb + stage key                SCOPE (sentence/page/board)
  serves      papers only                     any board: phyprofile, design
                                              boards, and papers' 0-lifecycle
  relation    CALLS the board family:         is CALLED: builds the site,
              a paper's 0-lifecycle IS        serves it live, hands page
              a board                         work to the one engine
  one line    the customer with the job       the workshop and its machines
```

**Click-through**: [haipipe-board/SKILL.md](../../board/haipipe-board/SKILL.md) · [build.py](../../board/haipipe-board/cli/build.py) · [serve.py](../../board/haipipe-board/cli/serve.py) · [check.py](../../board/haipipe-board/cli/check.py) · [status.py](../../board/haipipe-board/status.py) · the engine [haipipe-board-page/SKILL.md](../../board/haipipe-board-page/SKILL.md) · [haipipe-board-sentence/SKILL.md](../../board/haipipe-board-sentence/SKILL.md) · [haipipe-board-routing/SKILL.md](../../board/haipipe-board-routing/SKILL.md) · the paper door beside this one [haipipe-paper/SKILL.md](../../paper/haipipe-paper/SKILL.md)
...
live/shell.py                      815 ln  QD5 · the operating shell: index · page · chat, three frames that ignore each other.
...
## Log
- 260806 0410 · [REVISE-CC] figure [1/2] replaced on JL's correction ("what I want is a logic diagram, how are things are linked together"): now the full wiring, both doors to the engine, type/phase resolution reading stage.md back, the DPRC spine with the probe wall to the bank, the gate, build.py, and the browser's live write-back closing the loop.
- 260806 0350 · [REVISE-CC] Diagram authored beside Skill-0's (JL: "put it to haipipe-board aside, what is the different between them"): [1/2] this door's own flow (route by scope, the four tools it touches), [2/2] the two doors compared line by line; click-through links verified. The generator's workflow stub fence retired.
- 260806 0150 · [REVISE-CC] card synced to disk truth after 260805 (ten types · thin-paper phase 2 · first live RUN); version lifted from 0.118.0 to 0.120.1 and the variant-location line no longer names the retired haipipe-paper-stage
...
260730 · `0.53.0`
      ...
      - Every one of the 42 page ids survived, so no citation anywhere in the repo broke: QA1 now sits in Delivery, QB1 in Engine, QE1 in Working.
        ...
        Five group folders replace seven (`1-QA-design/ 2-QB-delivery/ 6-QC-engine/ QD-working-and-sharing/ QE-execute/`), moved with `git mv`, and every `oldfolder/page.md` citation inside the board was rewritten.
