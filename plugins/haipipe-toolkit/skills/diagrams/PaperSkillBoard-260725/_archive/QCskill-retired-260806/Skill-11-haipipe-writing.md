...
It would be finished when a caller can run score, rewrite, apply and check end to end without a person placing anything by hand; the paper humanizer already draws its diffs from `writing/haipipe-writing/cli/wdiff.py` instead of writing them itself (verified 260802).
...
WORKFLOW  ── who calls it, and where the judgment sits
  ...
  hands off to:
    ...
    haipipe-paper-revise-humanizer CALLS wdiff.py --host paper. VERIFIED 260802.
                                   It keeps no diff of its own, and since 0.5.0
                                   it converts no marks by hand.
                                   (haipipe-paper-revise, the other caller
                                   verified 260802, retired 260805 into the one
                                   door paper/haipipe-paper.)
```
...
## Log
- 260806 0150 · [REVISE-CC] card synced to disk truth after 260805 (ten types · thin-paper phase 2 · first live RUN); the workflow figure's haipipe-paper-revise row now records its 260805 retirement into paper/haipipe-paper and the humanizer finish-condition is recorded as met
