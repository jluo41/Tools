...
state: ✅ SETTLED
owner: JL
method: keep only the stage's real product in Content and route every other concern by role

## Question
What is the `## Content` of an S page?
...
## Boundary
- ✅ Covered here
  The boundary among Content, Stage Contract, Items to Finish, and Where we are.
- ↪ Covered elsewhere
  How Content gets its structure is `QBa2`; how one sentence is represented is `QBa3`.
...
For Section, Content is the section itself.
For Display, Content is the visual argument, candidate judgment, caption job, and stable unit meaning.
Rendered assets and source files are linked artifacts, not a replacement for Content.
...
## Where we are
The Board grammar already applies this distinction.
The paper stage contracts do not yet state it consistently.

## Files
- `haipipe-board/ref/q-template.md`
  The current S-page section grammar.
- `stages/*/stage.md`
  The stage-specific product declarations.
