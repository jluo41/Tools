...
```text
 ...
 1-QA-design/*.md    one page      ━▶ board/QA.html      one per group
 2-QB-delivery/*.md  per file      ━▶ board/QA/<page>.html
...
- `6-QC-engine/QC1-delivery-skill-map/QC1-delivery-skill-map.md`
...
- 260816 · Then every page took a folder of its own, `2-QB-delivery/QB1-opening/QB1-opening.md`, the 260815 shape the board-design board had been on since that ruling (73 pages, plus the ten pieces of plugin material the QBt group had been holding for its pages: five drawings, two display units, two QA-probe drawers, one venue pack). The engine had read this shape all along: `_page_home()` is `<name>/<name>.md` and `_in_plugin()` keeps discovery out of every other subfolder, so what was missing was the migration, not the capability. It is now `cli/refold.py`, which moves the material in with its page and re-anchors every path that had been written from the old location, and it is the command that answers "why can we not do it" with "we can". Verified on a copy first: same 73 pages, no new error, and the five `stray-scene` warnings gone, because a page's drawing is now inside the page.
- 260816 · The eight group folders took their place in `## Pages` as a leading number, `1-QA-design/` through `8-QF-execute/`, so the folder listing and the board now read in one order (JL 260816). Every path that cited a group folder was rewritten in the same pass. The board's own folder name changed to `PaperSkillBoard-260725` and the four figures still calling it `01-haipipe-paper-260725` were corrected. Link repair alongside it took the board from 121 dead references to none: the paper family's phase-era skills moved under `paper/_old/`, the board-design board numbered and folded its own pages, and the probe and display design boards retired on 260804, so their ids now point at the shipped contract that inherited each ruling.
