...
(unit: a topic · `../../paper/haipipe-paper-stage/stages/index.yml` declares eight stages and none of them is literature)
Opening and Work can run one page per stage because `../../paper/haipipe-paper-stage/stages/index.yml` gives them seed, venue, pitch, resource, claims and narrative.
...
`../../paper/haipipe-paper-stage/stages/index.yml` declares eight stages, and none of them is `value` or `present`, so neither group owns a stage.
...
```text
  ...
  ❓ why no S page at all, now or ever
     `../../paper/haipipe-paper-stage/stages/index.yml` declares 8 stages and none is `present`
...
(unit: none, and `../../paper/haipipe-paper-stage/stages/index.yml` names 8 stages with no `present` among them)
`../../paper/haipipe-paper-stage/stages/index.yml` declares seed, resource, claims, venue, pitch, narrative, display and section-edit, and that is the whole list.
...
```text
  ...
  🧩 5 of the 6 sit in  board/haipipe-board/
     the 6th sits in      paper/haipipe-paper-stage/
...
Each of the eight stage folders declares `board_family`, `board_unit` and `board_slug` in its own `stages/<dir>/stage.md` frontmatter, and `../../paper/haipipe-paper-stage/stages/index.yml` carries none of the three, only `key`, `order`, `dir`, `migrated` and `triggers`.
...
On 260803 the sibling page `../QB3-literature/QB3-literature.md` said the list was closed in three places and named three, and the two it left out were `src/page_board.py:497` and `live/chat.py:201`, which are exactly the two that fail with no message at all.
`../QB4-value/QB4-value.md` carried the same undercount in three places on the same day, so the miss this division warns about has now happened twice on two different pages.
...
(five lists in `board/haipipe-board/`, one in `paper/haipipe-paper-stage/`)
...
(counted off `board.md`'s `## Pages` against `../../paper/haipipe-paper-stage/stages/index.yml` on 260803)
`../../paper/haipipe-paper-stage/stages/index.yml` declares eight stages, and the six that run once each name exactly one live page, from `S-Seed-0-seed` through `S-Venue-2-narrative`.
...
- A13.3 · The one file that reports an unknown family runs again.
  **Done when:** `../../paper/haipipe-paper-stage/check-contracts.py` imports the composer from the path it lives at today and finishes a real check instead of a traceback.
...
- `../QB1-opening/QB1-opening.md` · the first concern, and the pattern the other nine follow

📥 **Input files** · what the work reads

- `../../paper/haipipe-paper-stage/stages/index.yml` · declares which stages exist, and therefore which concerns own one
...
- `../../paper/haipipe-paper-stage/check-contracts.py` · the only file that reports an unknown family
- `6-QC-engine/QC3b-page-name/QC3b-page-name.md` · owns the per-unit test and the naming Law
