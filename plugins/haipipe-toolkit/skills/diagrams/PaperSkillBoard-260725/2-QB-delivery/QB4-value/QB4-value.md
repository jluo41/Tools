...
- `../QB9-build/QB9-build.md` · where the G3 extension that would enforce this has to land
...
260803 · The family-list count on this page was wrong in three places: `§2.3`, A2.1's State and the 260802 Log entry all said three files and named three. It is SIX. The omitted ones were `src/parse.py:301`, `src/page_board.py:497` and `live/chat.py:201`, and two of those three fail with no message at all. `§2.3` and the State are corrected; the Log entry below is left as written, because it records what was believed then. This is the same undercount `../QB3-literature/QB3-literature.md` carried until the same day, which is the evidence `QB0 §13.2` cites for declaring the list once.
