...
The linked label carries the board's OWN name, taken from `board.md`'s first heading, so `BoardSkillBoard-260722` reads as `haipipe-board` (JL 260802: of `boardform · QB/QB4`, "I think it should be haipipe-board · QB4"). A board whose title is a sentence rather than a name falls back to the folder name with its `NN-` ordinal and `-YYMMDD` date stripped.
