...
### 3 · What exists today
**Today's fragments**: where each existing rule lives and what none of them decide.
```text
...
- [ ] 🧱 Approve or reject the two remaining proposed pages on the live read/write architecture
      ...
      The approved write-path question lives on after the 260801 renumbering as `QC-engine/QC3-roundtrip/QC3-roundtrip.md`, with the write path itself split out to `QC4a-writepath/QC3a-writepath.md`.
      `QC9`, the unit of change, was absorbed by `QC4-roundtrip/QC3-roundtrip.md`, whose method rules the unit of change; the 1.6MB refetch was measured and closed on `QO-operating/QO3-pagecost/QO3-pagecost.md`.
      `QD9`, what the page shows before the server answers, was absorbed by `QO-operating/QO6-whereitruns/QO6-whereitruns.md`, now settled; poll against SSE is `QC4-roundtrip/QC3-roundtrip.md`'s own remaining open item; concurrent writers stay with `QE4`.
...
- `QB-board/QB2-board-webpage-design/QB2-board-webpage-design.md`
