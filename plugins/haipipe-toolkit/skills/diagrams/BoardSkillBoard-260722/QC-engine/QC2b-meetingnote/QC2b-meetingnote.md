...
**Covered elsewhere**: How a folder is mirrored into a page at all: `QC3a`, which already does exactly this for a skill folder. How one input becomes one anchored write: `Design-2-haipipe-board-routing`. What a Meeting page itself owes, since 260805: `haipipe-page-for-meeting`, one of the ten Page Types under `board/page-types/`. The Related Folders fold that can open the file today without any of this: `QB2`. What a page's sections must contain once it exists: `QB4`.
...
```text
   ...
   🎙 echo-meeting (Obsidian plugin, jluo41)
      records · transcribes · summarizes
              │
              ▼
   QB-board/Design-1-haipipe-board/meeting/260723-meeting.md        8,573 words · 1,045 transcript lines
...
**Name against glob**: the discovery glob matches a name shape a dated note never has.
```text
page_files(d)          matches a NAME, at any depth
  Q*.md  S*.md  Agent*.md          ✅ QB4c-content.md · Skill-0-….md
  anything else                    ❌ QB-board/Design-1-haipipe-board/meeting/260723-meeting.md
...
**One meeting, two destinations**: the record becomes a page, the decisions route onward.
```text
   Meeting-1-260723.md
   ┌──────────────────────────────────────────────┐
...
**Input, not archive**: a meeting note enters the board the way a chat message does.
```text
...
`Design-2-haipipe-board-routing` is the verb that turns one input into one anchored write, and a meeting note is a batch of exactly that shape.

### §4 What the producing end actually emits

**The producer's shape**: what echo-meeting writes, before this board touches it.
```text
...
**The importer in one read**: the modules and the spans they own.
```text
...
- [x] ⚙️ Generate it
      `meetingpage.py new|sync` ships; `Meeting-1-260723-boardform-demo/Meeting-1-260723-boardform-demo.md` was generated from the real note and renders all six base sections.
...
- 260731 JL · 🗂 `Meeting-<n>` is a page kind, and it fits the base page exactly
  ...
  `Meeting-1-260723-boardform-demo/Meeting-1-260723-boardform-demo.md` was generated from the real note and renders all six sections, 18 Content divisions, and 15 sentence drawers, because an Obsidian `[!quote]-` callout becomes exactly this board's sentence apparatus: click a chapter's summary line and the words that produced it open underneath.
...
### Input files
- `../../QB-board/Design-1-haipipe-board/meeting/260723-meeting.md`
...
260731 · Opened when JL asked how meeting notes get onto the board; `QB-board/Design-1-haipipe-board/meeting/260723-meeting.md` has been on disk since 260723 and invisible to `page_files()` the whole time. jluo41/echo-meeting + jluo41/echo-capture identified as the producing end
