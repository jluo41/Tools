...
- 260731 JL · 🧩 The browser assets split by topic, then split again
  ...
  Parsing is not running, though, so the split was also driven in a real browser afterwards: chat drawer, chat fab, comment dock, comment fab, page list, page list handle, `boardPath()`, and a discussion write that reached the `.md`, all green.
