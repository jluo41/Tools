...
`QA2` takes the next question, how a board is born and how it ends, and the ladder groups (`QB` down to `QS`) carry the contracts themselves.
...
```text
  ...
  🧭 QA · Constitution  what holds before anything is built: this
        │               chapter, birth and close, the round, what ships
...
Since 260726 every board keeps one folder per page group, named `<N>-Q<letter>-<slug of the group title>`.
...
The leading `<N>` is the group's place in `## Pages`, added 260816.
A letter says which group this is and cannot say which comes first, so `QC-engine/` sat four rows above `QPs-page-structure/` in every listing while this board read them the other way round, and `QPs` and `QPf` were inverted against each other for the same reason.
The number was chosen over renaming the letters because nobody cites a folder: numbering moved 197 path strings, while `QC → QT` and its neighbours would have moved 1594 id mentions across 43 pages, needed a `## Links` row each to keep the old ids resolving, and still could not order `QPs`/`QPf`/`QPw` without spending the s, f and w that say structure, folder and workflow.
`## Pages` stays the only authority: the number is derived from it, so the order changes by moving a `###` block and then renaming the folder, never the reverse.
The engine strips the number before it reads the letter, so `7-QC-engine/` still renders to `board/QC/`, and `check.py` reports `group-number-order` the moment the folder and `## Pages` disagree.
...
```text
  ...
  🌱 QA2  the life      a topic becomes pages, and what ends a board
  🔁 QA3  the round     when an agent may hand the board back
  🎓 QA6  what ships    the roster + what SKILL.md says (6a·6b folded in)
```

The chapter is one page per rung of the loop `§1.2` describes: why a board exists, how one is born and ends, when a round may be handed back, and what graduates out of it all.
The group holds no deliverable of its own, which is why the identity page left for `QC` on 260816: what it rules is engine code, not what must be true before code exists.
...
`QA2`'s second half was the newest hole on the board and was ruled on 260816: the `close` verb had always shipped with no page arguing it, and what happens after the last ✅ now has three rules waiting to graduate.
...
```text
  ...
  🪪 QC4  identity by NAME, and lookups that return EMPTY
```
...
The group's warning story is `QC4`, which arrived from `QA` on 260816: the same defect reported four times in different clothes, because the code decided what a thing IS from what it is CALLED, and a lookup that found nothing said nothing.
...
## Log
- 260816 · [RESTRUCTURE-CC, JL ruled] the QA group became one page per rung of the loop and was renamed `QA · Constitution` (the old name collided with the `Design-<n>` page kind).
  This chapter's `§6.1` follows: the identity page left for `QC4`, because it rules engine code rather than what must be true before code exists, and `§6.7` gained its row; `QA6a` and `QA6b` folded into `QA6`; `QA2` absorbed the close and became the board's life.
  Four pages now, from seven; the group's folder is `1-QA-constitution/`.
