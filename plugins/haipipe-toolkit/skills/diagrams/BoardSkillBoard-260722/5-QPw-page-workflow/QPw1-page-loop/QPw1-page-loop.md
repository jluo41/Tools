...
QPw1-Display1 states the same controller as an algorithm, so a reader can see the three actors alternate and read the stop conditions in the order the loop tests them.
...
QPw1-Display2 writes that table as two rules and derives both laws from them, including one this prose never states: CHECK cannot route to CHECK, so a judged version reaches a second judgment only through a producer.
