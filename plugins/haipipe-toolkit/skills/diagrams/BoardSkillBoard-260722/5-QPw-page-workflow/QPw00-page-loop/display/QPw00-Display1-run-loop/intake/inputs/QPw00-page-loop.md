...
state: 🟡 IN PROGRESS · renumbered QPw00, the group holder; §1-§4 hand off to QPw1-QPw6 · open: 14
owner: JL
...
```
📌 Five of the eleven rows need a person and six run machine-only from end to end, which is the same fact `§9` states as authority, priced instead.
...
### A12 · 💰 The same loop, priced: what a person is actually asked to do
- ✅ A12.1 · Met 260818. `intake/manifest.yaml` carries a sha256 for all EIGHT frozen inputs, and `cli/check.py` reports no `display-intake-unfrozen` against `QPw00-Display3`. It is the FIRST unit on this board with a frozen intake: `QPw00-Display1`, `QPw00-Display2`, `QPf5-Display1` and `QPf5-Display2` are all still unfrozen.
