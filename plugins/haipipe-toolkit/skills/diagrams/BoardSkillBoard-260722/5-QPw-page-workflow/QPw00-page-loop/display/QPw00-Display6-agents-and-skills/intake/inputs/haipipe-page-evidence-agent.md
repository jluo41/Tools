...
**Load, in order:** `haipipe-page` → the Page's matching Page Type → `haipipe-page-evidence` → the lane plugins it needs (`haipipe-plugin-probe`, `haipipe-plugin-bibex`, `haipipe-plugin-display`, `haipipe-plugin-value`).
