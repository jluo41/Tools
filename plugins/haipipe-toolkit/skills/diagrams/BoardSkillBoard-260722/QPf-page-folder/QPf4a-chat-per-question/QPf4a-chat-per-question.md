...
- [ ] Write down what happens when two agents edit the same file
      ...
      A `QC-engine/` session and a `QC3` page session may both edit `QC3-*.md`, and HOLD does not see the clash, because the two scopes hash to different keys.
...
- 260731 JL · 🗂 The levels gained a third one: the page GROUP
  ...
  It opens the same panel attached to the GROUP, whose identity is the group FOLDER (QC-engine/).
...
- Three levels, one rule (JL 260731, built 0.77.0)
      Board level runs the whole board, group level runs one group FOLDER (`QC-engine/`), page level runs one page file.
