...
**The kinds, and who draws them**: one door sends the ask to four drawing skills, plus one way with no skill behind it.
```text
...
The ✒️ tikz row is the one way with no skill behind it.
A person writes the `.tikz.tex` by hand, and that is how both units on this page were drawn.
It earns a row because it is written in TeX itself.
It picks up the paper's fonts and math labels, and `float.tex` can pull it in directly.
