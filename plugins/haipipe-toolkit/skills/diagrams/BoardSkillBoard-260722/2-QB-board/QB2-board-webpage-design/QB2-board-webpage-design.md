...
This page owns the first, and the group page one rung below it. `QPs1` owns the second.

**Where this page sits**: The board walks down one ladder: Board, Group, Page, Section, Sentence.
`QB1` owns the board's folder on disk, and `QPf1` puts a page's file beside the work it describes.
...
**What is settled elsewhere**: Whether a page's prose is well written belongs to `Design-4`, and the mechanical checks run after any change belong to `QF1`.
...
This board has 56 of them, counted on 260816, and most of those have nothing waiting on anyone.
...
- 260816 · [REVISE-CC] `### 14`, which pointed at the pages one rung down, was deleted: the page contract has no Boundary section, so a division whose only job is to say what this page does not own is noise, and its three ids were all retired ones. The slot became the group page, `board/<GROUP>.html`, which `board.md`'s Board Structure has declared since it was written and which no page argued: its whole text comes from that group's entry in `## Pages`, so it has no source of its own and cannot disagree with the Index. `A14` opened in Aims and States alongside it. Opening ids repointed: `QB4` to `QPs1`, `QB3` to `QPf1`, `QA9` to `Design-4` and `QF1`; the page count remeasured at 56 on 260816.
...
### 14 · The group page, the rung between the Index and a page
**One rung down from the Index**: what `board/<GROUP>.html` shows, and where each part of it comes from.

```text
🗂 INDEX  ─▶  📚 GROUP  ─▶  📄 PAGE     three rendered surfaces, one build

  📚 board/<GROUP>.html
  ├── 🏷 purpose        the group's one line in board.md ## Pages
  ├── 🖼 lane diagram   the ascii block under that line, if it has one
  ├── ▓▓░░ progress     settled / total, derived from the pages
  └── 📄 page rows      the same row shape the Index uses

  🔑 authored in ONE place: everything above the filenames comes from
     the group's own entry in board.md, so the group page has no
     source of its own and cannot disagree with the Index
  📥 the entry's grammar is the topic-entry-contract in ① ref/
```
📌 Establishes that a group is a rendered surface with a contract, and that its whole text is authored in `board.md` rather than anywhere else.

#### 14.1 · A group answers "why are these pages together" before a reader opens any of them
(nine groups on this board, each carrying a purpose line and most carrying a lane diagram)
The Index shows every group's rows at once, which is the right view when you know what you are looking for and the wrong one when you do not.
The group page is the view for a reader who has picked a subject and wants its shape before its list.

#### 14.2 · Its text has one source, the group's own entry in `board.md`
(the purpose line, the optional ascii lane block, and the bare filenames underneath it)
Nothing on the group page is authored twice, so a group cannot say one thing on the Index and another on its own page.
That is the same rule the sidebar follows, and it is why neither surface can drift from the source.

#### 14.3 · The lane diagram is the group's own figure, and it is optional
(`QPf` and `QPs` carry one; `QB`, `QC` and `QF` carry a prose line instead)
A group with more than a handful of pages needs a shape a reader can hold, and a small group does not.
The contract does not require one, so a group that gains pages gains a diagram when somebody writes it rather than the day it crosses a count.
