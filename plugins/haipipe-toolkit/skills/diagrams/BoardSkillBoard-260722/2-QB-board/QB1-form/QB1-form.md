# Board · the folder: the two folders, where a board lives, and what sits beside board.md
state: 🟡 PARTIAL · the two-folder map is unapproved · the folder decisions below stay settled
owner: JL
method: name the two folders, say what may cross between them, then say what one board folder holds and how its pages are grouped
...
This page names the two folders, and then says what one board folder holds and how its pages are grouped.

**What the two folders are**: `①` is `skills/board/`, the family that ships: its skills, Page Types, Page Phases, page plugins and agents.
`②` is this folder, `skills/diagrams/BoardSkillBoard-260722/`, where those rules are argued one page at a time.
A decision leaves `②` for `①` when a person copies it in, and that copy is the moment it starts to bind.

**Where this page sits**: `QA00` names the folders this board works with and says which kind of truth each one holds.
This page takes the board FOLDER: where it sits in a tree, what is inside it, and how its pages are grouped into subfolders.
`QB1a` takes the third case, a write into a board this family renders but does not own.
`QB2` takes the webpage a reader opens, `QPf1` puts a page's file beside the work it describes, and `QPs1` takes what one page looks like inside.

**What this page carries**: the unit that runs a board, `haipipe-board`, sits in this page's `skill/` plugin, because its `SKILL.md` 🗂 Shape section and `ref/board-form.md` §1 and §2 are where this page's Law graduates.
The 260723 demo that first showed the folder working sits in the `meeting/` plugin beside it.

**What is still open**: JL has not approved the two folders.
...
Count it with the parser or with `ls`, write the number, and date it: `56 pages in 9 group folders, counted on 260816`.
...
**Say page, not face**: the shipped page contract now uses "face" only for a sub-page whose id carries its parent's number, such as `QB1a`.
...
── [1/3] Two folders, and that is the whole map ──────────────────────────
  ...
  │
  ├── 🤖 haipipe-board/                 the door you invoke · v0.137.0
  │   ├── 📖 SKILL.md                   the operating manual, kept shortest
  │   ├── 📐 ref/           7 specs     board-form · page-template ·
  │   │                                 writing-rules · board-example …
  │   ├── ⚙️  cli/         25 scripts   build · check · serve · watch · regroup …
  │   ├── 🧩 src/          15 modules   the parser, split by page topic   (QC1b)
  │   ├── 📡 live/         21 modules   the running server, split off     (QC1c)
  │   ├── 🎨 assets/                    css · js · board-mark.svg         (QC1)
  │   ├── 🖥️  vendor/xterm               the terminal's front end          (QO8)
  │   └── 🧪 tests/        30 files     what proves a change              (QF1)
  │
  ...
  💡 5 skills · 4 types · 11 plugins · 5 workflows · 3 agents ship today

  ────────────────────────────────────────────────────────────────────────

  ②  📂 skills/diagrams/BoardSkillBoard-260722/ 💬 WHAT IS ARGUED · 📍 here
  │
  ├── 🧭 board.md                spine · Topic · Pipeline · Board Map · Pages · Links
  ├── 🗂️  QA-…/ … QS-…/           56 pages · 9 group folders           (260816)
  ├── 🌐 board/                  generated site · ⛔ NEVER hand-edited
  ├── 🖌️  board.excalidraw        ONE scene, one frame per page, at the ROOT
  ├── 🖼️  fig/                    images only, 33 of them
  ├── 🧾 _runs/                  the page lifecycle's receipts
  └── 📦 _archive/               30 retired entries · moved, never deleted

  🧨 binds NOTHING · delete ② and every script in ① still runs
  🏭 what ① PRODUCES is neither: 6 boards · 228 pages · 165 not ours → QB1a
```
...
**What sits beside board.md**: the board folder's own plugins, and which of them a build may write.

```text
── [3/3] The board folder's plugins  (§4) ────────────────────────────────

  📂 <board>/
  ├── 🧭 board.md            ✍️ AUTHORED · the only source of truth
  ├── 📄 <GROUP>/<page>/     ✍️ AUTHORED · one folder per page   (QPf1)
  │
  ├── 🖌️  board.excalidraw    ✍️ AUTHORED · ONE scene · at the ROOT,
  │                          beside board.md, not one of the figures
  ├── 🖼️  fig/                ✍️ AUTHORED · images only
  ├── 📦 _archive/           ✍️ MOVED · a retired page, never deleted
  ├── 🧾 _runs/              🤖 WRITTEN · the page lifecycle's receipts
  └── 🌐 board/              🤖 GENERATED · ⛔ never hand-edited

  🔑 the `_` prefix is the parser's rule, not a habit: q_files() skips
     any segment starting with `_` or `.`, and skips fig/
  📄 the same shape one rung down: a page owns its folder and every
     subfolder of it is a plugin (QPf1)
```
...
#### 1.3 · `①` is where a decision from `②` starts to bind, and the copy is a person's act
(an operating rule goes into `SKILL.md`, a display or grammar spec into `ref/`)
Nothing about a page reaching `✅` moves it: a settled page whose Law was never copied binds nobody, however finished it looks.
This page's own Law graduates into `SKILL.md`'s 🗂 Shape section and `ref/board-form.md` §1 and §2, both of which ship in the `haipipe-board` unit carried in this page's `skill/` plugin.
What `②` holds besides its pages is `### 4`.
...
### 4 · What sits beside board.md: the board folder's own plugins
**The five things that are not pages**: what each one holds, and who is allowed to write it.

```text
📂 A BOARD FOLDER HOLDS PAGES, AND FIVE THINGS THAT ARE NOT PAGES

  🖌️ board.excalidraw   ONE scene · at the ROOT, beside board.md
  🖼️ fig/               images only
  📦 _archive/          a retired page, moved and never deleted
  🧾 _runs/             the page lifecycle's receipts
  🌐 board/             🤖 GENERATED · ⛔ never hand-edited

  🔑 what keeps them off the board: q_files() skips any path segment
     starting with `_` or `.`, and skips fig/
  📄 one rung down the same shape repeats: a page owns its folder and
     every subfolder of it is a plugin (QPf1)
```
📂 Establishes what a board folder carries besides its pages, and which of those a person may write.

#### 4.1 · The drawing sits at the board root, as a first-class citizen
(`board.excalidraw` beside `board.md` and the `board/` site, on JL's call of 260729)
It is the one thing in this folder whose content does not come from markdown: the ascii in a page's `## Diagram` seeds a frame one way, and anything drawn on the canvas never flows back.
`fig/` holds images only, 33 of them today, which is why the scene is not filed there.

#### 4.2 · A retired page moves into `_archive/`, and never out of the repository
(30 entries today, some at the board root and the rest in the group folder they were retired from)
Each stays reachable through a `## Links` row, so an id that was written into somebody's notes a month ago still resolves.
A folded page keeps its whole folder, plugins included, which is how `QPs00` and the two Design pages were retired.

#### 4.3 · The `_` prefix is a parser rule rather than a naming habit
(`q_files()` is `rglob("Q*.md")`, skipping segments that start with `_` or `.`, and skipping `fig/`)
That single rule is what makes `_archive/` and `_runs/` invisible to the board without anything registering them.
It is also why a new board-level attachment gets an underscore: naming it `runs/` would put every receipt on the board.

#### 4.4 · Everything under `board/` is derived, and the markdown is the only source
(written by `cli/build.py` on every build, an index, one page per group, one page per page)
Hand-editing it produces a change that the next build silently destroys, which is worse than a change that fails.
The board's own state is not kept there either, so the site can be deleted and rebuilt at any time.
...
📥 YOU HAVE SOMETHING NEW · this is where it goes
  ...
  🧩 a python module                ─▶  ① src/, split by page topic     (QC1b)
  📡 a live-server module           ─▶  ① live/, split off the CLI      (QC1c)
  🎨 CSS or JS                      ─▶  ① assets/                        (QC1)
...
`QPf1` made discovery recursive on 260724 for a different reason, so by the time the decision was taken on 260726 the capacity to group by folder had already been shipping for two days without being named.
...
Cross-board links are the ones that break: a board declaring `../PaperSkillBoard-260725/QC0-sentence-unit.md` is naming a page in somebody else's tree, and it has no way to know that tree was reorganized.
...
Those are the write-back paths, and carrying the folder in them is what `QPf1` already ships and smoke-tested at depth 2.
...
🪄 ＋Q · WHERE A NEW PAGE IS BORN
  ...
  ✅ so ONE rule covers the GROUP folder and the QPf1 SUBJECT folder
```
...
Choosing "where does this group already live" over "a folder called `QA`" is the whole point: it is equally correct when the folder is the GROUP and when the folder is the SUBJECT (`QPf1`), which are the two reasons a page sits in a folder and are one rule.
...
```text
🏷 QA-design/QA00-overview/QA00-overview.md · the QA is written twice, on purpose

  ✂️ strip it to QA-design/00-overview.md and three things break at once
     🔎 rglob("Q*.md")  ─▶ 💥 the file stops being a page at all
     📋 ## Pages        ─▶ 💥 bare names collide the day two groups have a 1-
     🔍 grep QA00       ─▶ 💥 the id stops being findable across the repo
...
`QA-design/QA00-overview/QA00-overview.md` repeats `QA`, and that repetition is doing work.
`rglob("Q*.md")` would no longer match a file called `00-overview.md`.
...
- [x] 🪄 `＋Q` creates the file inside the group it was pressed under
      ...
      It deliberately does not look for "a folder named QA": following where the group already lives is the same rule for a group folder and for a `QPf1` subject folder, so `＋Q` lands correctly on a paper's `0-lifecycle/` without knowing that board is different.
...
- [x] 📂 Name what a board folder holds besides its pages, and who writes each one
      `board.excalidraw` at the root, `fig/` for images, `_archive/` for retired pages, `_runs/` for lifecycle receipts, and the generated `board/` site.
      Written as `### 4`, with the `_` prefix named as the parser rule that keeps them off the board rather than as a naming habit.
- [ ] 📄 State the board-altitude plugin rule the way `QPf1` states the page one
      `QPf1` rules that a page owns its folder and every subfolder of it is a plugin.
      The same shape holds one rung up and has never been written as a rule, so a new board-level attachment is still placed by precedent.
...
- `cli/check.py`
  `pages-ghost` and `dead-link`, the two findings that report a folder or a declared path that no longer resolves.
...
- 📂 **The board's own plugins**: a board folder holds its pages, and five things that are not pages
  `board.excalidraw` sits at the root beside `board.md`, `fig/` holds images only, `_archive/` holds retired pages, `_runs/` holds lifecycle receipts, and `board/` is generated and never hand-edited.
  What keeps them off the board is the parser rule and not a convention: `q_files()` skips any path segment starting with `_` or `.`, and skips `fig/`.
...
- 🌍 **One origin: the SPACE root** (JL 260816). A path a page writes is relative to the SPACE root, `examples/Project-Personality-OpioidRx/tasks/...`, and never counted in `../` from wherever the page happens to sit.
  The server is already rooted there (`serve.py --root <repo root>`), so the SPACE-relative path and the URL a reader opens are THE SAME STRING; `check.py` already anchors on the nearest `pyproject.toml`, which is that same root, so the form resolves from any page at any depth.
  The one exception is a target inside the page's own folder, which stays page-relative because it moves with the page.
  What this replaces: four different `../` depths were in use on one board, each written against wherever the page sat that day, and refold moved every page one level deeper. `../../../` reached a project root before the fold and `../../../../` reached it after, so a correct row and a stale row are indistinguishable by reading.
