...
owner: JL
method: one sentence per source line; everything that attaches to a sentence gets its own face; the unit that ships the contract is plugged in `skill/`
...
**Where this page sits**: `QPs1` takes the whole page and the order its sections keep, and it stops there.
...
**Covered elsewhere**: `QS3` owns the generated `C/H/P/S` address and what an agent acting on one is handed, which is not an attachment: nothing is written under the sentence and nothing enters the file.
`QS2` owns what happens once the records pile up, `QS4` owns proving a write still lands, `QS5` owns the verb that rewrites prose and writes a `✎`, and `QO7` owns locks and two people writing at once.

**What this page carries**: the unit that ships this contract, `haipipe-sentence`, sits in this page's `skill/` plugin, plugged and versioned.
Its own design page, `Design-5`, folded in here on 260816 when the Design kind was folded into the Q series.
...
`QB8a` to `QB8d` and the older `QAb0` to `QAb4` still resolve through `board.md`'s alias map, so old lines keep working; nothing new is written with them.
...
📄 STILL ITS OWN PAGE
   QB8e  filter · resolve · archive · restore · purge
         nothing in it is built, and its identity question is open

🗄 ARCHIVED 260802   _archive/QB8a  QB8b  QB8c
...
The card ruling turned it into TWO surfaces, and the split stopped carrying that: `QB8a` ended up owning both while its own title still said "click a sentence", which is the lane gesture.
...
`QB8e` rules what happens as records pile up: typed views, record states, previewed cleanup, archive, restore, and purge.
...
- `2-QB-board/_archive/QB5a-evidence-card.md`
  Became `### 3` and `### 4`. Its design history stays here rather than being rewritten.
- `2-QB-board/_archive/QB5b-comments.md` · `2-QB-board/_archive/QB5c-editing.md`
  Became `### 5` and `### 6`.
...
- 260802 CC · 🧪 A test that writes into the page it is testing is not a test
  The first browser drive drove `QB8` itself, and did two unforgivable things at once.
...
## Log
- 260816 · [REVISE-CC] `Design-5` folded in, the way `QPs00` folded into `QPs1`: the `haipipe-sentence` unit now rides this page as its `skill/` plugin, because this page is the contract that unit ships. The Design page's prose is archived whole at `_archive/Design-5-haipipe-sentence/`, and `Design-5` resolves here through `board.md`'s `## Links`. Stale ids repointed in the Opening: `QB4` to `QPs1`, `QD8` to `QS3`, `QB8e` to `QS2`, `QE4` to `QO7`, and `QS4` and `QS5` added to the neighbours.
...
260802 · 🗺 The five faces FOLDED, on JL's go ("ok go, and work on it and test it"), option C. `QB8a` `QB8b` `QB8c` became this page's `### 3` to `### 6` and were archived; `QB8d` moved to `QD8` in the working lane, because a generated address is how a machine POINTS AT a location rather than a thing attached to a sentence, and its readers are the chat drawer and the routing verb; `QB8e` stayed its own page, since nothing in it is built and its identity question is open. Same shape and same answer as `QB4`'s seven section faces on 260801. Aims and States rebuilt to eight groups mirroring the eight divisions, every live cross-reference on eight sibling pages repointed, and every retired id aliased in `board.md`'s Links table so nothing already written breaks. Board 54 pages to 51
...
260802 · Legacy ids `QAb0`-`QAb4` and `QA6` replaced by `QB8` and `QB8a`-`QB8e` throughout the prose and Files; the alias map in `board.md` still resolves the old ones, so nothing already written breaks
