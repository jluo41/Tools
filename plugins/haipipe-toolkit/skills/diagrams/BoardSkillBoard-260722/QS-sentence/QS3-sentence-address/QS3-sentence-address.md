...
🧭 THE BREADCRUMB · every ## section and ### subsection
   QB8 / States / Decision Now · QS-sentence/QS1-overview/QS1-overview.md
...
- [x] Copy page id, breadcrumb, and Markdown source path from a heading
      Clicking the chip copies `QB8 / States / Decision Now · QS-sentence/QS1-overview/QS1-overview.md`; a subsection also gains its own `⧉`, which copies that subsection's text the way `##` headings already do.
...
- 260731 CC · 🧭 Heading focus is built, and it reuses every contract the sentence rail already had
  ...
  Driven in Chrome rather than read: the chip put `QB8 / States / Decision Now · QB-board/QB8-where-we-are.md` on the clipboard, `⧉` put that subsection's text there, and `🤖` opened this page's own session with the Focus card showing the breadcrumb and the source path.
