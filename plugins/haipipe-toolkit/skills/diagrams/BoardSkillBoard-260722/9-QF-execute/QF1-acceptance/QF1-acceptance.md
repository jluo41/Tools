...
- 260801 JL · 🔬 A THIRD instrument exists, and this face's two were green through every failure it catches
  ...
  Six failures shipped on 260731 with this face's checker green and a reader who would have found nothing: the index rendering as inline links because invented class names matched no CSS, the left page list missing from a template, six ASCII figures never emitted, every write from a tree page failing silently, a CSS fix that never reached the page for want of cache-busting, and a stray triangle that survived two fixes on a specificity tie.
