# haipipe-page-for-venue · v0.1.2
state: 🟡 in flux · lifted 260803 · 13 of 16 venue pages conform · the 0.1.0 contradictions fixed at 0.1.1
owner: JL
method: three managed spans sync from the skill folder; everything else is written by hand

## Opening
`haipipe-page-for-venue` is the contract for a venue page, which is one page per place a paper is submitted to.
That place is a journal, a funder, or a patent office.
Its sibling `haipipe-page-for-skill` governs the other page kind that describes rather than decides.
...
**Why its placement was worth a second look**: it shipped in `skills/board/`, beside the base, while `haipipe-page` then stated that a variant ships under its CONSUMER and never beside the base.
`haipipe-page-for-skill` was allowed there because its consumer is the board family itself.
...
**Covered elsewhere**: `haipipe-page` owns the frame both variants extend and this file never repeats a rule from it.
`Skill-6` mirrors `haipipe-page-for-skill`, the variant this one is most often confused with, and the two disagree on purpose about Aim form: the skill-page variant drops the base's `A<n>` ids and `Done when`, and this one keeps them, which its changelog says out loud so a reader arriving from the roster does not assume every variant drops them.
...
**Where it stands**: it is registered, which the sibling variant was not, so the failure that made `haipipe-page-for-skill` unloadable for a day did not repeat here.
...
## Diagram
<!-- haipipe:skill:tree:start fd019b20e078a98a board/page-types/haipipe-page-for-venue -->

**What `haipipe-page-for-venue` ships**: every file in the folder, with the one-line purpose each one states for itself.

```
haipipe-page-for-venue/
  CHANGELOG.md          50 ln  haipipe-page-for-venue · Changelog
  SKILL.md             381 ln  /haipipe-page-for-venue · a venue page records a desk it cannot argue with
  template.md          608 ln  <!-- TEMPLATE · ONE SUBMISSION TARGET = ONE QBv PAGE.
```

<!-- haipipe:skill:tree:end -->

**How `haipipe-page-for-venue` is used**: the two sources it merges, the one that wins, and the board it writes onto, which is not this one.

```text
WORKFLOW  writing or fixing ONE QBv page

   📄 haipipe-page              the BASE · every page kind
      five on-stage sections, fixed order · the Opening split · numbering
        │
        │  loaded FIRST, never repeated below
        ▼
   🏛 haipipe-page-for-venue           ONLY the delta
      ┌──────────────────────────────────────────────────────────┐
...
## Content
<!-- haipipe:skill:body:start fd019b20e078a98a board/page-types/haipipe-page-for-venue -->

**haipipe-page-for-venue** · `0.1.2` · last shipped 2026-08-04

- folder   `board/page-types/haipipe-page-for-venue/`
- tools    not declared
...
- 6 · 📐 The division shape a section division repeats
      ...
      **A blueprint-only pack has no exemplar language, and the page says so.** The per-section shape (`n.1` moves, `n.2` refusals, `n.3` format values, `n.4` the papers' own words) presumes measured work. Fill only what the pack or the office actually says, and never print four subsubsections with empty rows to satisfy the shape.
      ...
      📋 Contracts     THIS FILE, and the base it        haipipe-page-for-venue ·
                       extends                           haipipe-page
      ...
      **A page is a unit of work** (`QB6` §7). A venue page reads a shared playbook and hands the desk's rules to every page downstream.

      ```text
       🏦 BANK    paper/venue/playbook-<venue>/     shared, outside every paper
                    style-profile.md · <OUTLET>/taste.md · <OUTLET>/examples/
       📥 INPUT   <stage>/venue/<page name>/        WHAT THIS PAPER CLAIMED from it,
                                                    and what it deliberately did not
       📤 OUTPUT  the blueprint, which turns the desk's totals into per-section
                  allocations that every section page binds to, and
                  <paper root>/<venue>.bst + .cls, copied and really used
      ```

      A venue page never copies the playbook. It records what it claimed, the same bank-outside / record-inside split ruled for references.

      ⚠️ **Distinguish a DESK RULE from a PACK OBSERVATION.** A desk rule binds and a manuscript breaking it is returned unreviewed; an observation is the pack's measurement of published papers and a drafter may depart from it. A pack that has measured nothing says PACK PRESCRIPTION instead, which is weaker than an observation and much weaker than a rule. Every length on a venue page must say which of the three it is, because a wrong word cap that reads as real can cost a live submission.

      ## 📂 Files

      ```
      page-types/haipipe-page-for-venue/
      ├── SKILL.md            this variant contract
      └── CHANGELOG.md        version history
      ```

      Owns no scripts. The base frame is `haipipe-page`; the engine is `haipipe-board`; the generator for the marked spans is the paper board's `_tools/sync-exemplars.py`; the group's own law lives in that board's `board.md` under `### QBv · Delivery Venue`; the reference implementation is `QBv1-misq.md`.
...
- [x] 🔌 A writer who loads only the base is routed here
      `haipipe-page` 0.11.1 names `haipipe-page-for-skill` as its variant and never mentions this one, so nothing sends the author of a `QBv` page from the base to this contract.
...
- [x] 🗺 Where this variant BELONGS is ruled
      It ships beside the base while its consumer is the paper family, and `haipipe-page` states that a variant ships under its consumer and never beside the base.
...
- 260803 CC · 📄 Page opened for a unit that shipped without one
  Generated with `skillpage.py new --group QC` and written to `haipipe-page-for-venue`, the eighth Skill row on the roster and the second variant of `Skill-3`.
...
260803 0043 · authored half written: Opening, the WORKFLOW figure, Aims, States; `state:` ruled 🟡 in flux from 🔴 OPEN; Aims and States converted from the generator's base-form stub to the checkbox and dated-record form `haipipe-page-for-skill` overrides to
260803 0043 · page generated from `board/page-types/haipipe-page-for-venue/` by `skillpage.py new`

<!-- haipipe:skill:log:start fd019b20e078a98a board/page-types/haipipe-page-for-venue -->
...
260803 · `0.1.0`
      ...
      - **📎 Two Files groups this kind adds**: `🔗 Authority`, what the desk itself publishes, and `📤 Generated`, what a tool writes between markers. Both state an action, which is the base's test for a group name. Recorded after `QBv1`'s Files section was found flat, against `QB4-overall/QB4-overall.md` §6's action menu.
