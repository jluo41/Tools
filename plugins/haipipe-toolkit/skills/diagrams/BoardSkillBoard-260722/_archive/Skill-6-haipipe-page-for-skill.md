# haipipe-page-for-skill · v0.4.2
state: 🟡 in flux · shipped 260802, applied to eight pages, one independent review 260802
owner: JL
method: three managed spans sync from the skill folder; everything else is written by hand

## Opening
`haipipe-page-for-skill` says how to write a page that MIRRORS something: a `Skill-<n>` page standing in for a skill folder, or an `Agent-<n>` page standing in for one agent file.
Load it on top of `haipipe-page`, which covers every page kind; reach past it to this one whenever the page you are writing is about a unit that ships somewhere else.
It shipped 260802, its rules were applied to eight pages that day, and one independent review on 260802 passed the noun-swap check it was built for while failing the pages on their facts.
...
`../Skill-0-haipipe-board/Skill-0-haipipe-board.md` on this board is one: it stands in for the folder `skills/board/haipipe-board/`, which would still be there if this board were deleted.
A decision page is the opposite, because the question it asks exists nowhere but on the page.

**Why the base alone was not enough**: `haipipe-page` already forbids interchangeable prose, in the noun-substitution test, and five skill and agent pages were written from one template anyway.
...
**Covered elsewhere**: `haipipe-page` owns the frame this extends, the sections and their fixed order, and this file never repeats a rule from it.
...
The stage variant, `haipipe-page-for-stage`, began as `haipipe-paper-stage` under the paper family; since 260805 that door is retired and all ten variants, this one included, live under the base's `page-types/`, because a variant ships where the board family maintains it.
...
## Diagram
<!-- haipipe:skill:tree:start e3fe15c4928f92cb board/page-types/haipipe-page-for-skill -->

**What `haipipe-page-for-skill` ships**: every file in the folder, with the one-line purpose each one states for itself.

```
haipipe-page-for-skill/
  CHANGELOG.md          90 ln  haipipe-page-for-skill · Changelog
  SKILL.md             307 ln  /haipipe-page-for-skill · a skill page is not a decision page
  template.md          369 ln  Template · the GENERATED mirror Page, and the eight slots a person writes
```
...
## Content
<!-- haipipe:skill:body:start e3fe15c4928f92cb board/page-types/haipipe-page-for-skill -->

**haipipe-page-for-skill** · `0.4.2` · last shipped 2026-08-04

- folder   `board/page-types/haipipe-page-for-skill/`
- tools    not declared
...
## States
The contract shipped 260802, was applied to eight pages that day, and has been through one independent review that passed the check it was built for and failed the pages on their facts.
...
## Log
- 260806 2115 · [REVISE-CC] swept to the 260806 architecture; dropped the retired "seven sections" count from the Opening and the WORKFLOW fence (the base stopped claiming a count, recorded by this unit's own 0.4.1) and anchored the two live-prose "applied to eight pages" claims to 260802, since ten pages on this roster alone now follow the contract
...
<!-- haipipe:skill:log:start e3fe15c4928f92cb board/page-types/haipipe-page-for-skill -->
...
260802 · `0.1.0`
      ...
      - **Why the base could not have prevented it, which is the reason to ship a variant
        rather than tighten a rule.** `haipipe-page` already carries the
