...
`QB8 §6` owns editing body prose from the page, and `QE4` owns what happens when two people write at the same time.
...
No canvas frame is linked here. `board.excalidraw` holds no frame named `QB7`, and JL ruled on 260802 that a page shows no link rather than one that 404s; the link returns when someone draws the frame.
...
`board.excalidraw` is one file holding one frame per page, named for the page: `QB7`, `QD5`, `QB8`.
...
```
 the whole board, for drawing
   127.0.0.1:5599/_excalidraw/#url=http://127.0.0.1:5599/Tools/plugins/haipipe-toolkit/skills/diagrams/BoardSkillBoard-260722/board.excalidraw
...
One surface can say QB8 feeds QB4, that QD is the live layer, that QE6 is the local half of QE1, and it can say it by position and arrow rather than by prose.
...
- P2 · This page's own 🖼 Diagram carries a canvas that opens.
  **Done when:** `board.excalidraw` holds a frame named `QB7` and the ✏️ Excalidraw row shows it.
...
- 🧠 P2 · Waiting on a drawing. The URL line was repointed from `frame=QD5` to `frame=QB7` on 260802, since QD5 is a different page today; `board.excalidraw` carries no `QB7` frame yet, so the row stays empty until someone draws one.
...
- `board/QPf/QPf2-diagramattach.html`
