...
```
   ...
   src/            the RENDER half           live/activity.py     the focus-time DB
     ...
     over 8 modules (QC1b, shipped 260724)
                                            src/common.py stays the one shared floor
   one process · one port · 5599 only       both halves, no duplication (QC1b Law)
                                            since then: home · shell · turnring joined live/
```

## Content
### 1 · What is actually in there
**Inventory 260731**: serve.py's 2938 lines counted by area, biggest first.
```text
📏 serve.py · 2938 lines · one Handler class · 65 methods
🎨 excalidraw       443  ███████████
💬 chat             414  ██████████
⏱ activity         390  ██████████
✍️ write-back       334  ████████
⌨️ terminal         322  ████████
🌐 http plumbing    291  ███████
🧩 image paste · HOLD · structure edits   48
📦 module level    ~670  rules texts · prime_context · tool_brief · _slugify · PTY helpers
```
...
**Two readings of the server prefix**: the fleet that 5599-only rules out, and the package that shipped.
```text
❌ a fleet    server_excalidraw :?  ·  server_xxx :?  ·  server_main :5599
   🚧 blocked   only 5599 is forwarded (QD3) · tailnet + Remote-SSH reach nothing else
   🔗 torn      one --root · one HOLD table (QD1) · one rebuild path, split across processes
✅ a package  one process · one port 5599 · one do_POST routing table
   📂 modules   live/chat.py · live/term.py · live/xcal.py · live/write.py · ...
   🤝 shared    the same board files on disk, no duplication
```
...
**Mixins against a context object**: how 65 methods sharing one self can leave the file.
```text
🧲 shared self   root · reply() · target() · hold() · rebuild()
🅰️ mixins        Handler(ChatMixin, TermMixin, XcalMixin, WriteMixin,
                 ActivityMixin, StructureMixin, BaseHandler)
   ✅ textual     every body and its self unchanged · the gate can prove it
🅱️ context obj   plain modules · explicit ctx · testable offline
   ❌ redesign    every signature changes · no gate tells move from rewrite
⚖️ QC1b Law      A for the move · B stays available as its own gated step
```
...
`QC1b`'s Law rules this without needing a new one: refactors move code under a byte-identical gate, and features never ride along in the same step.
...
**The live layer's home**: two halves, one shared floor, modules named for what they serve.
```text
🏠 src/        the render half (QC1b, 260724) · named for what they RENDER
🛰 live/       the live layer · named for what they SERVE
   💬 chat.py · ⌨️ term.py · 🎨 xcal.py · ✍️ write.py · ⏱ activity.py · 🧱 structure.py
🧱 common.py   the one shared floor, imported by both halves
🚪 serve.py    thin CLI + routing table · build.py's 260724 shape
🔎 name clash  root xcal.py ↔ live/xcal.py · shipped anyway, no confusion
```
...
The board's own word for that half is the live layer (`QC1b`), so `live/` uses vocabulary the board already speaks.
The modules are named for what they SERVE, mirroring `QC1b`'s rule that render modules are named for what they render.
...
**The order, planned and inverted**: settled areas first, moving areas last, then the slicer moved all seven.
```text
🥇 planned first  🎨 excalidraw · ⏱ activity · ✍️ write-back  = 1167 lines, all still
   🧱 rides along  structure edits
🕒 planned last   💬 chat, QD2 M1 about to rewrite it · ⌨️ terminal, PTY fresh (0.64.0)
🌀 inversion      the ast slicer made the move free → all seven moved 260731
✅ landed         QD2 M1 inside live/chat.py, the same day (recorded in Aims)
```
...
- [x] 🧪 Build the gate before moving anything
      ...
      `QC1b`'s Lesson repeated itself on the first run: the gate caught `@staticmethod` decorators being dropped by the slicer, which would have shipped eight silently broken methods.
...
- 260731 CC · 🧭 This is QC1b's deferred half, and its Law already governs it
...
### Related work
- `7-QC-engine/QC1b-srcsplit/QC1b-srcsplit.md`
...
260801 0130 · Reindexed QC8 -> QC1c under the new QC2 code-shape parent (JL 260801)
