...
**Covered elsewhere**: `build.py` → `assets/`: `QC1a`. The `src/` render split: `QC1b`. `serve.py` → `live/`: `QC1c`. What SKILL.md exports about all this: `QA6a`. The round trip the split files sit inside: `QC4`.
...
```
   one Law ── "mechanical move under an output-identical gate, features second"
        │
        ├── 🏗  build.py  →  assets/*.css *.js       QC1a  ✅ the template out of the parser
        ├── 🧩  src/ modules, named by what they render QC1b  ✅ page_question.py page_stage.py
        └── 🔌  serve.py  →  live/ mixins + thin CLI   QC1c  🟡 the live layer, moved area by area
```

## Content
### §1 Why one Law, three files
**One gate, three faces**: the same Law applied to build.py, the render Python, and serve.py.
```text
⚖️ the Law     mechanical move · output-identical gate · features second
🏗 QC1a        build.py → assets/     ✅ the grammar stayed, the 800-line JS string left
🧩 QC1b        render .py → src/      ✅ modules named for the page they render
🔌 QC1c        serve.py → live/       🟡 QC1b's deferred half, held while the live layer formed
📄 this page   the shared gate only · each face records its own
```
`QC1a` moved the CSS/JS out of `build.py` so the grammar stayed but the 800-line JS string left.
`QC1b` split the render Python into `src/` modules named for the page they render, giving each future feature a seat before it is written.
`QC1c` is `QC1b`'s deferred half: the same move on `serve.py`, held back only while the live layer was still forming.
...
**25 loose files to 1**: how the top level emptied into tests/ and cli/, and which count actually decided it.
```text
🗂 260801 start    25 loose .py at the top level · 11 of them tests
🧪 the free half   11 test_*.py → tests/         50 passed → 50 passed
🧮 wrong count     serve.py 73 · build.py 64 · stage.py 51 · check.py 50 mentions
✅ real count      15 executing paths, in 6 SKILL.md files
📦 JL's ruling     the 13 runnable scripts → cli/
🏠 top level now   SKILL.md · CHANGELOG.md · status.py (absolute-path exemption) · folders
🔎 check.py's catch  96 dead ## Links hrefs → 0, invisible to the suite
```
...
- `7-QC-engine/QC1a-buildsplit/QC1a-buildsplit.md` · `7-QC-engine/QC1b-srcsplit/QC1b-srcsplit.md` · `7-QC-engine/QC1c-livesplit/QC1c-livesplit.md`
  The family, one file each.

## Log
- 260815 1500 · [REVISE-CC] opening figures added to §1-§2 (division-no-figure debt).
- 260806 2128 · [REVISE-CC] swept to the 260806 architecture; verified clean against the live tree (top level = SKILL.md + CHANGELOG.md + status.py + cli/ src/ live/ tests/ checks/ assets/ ref/ vendor/; cli/build.py, cli/serve.py, src/ modules, QC1a/b/c states), no corrections needed
