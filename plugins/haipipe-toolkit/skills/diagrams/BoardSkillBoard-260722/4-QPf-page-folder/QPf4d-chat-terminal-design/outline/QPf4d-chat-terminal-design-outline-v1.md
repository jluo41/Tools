...
- B4 · So the phone failures arrive together rather than one at a time.   🧮 proof
...
- B7 · So a phone showing the same symptom is a third cause.   🧮 proof
...
- B3 · A phone does not clear that bar.   🧮 proof
...
- B5 · The chat half stalls instead of dying.   🧮 proof
- B6 · A sleeping streaming `fetch` can hang without resolving or rejecting, so the `await` never returns, the `catch` never runs, and the cleanup that clears `inflight` never runs either.   🧮 proof
...
- B2 · `/_board/image` already worked on any device, and both ways in were `paste` listeners, so a phone could not put an image on the board at all.   🧮 proof
...
- B2 · Reading it off the screenshot against the CSS turned one impression into five separate defects.   🧮 proof
