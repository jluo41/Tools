...
The cause moved with the 260816 QA restructure and is now the other way round: the group scene was trimmed to the pages that are left (QA2, QA3, QA6), and `QA00` is on the list with no scene of its own, so the check reports it missing.
A page that carries no drawing has no place in this check, which is the rule §3 still owes.
...
- 260816 · [REVISE-CC] second findings pass
      ...
      States gained the current blocker under §3 and the missing fact for the ascii-only Aim: `draw.py verify` aborts on `1-QA-constitution/draw/group.excalidraw`, which still imports the QA0 and QA1 scenes after both pages folded into QA00, and no live page's `## Diagram` carries an Excalidraw link.
