...
## Log
- 🚢 260831 · [HAIPIPE-PAGE-SKILL, JL ruled] the chat became the page's RUN controller with FULL SPACE access (0.2.0: walls = rules + teeth + one log record per write; phase words run phases in session; the strip from disk) and 0.3.0 added the collect-this-page's-values row (haipipe-task-for-page). Verified live: scoped haiku turn on SM01 after the .venv interpreter fix.
