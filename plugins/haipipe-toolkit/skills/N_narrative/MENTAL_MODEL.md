...
```
Layer     Produces                 Files to E_insight?    Reads E_insight?
────────  ───────────────────────  ─────────────────────  ────────────────
...
becomes permanent knowledge. E_insight has ONE writer: the narrative.
...
📖 ARGUE layer  — "why does this matter?"
    N_narrative     📖 the editor       story + claims + ignite
    A_discover      🔍 the eyes         landscape + literature filter
    F_paper         📰 the typesetter   render story → paper (OUTPUT)
    G_application   💬 the translator   deliver to audiences (OUTPUT)

📊 CLAIM layer  — "does the hypothesis hold?"
    D_probe         📊 the judge        test one hypothesis
    E_insight       🧠 the memory       store validated DIKW

💼 DO layer     — "did this run work?"
    B_project       🏗️ the skeleton     scaffold project structure
    C_task          💼 the hands        execute code → metrics
...
DO runs the experiment → "MAE = 24.6"
  ...
  │ metrics produced
  ▼
ARGUE harvests → files DIKW to E_insight → rechecks claims
  │
...
When Ignite = YES, the ignited narrative feeds BOTH F_paper and G_application:

```
📖 Ignited narrative (curated K/W + angle + claims)
     │
     ├──→ 📰 F_paper           "render for academics" (venue-specific LaTeX)
...
A: Yes — probes can run independently for ad-hoc hypothesis testing.
   ...
   to E_insight, and the results won't feed a paper.

**Q: Can I have a narrative without probes?**
A: Yes — if all claims are already in the literature (A_discover confirms them),
   the narrative can go straight to Ignite with lit-sourced K cards, no probes needed.

**Q: Where does A_discover fit?**
