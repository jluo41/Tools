---
name: haipipe-probe-digest
description: "Utility verb. Digests a session -- the CURRENT one, or a PAST session named/id'd as an argument (run from a fresh session) -- scanning its transcript for tool/skill feedback (gripes + preferences), distilling discrete items, deduping (merge-or-create), and after a MANDATORY confirm gate routing each through the feedback router into the unit it concerns (agents / orchestrator fallback). The bulk harvester for /haipipe-probe feedback. Never auto-files."
...
into the right unit's inbox. A digest is, by definition, deduped and condensed
...
harvest -- `/haipipe-probe digest "Display-for-Opioid-MISQ"`. With no argument it
...
agent-keyword->unit map, the cross-cutting guard, and the merge-or-create
capture. No routing or merge logic is duplicated here. digest's only new job is
turning a session into a clean candidate list and gating it.

## Run: `/haipipe-probe digest ["<session-name|id>"] [--dry-run]`
...
1. SCAN the target session's transcript for TOOL/SKILL feedback signals:
     KEEP  - explicit gripes ("this is clunky", "why did you...", "don't do X")
           ...
           - stated preferences about how a step/skill SHOULD work
           - repeated friction (you had to ask twice, re-route, re-explain)
     DROP  - evidence/claim discussion (about the PROBE's findings, not the TOOL)
           - one-off task instructions ("now link that task", "judge it as partial")
...
2. DISTILL into discrete candidate items, ONE concern each, in YOUR words
   ...
   feedback + a one-off task instruction + a file path (e.g. "the dashboard
   should be a compact glance, not a wall of tables ... go ahead and re-open the
   0605 probe, and link probes/T0621_*"). Extract ONLY the feedback clause(s);
   DROP the task clause ("go re-open it", "link X now"); a path is a location,
...
3. DEDUP each candidate, and infer its target via the feedback router. ROUTING
   ORDER MATTERS: the cross-cutting GUARD runs BEFORE the agent-keyword map
   (fn/feedback.md resolve step 0), so a lifecycle-wide habit like "the stage
   strip never reflects what actually ran" is caught by the guard / the Scope
   rule below and is NEVER keyword-routed to agents. Route by the complaint's
   SUBJECT, not by a unit-name or path it MENTIONS: "add feedback that the
   creator agent's evidence.md kept leaking verdict language" is about the
   Read-step verdict-language rule (a lifecycle-procedure rule true with or
   without agents -> orchestrator fallback), not "agents" just because it names
   an agent.
   ...
   and assign  target-unit  (agents | orchestrator).
4. PRESENT the candidate list for the MANDATORY confirm gate. Group by target
   ...
   on its own (e.g. two orchestrator files both about "the stage strip render").
   ...
   steps use; a session sweep WILL mis-read, so the gate is non-negotiable.)
...
6. REPORT what landed where: "N new, M merged, K dropped" (dup-in-batch items
   ...
   `/haipipe-probe feedback list` to review the result.
```
...
```
digest files ONLY skill-feedback into the unit inboxes (agents / orchestrator
fallback). Some session remarks are GLOBAL BEHAVIORAL preferences (how the agent
should act across ALL sessions, not a probe-skill defect). Do NOT file those --
FLAG them in the report and suggest you `/remember` them. One job per verb.
  skill defect    "the probe dashboard is hard to read"          -> inbox
...
- drop transient:  one-off instructions and evidence/claim talk are excluded.
