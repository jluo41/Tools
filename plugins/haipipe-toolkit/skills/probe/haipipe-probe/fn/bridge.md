...
description: "Bridge specialist of haipipe-probe. Takes a designed probe.yaml (claim + planned arms) and DISPATCHES executable task contracts to task. Invokes the Run Script Reviewer agent on each scaffolded task before deploy (centralized intent ↔ implementation review), optionally runs a sanity arm first, and records task/run refs for later Probe-post resume. The Probe-open → task handoff connector. Called by /haipipe-probe orchestrator. Direct invocation works for bridge-scoped work."
...
The **Probe-open → task handoff connector**. Where `-design` defines what to
test (claim + arms in `probe.yaml`) and `-result` interprets what came back,
this specialist dispatches task contracts to task: it scaffolds the arms as
tasks, optionally reviews the implementation, optionally runs a sanity arm,
then records task/run refs and leaves the probe in `waiting_for_tasks` until
Probe-post resumes.

This is the migrated `probe-bridge` from research-toolkit
(Workflow 1.5), adapted to read from probe's `probe.yaml`
instead of `EXPERIMENT_PLAN.md`, and to call task specialists
...
/haipipe-probe bridge status <probe>
  Show: which arms have been dispatched as tasks, which are running,
...
CODE_REVIEW    = true   Invoke Run Script Reviewer agent on each scaffolded
                        ...
                        used by both task pre-flight and this bridge —
...
Step 2: SCAFFOLD into task
  For each arm run_spec:
    Skill("haipipe-task", args="task-folder <task_type> <arm-task-id>")
    Create/update the task folder under examples/Proj-X/tasks/
...
Step 3: PRE-FLIGHT CODE REVIEW (CODE_REVIEW=true)
  ...
  **Run Script Reviewer** agent (out-of-family intent ↔ impl audit).
  ...
  it by reading its file and handing the body to a Task subagent:
    ...
    Read  skills/task/agents/haipipe-task-reviewer-agent.md
  ...
  This is the SAME agent the task `run.sh` pre-flight gate checks for.
  Running it here means launches at deploy time won't be blocked by a
  missing CODE_REVIEW.md.

Step 4: SANITY (SANITY_FIRST=true)
  Deploy ONLY the sanity arm via 2_nn (which calls task tasks).
...
Step 7: LINK runs to arms
  ...
  probe.yaml arms.<arm>.runs now points to dispatched/completed runs.

Step 8: HANDOFF
  Set probe status to waiting_for_tasks unless all linked runs are already complete.
  task owns execution. When the completion gate is satisfied, resume with:
    /haipipe-probe post <probe>
```
...
examples/Proj-X/
├── probes/<MMDD>_<slug>/
│   ├── probe.yaml              ← READ/WRITE: source of truth (arms + claim + dispatch)
...
WRITES heavily:
- New task folders under `examples/Proj-X/tasks/` (via task scaffold)
- `bridge-log.md` per-probe
- Updates `probe.yaml` with linked-run pointers (via `-design link`)
- Triggers GPU training jobs (via 2_nn / task execution)
...
```
probe-design       defines arms (in probe.yaml)
        │
        ▼
probe-bridge       ← YOU ARE HERE
  └──► task          scaffolds/runs tasks independently
        │
        ▼
probe-design link  binds completed runs back to arms
        │
        ▼
probe-post         resumes after task completion and aggregates linked runs
```
...
next:      /haipipe-probe post <probe>       (after all linked tasks complete)
```
