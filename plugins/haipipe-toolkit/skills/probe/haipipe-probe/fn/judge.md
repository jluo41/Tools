...
(Per-RUN quality moved to task `haipipe-task-reviewer-agent` (GATE 2) —
`review run` delegates to it; see Commands below.)

**Agent embodiment (builder ≠ judge).** The three checks below ARE three
independent probe reviewer agents — dispatch them as `subagent_type` (e.g.
from `haipipe-probe-loop` or a `application` session) for an out-of-family
...
agents are thin pointers to it (`skills/probe/agents/`).
...
/haipipe-probe review run <run-path>
  STRUCTURAL: per-run trustworthiness. DELEGATES to the task agent
  haipipe-task-reviewer-agent (GATE 2) — read skills/task/agents/haipipe-task-reviewer-agent.md
  and dispatch its body to a Task subagent over <run-path>. Per-run
  quality is a task judgment; probe only consumes the verdict.
...
Per-run checklist — MOVED to task
------------------------------------
...
agent that owns it: **`skills/task/agents/haipipe-task-reviewer-agent.md`**
(GATE 2). "Did THIS run produce a trustworthy artifact?" is a task
question, not a probe one.
...
probe does NOT re-implement the per-run list — single source of truth.
...
□ all linked runs pass haipipe-task-reviewer-agent GATE 2 (task)
