...
description: "Utility verb. Captures a complaint/confusion/wish about the probe SKILL itself into feedback/ (fix later); not a probe verdict. `feedback list` shows open items."
...
Captures feedback about the probe SKILL (confusing output, clunky step, missing
verb) into this skill's `feedback/` folder. Does NOT fix anything; fixing is a
separate revision pass. Distinguish from a probe verdict: feedback is about the
TOOL, not a claim.

## Capture: `/haipipe-probe feedback "<text>"`

```
1. Read the active probe + step from .probe-console.yaml if present (for context).
2. Write one file: feedback/<YYYY-MM-DD>_<short-slug>.md
   frontmatter: status: open | created | context (probe/step or "general") | fixed_in: ""
   body: the feedback in the reporter's words, then a trailing "Fix:" line.
3. Confirm captured. Do NOT attempt a fix now.
```

## List: `/haipipe-probe feedback list`
