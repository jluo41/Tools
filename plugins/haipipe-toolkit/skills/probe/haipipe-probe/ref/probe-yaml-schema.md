...
`probe.yaml` is the **state of one research contract**, not a result artifact
and not a container for discoveries or tasks. It belongs to the research
pipeline (probe), which wraps external discovery evidence and internal
execution work like a sandwich:

```
PROBE-OPEN        design + dispatch discovery refs and task contracts
DISCOVERIES       outside evidence → what does the world already know?
C_TASK            task/run → what do our own runs show?
PROBE-POST        harvest/judge → does the hypothesis hold?
```

The yaml evolves over the lifetime of the thread:
  open       hypothesis, claim_target, planned arms, aggregation spec
  dispatch   discovery/task/run refs + completion gate; status=waiting_for_evidence
  post       result block, caveats, claim sentence, evidence refs
...
Owner:    Written by `haipipe-probe-design` (`new`/`link`),
          moved to waiting/resume states by `haipipe-probe-bridge`,
...
NO code, no notebooks, no plots in probe folder. Literature/source evidence
lives in discoveries/. Code/runs/metrics live in tasks/. Both are referenced
from `evidence_refs:` and `dispatch:`.
...
| status          | enum    | any stage        | yes      |
| hypothesis      | string  | design new       | yes      |
| claim_target    | string  | design new       | yes      |
| resolves_K      | string  | design new       | (opt)    |
| evidence_plan   | mapping | design new       | yes      |
| arms            | mapping | design new+link  | if task-backed |
| evidence_refs   | mapping | design/dispatch  | (opt)   |
| aggregation     | mapping | design new       | if task-backed |
| dispatch        | mapping | bridge/dispatch  | (post-open) |
| completion_gate | mapping | design/dispatch  | (opt)    |
...
status: planned       # planned|dispatched|waiting_for_evidence|ready_to_harvest|judged|closed|loop_needed|blocked

# ── INTENT (pre-run, user/Claude writes) ────────────────────────────
hypothesis: |
  LHM-A test-id MAE lower than baseline by ≥ 0.5 mg/dL,
  paired-t p<0.05 at N=3, and prior work does not already establish
  the same claim.
...
# ── DESIGN (external evidence + optional arms to run) ───────────────
evidence_plan:
  discoveries:
    - role: prior_art_check
      question: "Has prior work already shown this robustness mechanism?"
      required: true
  tasks:
    - role: baseline
      required: true
...
# arms = what to run + which runs support each side.
# Required only for task-backed probes.
arms:
  baseline:
    task_type: training
    run_specs:
      - name: run_seed42_baseline
        seed: 42
        params: {}
    runs: []              # filled by `design link` after dispatch/completion
  lhm:
    task_type: training
    run_specs:
      - name: run_seed42_lhm
...
# ── EVIDENCE REFS (filled by design/dispatch; probe reads, never owns) ─
evidence_refs:
  discoveries: []                      # project-relative discoveries/<id> folders
  tasks: []                            # project-relative task folders
  runs: []                             # project-relative run scripts or run ids once known

# ── DISPATCH (filled by bridge/open; discover/task own work) ────
dispatch:
  status: not_dispatched               # not_dispatched|dispatched|waiting_for_evidence|ready_to_harvest
  discovery_refs: []                    # project-relative discovery folders requested by this probe
  task_refs: []                         # project-relative task folders requested by this probe
  run_refs: []                          # project-relative run scripts or run ids once known
  dispatched_at: null
  resume_command: "/haipipe-probe post P.0601"

completion_gate:
  require_discovery_verdicts: false     # if true, every required discovery has verdict.md/status ok
  require_runtime_ok: true
  require_metrics: true
  require_run_audit: true
  trigger_on_complete: "/haipipe-probe post P.0601"
...
must use the object form above so `bridge` has pre-run specs to dispatch.


evidence refs, dispatch, and completion gate
----------------------------

`dispatch:` is the handoff seam between Probe-open and evidence work. It
records what external discovery and internal execution contracts the probe
requested; it does not make discoveries or tasks children of the probe.
...
evidence_refs:
  discoveries:
    - discoveries/D0601_noisy-labels-prior-art
  tasks:
    - tasks/A01_pretraining_clm/01_pretrain_baseline
    - tasks/A01_pretraining_clm/02_pretrain_lhm
  runs:
    - tasks/A01_pretraining_clm/01_pretrain_baseline/runs/run_seed42_baseline.sh
    - tasks/A01_pretraining_clm/02_pretrain_lhm/runs/run_seed42_lhm.sh
...
dispatch:
  ...
  run_refs:
    - tasks/A01_pretraining_clm/01_pretrain_baseline/runs/run_seed42_baseline.sh
    - tasks/A01_pretraining_clm/02_pretrain_lhm/runs/run_seed42_lhm.sh
  dispatched_at: 2026-06-01T18:20:00-04:00
  resume_command: "/haipipe-probe post P.0601"

completion_gate:
  require_discovery_verdicts: true # every linked required discovery has verdict.md + status ok/inconclusive
  require_runtime_ok: true        # every linked run has runtime.yaml status=ok
  require_metrics: true           # every linked run has metrics.json / accepted summary json
  require_run_audit: true         # every linked task has RUN_AUDIT.md pass/warn unless exempt
  trigger_on_complete: "/haipipe-probe post P.0601"
```

discover and task may emit or expose completion status, but neither should
interpret the probe. A watcher/orchestrator or the user resumes Probe-post
when the completion gate is satisfied.
