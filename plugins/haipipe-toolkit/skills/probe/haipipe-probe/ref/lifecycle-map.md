...
| `Gather` | `fn/gather.md` | Is needed evidence missing or already present? | Call missing work, link existing artifacts, check readiness | `probe.yaml`, candidate task/discovery/insight artifacts | `probe.yaml`, `status.md`, optional `gather.md` | `haipipe-task`, `haipipe-discovery` | `status.md`, optional `gather.md` | `probe.yaml.evidence_refs`, `probe.yaml.calls` | costly/PHI work, ambiguous link, approval needed |
