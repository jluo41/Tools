...
metadata:
  ...
  summary: "ADVANCER agent for probe (the third family — neither builder nor judge)."
  changelog:
...
The **advancer** — probe's third role. Builders make probes; reviewers
judge them; I decide where the research should go next. (task has no analog
...
1. Scan all `active probes/*/probe.yaml and archived probes/*-archive/*/probe.yaml` + linked runs; auto-detect design-space axes
...
summary:   "coverage: 3/12 cells confirmed; proposed P.0604 (LHM x data_v2), P.0605 (param-matched re-test)"
