...
input:       a SET of STILL-COLLECTING q-executors — the consumer's 1-probes/ sections whose
             state is planned or commissioned (NOT yet answered/read) — each with its section
             id + a route hint (task | discovery), and the project_root. answered/read sections
             are never sent: there is nothing left to collect.
...
- Match against the CONSUMER's OWN registries (the cost ladder's T1 LOCAL — `_CITATION_`/`_VALUES_`/read sections/`0-displays`) — those are consumer-specific; the stage runs T1 LOCAL FIRST and hands me only the residual. My ② is the BANK (T2) alone.
...
- Choose or author a destination FOLDER for fresh work — the executor orchestrator owns its namespace, decides ENRICH-vs-NEW, and RETURNS the path. I never invent a `leaf:` (I pass an existing folder only when local/reuse already named one).
...
③ DISPATCH   only the unmatched → the executor orchestrator (task or discovery, per the
             section's route hint), the q-executor VERBATIM, run_in_background for fresh work.
             OMIT the leaf for fresh (the orchestrator picks the folder); pass a leaf only when
             local/reuse already named an existing folder (ENRICH). It returns a QA-file PATH.
