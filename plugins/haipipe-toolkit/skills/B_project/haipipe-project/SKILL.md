...
**For the boundary between C_task and D_probe** (the most-confused pair, since both touch results/), see `D_probe/MENTAL_MODEL.md` — onboarding doc with FAQ + "rules of thumb" + walkthrough of one probe's full lifecycle.
...
task, task-folder, new task, scaffold task     -> haipipe-task (C_task/) — task-folder + below only
  (for scope=task-folder, -task dispatches to one of:
     haipipe-task-{data,algo,training,eval,display,individual,agent})
run, new run                                  -> haipipe-task (C_task/) — run scaffold
...
Step 3: Decide specialist:
  ...
  - task-folder / run scaffold           -> haipipe-task (in C_task/)
