# Task RF as an external Wisdom parent
...
`D → I → K → W → RF` under its own Task-only contract. An InsightBoard may use
that completed chain as an **external parent** for a local Wisdom Folder:

```text
Task instance/riNN@version/RF    InsightBoard                      DesignBoard
R + new dataset → RI → D/I/K/W/RF ─▶ Question QW → Wisdom contextual W ─✋─▶ X1 handoff
```

This is not permission to skip a rung inside an InsightBoard workflow. It is a
...
local D/I/K Folders are minted: their evidence authority remains in the Task
Page. Wisdom still performs the InsightBoard operation: applicability, counsel,
forbidden overreach, `serves:`, and human signature—and GI6 still settles the
Question row. The RF itself never satisfies X1 or any Design gate. A stale,
