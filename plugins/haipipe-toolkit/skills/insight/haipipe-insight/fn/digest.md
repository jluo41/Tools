---
name: haipipe-insight-digest
description: "Utility verb. Digests a session -- the CURRENT one, or a PAST session named/id'd as an argument (run from a fresh session) -- scanning its transcript for tool/skill feedback (gripes + preferences), distilling discrete items, deduping (merge-or-create), and after a MANDATORY confirm gate routing each through the feedback router into sub-skill inboxes. The bulk harvester for /haipipe-insight feedback. Never auto-files."
...
harvest -- `/haipipe-insight digest "Insight-for-Opioid-MISQ"`. With no argument it
...
## Run: `/haipipe-insight digest ["<session-name|id>"] [--dry-run]`
...
1. SCAN the target session's transcript for TOOL/SKILL feedback signals:
     KEEP  - explicit gripes ("this is clunky", "why did you...", "don't do X")
           ...
           - stated preferences about how a layer/skill SHOULD work
           - repeated friction (you had to ask twice, re-route, re-explain)
     DROP  - card-content discussion (about a D/I/K/W CARD, not the TOOL)
           - one-off filing instructions ("now file the K card", "fix this slug")
...
2. DISTILL into discrete candidate items, ONE concern each, in YOUR words
   ...
   feedback + a one-off filing instruction + a card path (e.g. "the K card
   overclaimed beyond its evidence scope ... go ahead and re-file it, and the
   card lives at insights/K_knowledge/K03_*.md"). Extract ONLY the feedback
   clause(s); DROP the task clause ("go re-file it", "file X now"); a path is a
   location, not feedback. One turn may yield TWO feedback items (split them) or
   ZERO.
3. DEDUP each candidate, and infer its target via the feedback router. ROUTING
   ...
   (fn/feedback.md resolve step 0), so a DIKW-wide habit like "the INDEX never
   rebuilds after I file a card" is caught by the guard / the Scope rule below
   and is NEVER keyword-routed to -data. Route by the complaint's SUBJECT, not by
   a skill-name or path it MENTIONS: "add feedback to /haipipe-insight-data that
   the K writer dropped the confidence field" is about KNOWLEDGE (the subject),
   not -data (a mis-named destination).
...
4. PRESENT the candidate list for the MANDATORY confirm gate. Group by target
   ...
   on its own (e.g. two knowledge files both about "K card overclaims scope").
   You: approve / edit text / re-route / drop each item. NOTHING is written
   before you confirm. (This is the same illuminate->confirm gate the review/apply
   flow uses; a session sweep WILL mis-read, so the gate is non-negotiable.)
...
6. REPORT what landed where: "N new, M merged, K dropped" (dup-in-batch items
   ...
   `/haipipe-insight feedback list` to review the result.
```
...
not an insight-skill defect). Do NOT file those -- FLAG them in the report and
suggest you `/remember` them. One job per verb.
  skill defect    "the K card overclaimed beyond its evidence scope"  -> inbox
...
- drop transient:  one-off instructions and card-content talk are excluded.
