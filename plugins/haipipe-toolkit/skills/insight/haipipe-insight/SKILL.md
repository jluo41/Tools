...
description: "Insight base orchestrator (the insight umbrella). The project's permanent knowledge base under examples/<project>/insights/ (D_data / I_information / K_knowledge / W_wisdom). Three jobs: (1) route filing requests to the right DIKW specialist, (2) post-file accumulation check — suggest promotion when enough cards accumulate, (3) dashboard — show KB state. Called by task Stage 5 (→D), probe convergence (→K), application Phase 4 (→any), or human directly. Owns I + W synthesis. Never executes code, never triggers probes."
...
```
task       EXECUTES runs          → files 🟦 D cards (Stage 5)
probe      TESTS hypotheses       → files 🟨 K cards (convergence)
insight    ACCUMULATES + PROMOTES  ← THIS SKILL FAMILY
paper      PUBLISHES from K + W
```

Unlike task (which transforms ONE folder through linear stages), insight is an **accumulation + promotion funnel**: cards arrive one at a time, build up, and when enough accumulate at one level the next level becomes fileable.
...
**Hard rule:** NO code, no Python, no notebooks, no plots inside insights/. That work belongs to task (code) or probe (claim verdicts). insight only synthesizes markdown.
...
This orchestrator has four jobs:
...
Job 4: EXPORT   write derived OKF-compatible views from the source cards
```

**Job 1 (Route)** is the verb dispatch that already exists — same as task routing by type.

**Job 2 (Check)** is what's new — the insight equivalent of task's stage transitions. But instead of "advance to the next stage," it's "suggest promotion to the next DIKW level when enough cards accumulate." This fires automatically after every successful file operation.

**Job 3 (Dashboard)** is the explore skill — shows what the KB has, what's promotable, what's missing.

**Job 4 (Export)** is the OKF compatibility view — writes a derived
...
```
haipipe-insight-data            D-LEVEL:  file observation card → D*.md  (called by task Stage 5)
haipipe-insight-information     I-LEVEL:  synthesize cross-D pattern → I*.md  (owned by insight)
haipipe-insight-knowledge       K-LEVEL:  file belief from probe → K*.md  (called by probe convergence)
haipipe-insight-wisdom          W-LEVEL:  synthesize recommendation → W*.md  (owned by insight)
...
insight/agents/
  creators/                                  the headless, agent-callable path
    card-creator-data-agent.md               🟦 called by task Stage 5
    card-creator-information-agent.md        🟩 called by application or human
    card-creator-knowledge-agent.md          🟨 called by probe convergence
    card-creator-wisdom-agent.md             🟧 called by application or human
...
Step 2: Resolve scope. Cascade (highest to lowest confidence):
  ...
  (4) EXPORT — first positional is `export-okf` or `okf` → run
      `scripts/export_okf.py <project-or-insights-path>` and report artifacts.

  (5) NO ARGS → Job 3 (dashboard): run explore to show KB state.

  (6) STILL AMBIGUOUS → AUTO: `status: blocked`. Interactive: ASK.
...
Step 2a: Source-type detection (path-based routing).
  ...
  For probe-folder: after filing K, optionally chain W if the K card is actionable (same as probe convergence behavior).
...
insight's flow is a funnel, not a pipeline. Cards arrive at different rates from different producers. Promotion happens when enough cards accumulate at one level:

```
        task Stage 5                probe convergence
              │                              │
              ▼                              ▼
        ┌───────────┐                 ┌───────────┐
        │ 🟦 D_data │                 │ 🟨 K_know │
...
```
🟦 D data      ← task Stage 5 files per-task observations (Skill("haipipe-insight-data"))
🟩 I info      ← insight synthesizes cross-task patterns (when ≥2 D cards accumulate)
🟨 K knowledge ← probe convergence files per-probe beliefs (card-creator-knowledge-agent)
🟧 W wisdom    ← insight synthesizes cross-probe actions (when K cards accumulate)
```
...
```
insight is CALLED BY task (Stage 5 → D) and probe (convergence → K)
insight OWNS synthesis: I (cross-D patterns) and W (cross-K actions)
...
probe NEVER reads from insights/
```
...
task        CALLS insight: Stage 5 → Skill("haipipe-insight-data") → 🟦 D card
              (per-task observations, insight-worthy types only)
probe       CALLS insight: convergence → card-creator-knowledge-agent → 🟨 K card
              (per-probe beliefs from confirmed/refuted claims)
paper       READS K + W entries → academic publication
application CALLS insight: Phase 4 → files D/I/K/W cards
