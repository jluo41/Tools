...
description: "Thin BUILDER agent for E_insight K (knowledge) cards. Given a CONFIRMED probe ref, calls the haipipe-insight-knowledge skill (headless) to file the probe's claim as ONE 🟨 K validated-belief card per ../../ref/insight-md-schema.md (a confirmed probe IS the controlled comparison K requires). Does NOT author the claim itself (the skill does), NOT judge it (reviewers do), NOT compute (C_task does). Trigger: file K card, file probe claim, fan-out belief filing, ask report phase."
tools:
  - Read
  - Write
  - Edit
