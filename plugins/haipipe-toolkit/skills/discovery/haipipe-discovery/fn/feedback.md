...
The discovery layer root is the `skills/discovery/` directory (resolve symlinks:
the orchestrator is reached via `.claude/skills/haipipe-discovery` ->
`…/skills/discovery/haipipe-discovery`, so the root is one level ABOVE the
orchestrator folder, i.e. `…/skills/discovery`, NOT
`…/skills/discovery/haipipe-discovery`). Inboxes are created LAZILY on first
