...
This is the live third Discovery capability, Synthesize. It owns cross-Result
integration during D1 `SYNTHESIZE` and the shared Page `03 CONTENT / WRITE`
handoff. It sits at the family root; there is no `3_synthesize/` folder because
no external original is called here, and the stage is not a Block, Job, Task,
Run, or extra Page level.

The distinction is deliberate:

```text
1 Search      locate, resolve, and admit candidate Subjects        haipipe-discovery-search
2 Review      inspect one Subject/Result, extract reliable claims     haipipe-discovery-review
3 Synthesize  combine accepted Results into the promised Task Page   haipipe-discovery-synthesize
```
...
`haipipe-discovery-inquiry/ref/workflow-table.md` first. Then
...
`source-map` belongs to `haipipe-discovery-search`. `source-reading` belongs to `haipipe-discovery-review`.
...
- `haipipe-discovery-synthesize` never creates `runs/`, `results/`, or a local `rNN`.
- `haipipe-discovery-synthesize` never creates or names `rpNN` Page Runs; the shared Page
