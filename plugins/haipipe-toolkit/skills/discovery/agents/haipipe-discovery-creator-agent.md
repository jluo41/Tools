...
`discoveries/<leaf>/QA/<n>-<slug>.md`. **I hold the pen.** Whoever asked may have CAUSED
...
`QA_CLAIM_TTL_HOURS = 24`, I leave it alone and report it. (Past the TTL it is a ZOMBIE —
the orchestrator's qa gate decides whether to RECLAIM it; that is a gate decision, not a
Report decision.)

Rules, all mechanical:

- `<n>` = creation order in THIS leaf. **The numbering IS the index** — `ls QA/` is the
  index. No INDEX file.
