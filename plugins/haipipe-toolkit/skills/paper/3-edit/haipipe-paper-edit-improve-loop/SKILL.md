---
name: haipipe-paper-edit-improve-loop
...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from auto-paper-improvement-loop to haipipe-paper-edit-improve-loop; consolidated into 3-edit/ (haipipe-paper-* name unification)."
...
This skill is designed to run **after** Workflow 3 (`/haipipe-paper-structure-plan` → `/haipipe-paper-structure-figure` → `/haipipe-paper-edit-write` → `/paper-compile`). It takes a compiled paper and iteratively improves it through external LLM review.
...
| Proof gap (theory papers) | Run `/haipipe-paper-edit-proof-checker` if PROOF_AUDIT.md doesn't exist yet; fix FATAL/CRITICAL issues |
| Writing clutter / passive voice | Apply sciwrite 5-pass audit: clutter extraction → active voice → sentence architecture → keyword consistency → numerical integrity. See `haipipe-paper-edit-write` Step 5 |
| Number mismatch (paper vs results) | Run `/haipipe-paper-edit-claim-audit` if PAPER_CLAIM_AUDIT.md doesn't exist; fix any `number_mismatch` or `aggregation_mismatch` claims |
