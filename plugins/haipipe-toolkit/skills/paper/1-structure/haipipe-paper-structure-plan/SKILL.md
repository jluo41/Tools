---
name: haipipe-paper-structure-plan
...
metadata:
  ...
  summary: "Generate a structured paper outline from review conclusions and experiment results."
  changelog:
    - "1.1.0 (2026-06-05): renamed from paper-plan to haipipe-paper-structure-plan (haipipe-paper-* name unification)."
...
- **TARGET_VENUE = `ICLR`** — Default venue. User can override (e.g., `/haipipe-paper-structure-plan "topic" — venue: NeurIPS`). Supported: `ICLR`, `NeurIPS`, `ICML`, `CVPR`, `ACL`, `AAAI`, `ACM`, `IEEE_JOURNAL` (IEEE Transactions / Letters), `IEEE_CONF` (IEEE conferences).
...
## Next Steps
- [ ] /haipipe-paper-structure-figure to generate all figures
