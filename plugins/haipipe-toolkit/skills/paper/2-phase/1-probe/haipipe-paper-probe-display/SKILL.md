...
display probe worker for `haipipe-paper-section-edit`. Owns the display lifecycle for one section: identifying what figures/tables the section needs, planning which display units serve which claims, requesting missing units via DR rows in the 4-display inbox (section-edit never creates displays), and linking existing/done units back to the section.
...
1. **NEVER create ad-hoc plots inline.** All figures and tables route through display units in `0-displays/` and are generated by task-folders. The paper layer plans, it does not plot.

2. **Section-edit never CREATES a display** (JL 2026-07-10: "we don't create the display ourself"). A unit that does not exist is REQUESTED: append a DR row to `0-lifecycle/4-display/_DISPLAY_REQUEST.md` (the display stage's inbox) and mark the need 📨 requested in `_DISPLAY_{section}.md`. The display stage — with its index, unit interrogation, venue display set, gallery, and CHECKLIST — is the ONLY creator of units. No `/haipipe-task` calls, no PP-card commissioning of unit generation from section context. SCOPE: this ban binds section/narrative context. When THIS worker is invoked BY the display stage (4-display) on its own accepted units, commissioning is that stage's job and follows the display hub's PROBE lanes — no DR row is filed into the stage's own inbox.

3. **NEVER fabricate display content.** Display units contain real data outputs. The agent plans layout and routing, not data.

4. **`_DISPLAY_{stage}.md` is the working document.** It records what displays are NEEDED (need → unit → status), parallel to `_CITATION_` and `_VALUES_`. The units themselves live in `0-displays/displayNN/` with their own `float.tex`, rendered output, and source data; the agent links units back to needs, not the other way.
...
**Missing unit -> DR request.** Append to `0-lifecycle/4-display/_DISPLAY_REQUEST.md`
(create the file on first request; bullet lines, no tables; DRNN numbering per file):
...
0. LINK only units that EXIST (or whose DR row is `done` with the unit path filled). A `requested`/`accepted` DR row stays 📨 pending — flag it for CHECK, never pre-place a `\ref` for it.
