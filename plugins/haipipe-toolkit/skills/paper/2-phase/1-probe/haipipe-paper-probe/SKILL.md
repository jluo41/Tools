...
2. **Reuse-before-create -- decided by the AGENT, not this worker.** The gateway's SWEEP scans the project's evidence base (discoveries, tasks, insights; legacy probes/ read-only) in clean context and picks the shape: (a) REUSED -- an existing artifact fully covers the need, pure read, the card's `refs:` point at it; (b) ENRICHED -- same-topic deltas dispatched into an existing discovery's ledger; (c) FRESH -- new discovery/task work commissioned. No shape creates a probe folder (folderless refactor 2026-07-05): a full-mode verdict's home is the PPNN card's `## Verdict` section, landed at TRANSLATE. Duplication is a mental-model tax: two half-overlapping evidence sets cost more than one enriched one.
...
0. **RE-INVOKE PER RUN.** Every stage's PROBE phase begins by invoking THIS skill fresh via the Skill tool — even when its text is already in context from an earlier stage of the same session. Contracts version fast; running from a stale in-context copy is how test-123333333's PP02 executed a 3-hour-old contract that missed same-day rules.
...
3. **TRANSLATE (probe is paper-unaware; this worker is the bilingual layer).** The worker reads NO project files; it may verify returned refs resolve with a bare `ls` (existence only, never content). Light returns -- anchored takeaways (<=5 lines, each with its source anchor) transcribe into the plan file (`status: read`; `refs:` = the execution artifacts the return names — discoveries/.../sources.md, tasks/... — always direct; probes/ folders no longer exist, folderless refactor 2026-07-05). When the return carries a `pick_list`, dispatch the citation HARVEST SUBAGENT (`haipipe-paper-probe-citation` harvest form) on the CHEAPEST model tier (Agent `model: "haiku"`, effort low) — harvest is pure transcription from sources.md, and the mechanical acceptance below catches failures; if the one acceptance-reject retry is needed, re-dispatch it one tier up instead of same-tier. The dispatch prompt tells the subagent to READ the card spec in that skill's SKILL.md -- NEVER paraphrase the spec into the prompt: a compressed re-enumeration is exactly how the identity bullet (authors/year/venue) got dropped in test-2-2222 (spec-drift by telephone game; the spec file is the single source of truth). The subagent expands the picked sources.md entries into `_CITATION_{stage}.md` cards in its own clean context; this worker then does MECHANICAL ACCEPTANCE -- and mechanical means RUN THE COMMANDS, never eyeball (run-3 acceptance claimed "each has anchor + finding" while `grep -c 'finding:'` returned 0):
   ...
   One reject → re-dispatch the harvest subagent with the defect list (one retry); still failing → mark the plan file `status: read (harvest DEFECTIVE)` and surface it in the stage reply. Produce and review are never the same context. Full returns -- the gateway's verdict block (G1/G2/G3 + verdict + reasoning + judged-by + date) lands in the PPNN card's `## Verdict` section, the card goes `status: verdicted`, and the claims ledger's C-section flips in the same TRANSLATE; sections / round logs backfill from the card, never from memory. Buffer convention: `../../../haipipe-paper/fn/probe-plans.md`.
