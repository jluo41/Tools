...
specifics, section-edit logic, and phase-status strip forms.
...
the entries still owed after ② MATCH, handed as a SET
        ...
        +-- on return, this worker writes NOTHING bank-side:
              the QA-probe's `#### A-executor` copies the answer in, and the owning E<n>
...
At ⑤ INTERPRET the QA-probe's `#### A-executor` is copied in and the owning E<n> division's A-consumer
...
See the Evidence Routing Protocol + Delivery Need Routing sections in ../SKILL.md.
