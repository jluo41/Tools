...
This is a craft pack for the S09 build group, loaded by the CHECK phase only when a section carries `\begin{proof}`; it is DATA, not a registered skill, and it does not run alone as the CHECK gate.
Source: converted from `workers/haipipe-paper-proof-checker/SKILL.md` on 2026-08-05 (thin-paper phase 2); its `ref/` files ride along in this folder.
