...
metadata:
  ...
  group-token: "Story-<letter>-narrative-<desk>"
  outline:
    mode: grammar
...
the Story's folder, one per desk, carrying its parent's letter (0.9.0, JL
260907; the flat phase-numbered sibling layout and the older `A2-NA-narrative`
group are grandfathered):

```text
Paper-<Slug>/A1-Story/
└── Story-<letter>/                   the Story · seed + RQ table
    ├── Story-<letter>-roadmap/       plan to COLLECT
    ├── Story-<letter>-narrative-<desk1>/ THIS PAGE · plan to SHOW · how the paper is
    │                                 told to desk 1 · writes the RQ table's
    │                                 "show" column
    └── Story-<letter>-narrative-<desk2>/ how it is told to desk 2 · a retarget adds
                                      a sibling here, never a new Story
```

The group law: the narrative pages decide the telling, one desk one page; no
manuscript prose lives here, and every `Story-<letter>-narrative-<desk>` names its
