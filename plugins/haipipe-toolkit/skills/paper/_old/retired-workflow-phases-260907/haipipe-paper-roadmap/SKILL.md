...
metadata:
  ...
  group-token: "Story-<letter>-roadmap"
  outline:
    mode: grammar
...
└── Story-<letter>/                   the Story · what the paper IS · the RQ table
    ├── Story-<letter>-roadmap/       THIS PAGE · plan to COLLECT · where to go
    │                                 next AND what came back · writes the RQ
    │                                 table's "collect" column
    └── Story-<letter>-narrative-<desk>/ plan to SHOW · one per desk
```

This page and its parent are wholly venue-free; the desk layer starts at the
`Story-<letter>-narrative-<desk>` sibling (0.9.0, JL 260907: one Story = one
idea, the letter is the stable Story identity and the child carries its
parent's letter). Every
...
```text
pagex/     binds the parent Story page, Story-<letter> (its §3 RQ rows
