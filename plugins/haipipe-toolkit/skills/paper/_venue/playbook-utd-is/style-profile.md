...
### Theory / Theoretical Framework section
- **Separate from literature review; different jobs.** Lit review surveys what others found ("X et al. found that..."). Theory derives what SHOULD be true in this study and WHY, using named mechanisms ("because X, we expect Y"; "this suggests that"; "therefore"). A theory paragraph argues toward a prediction; a lit review paragraph surveys a topic.
- **3-6 paragraphs per subsection, each 5-8 sentences.** One paragraph per subsection is too thin (reads as topic sentence without development). Each paragraph carries one step of the argument: define the construct, name the mechanism, cite evidence the mechanism exists, argue why it applies in this context, state the directional consequence.
- **Mechanism-derivation structure per paragraph.** (a) name a mechanism or causal logic, (b) cite evidence that this mechanism exists in the literature, (c) argue why it applies in THIS specific context, (d) state the directional consequence. This is the signature move that distinguishes theory from lit review.
- **Hypotheses embedded at end of each subsection**, not collected in a standalone "Hypotheses" subsection. Each H closes the argument that derived it. (Liu: H1 at end of Openness subsection, H2 at end of Conscientiousness, etc. Bao: H1 at end of 4.1, H2 at end of 4.2.)
- **~25-35% original theoretical assembly, ~65-75% cited.** The "theory" is assembling existing mechanisms into a new configuration that generates testable predictions. Not inventing new theory; assembling and deriving.
- **No measurement method in the theory section.** The theory takes the construct (e.g., "patient-perceived agreeableness") as given and derives what it should predict. How the construct is operationalized (LLM pipeline, survey, etc.) belongs in the Data/Methods section. Brief conceptual notes on WHY the construct matters (e.g., perceived vs objective) are acceptable.
- **Competing predictions: develop the favored pathway in detail, state the alternative concisely.** If competing hypotheses are used, give the developed pathway full subsection treatment (mechanisms + evidence + context); the alternative gets enough development (2-3 sentences citing evidence) to be genuinely grounded, not a straw man.
- **Falsification / placebo tests make a third H distinct.** A boundary hypothesis (H3) is genuinely distinct from a moderation hypothesis (H2) when it tests a different context where the mechanism should NOT operate (e.g., a different drug-condition pair with objective biomarkers), not merely the low end of the same gradient.
- **Citation density: ~0.50 citations per sentence (1 citation per 2 sentences).** Measured from Liu (MISQ, 0.48) and Bao (ISR, 0.53). About half of all sentences carry zero citations; the other half carry one or more. Citations are mandatory for: (a) construct definitions ("agreeableness encompasses..." + cite), (b) established-relationship claims ("X is associated with Y" + cite), (c) clinical/institutional facts ("CDC recommends..." + cite). Citations are absent from: logical inference and reasoning bridges ("this convergence leads to..."), transitional connectives, and hypothesis statements themselves. A theory section with <0.20 cites/sentence reads unsupported; >0.70 reads like a lit review dumping references without derivation.
- **Prefer top-tier journal citations when possible.** When searching for references, prioritize top-tier outlets first: Psychology (JPSP, Psychological Bulletin, Psychological Review, Annual Review of Psychology), Clinical/medical (NEJM, JAMA, Lancet, BMJ, Annals of Internal Medicine, plus domain-leading journals like Diabetes Care, Pain), IS (MISQ, ISR, Management Science, Organization Science), General (Science, Nature, PNAS). Canonical handbook chapters (Guilford, Wiley, Cambridge) are also strong for foundational constructs. Mid-tier and specialty journals are acceptable when no top-journal alternative covers the specific claim, but the discovery process should search top journals first and collect as many top-tier references as possible.
- **Micro-level mechanisms with aggregate-level tests are standard.** Theory sections routinely propose encounter-level or individual-level mechanisms (empathic responsiveness, knowledge sharing, information integration) that are not directly observed in the data. The theory proposes WHY the association should exist; the data tests WHETHER it exists. The cross-context gradient (e.g., H1-H2-H3 across varying ambiguity levels) is what makes the mechanism credible without direct observation. Do not add "we do not observe this directly" caveats in the theory section; discuss observability limits in limitations if a reviewer raises it.
