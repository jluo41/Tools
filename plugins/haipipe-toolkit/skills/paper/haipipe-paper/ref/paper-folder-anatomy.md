# haipipe-paper/ref : paper folder anatomy

What a whole paper folder should look like. Every paper build skill creates,
migrates toward, or audits against this contract. The door's `conform` verb
(`fn/conform.md` + `scripts/check_structure.sh`) is its executable form: if the
two ever disagree, the script is the one that ran and this doc is the one that
is wrong.

The companion doc `tex-file-anatomy.md` (this folder) defines the anatomy of
...
The `conform` verb runs this as an actual test: block J resolves every
...
- The filename is the structural address. See
  `tex-file-anatomy.md` (this folder) for what goes inside each file role.
...
Mechanical version: `sh paper/haipipe-paper/scripts/check_structure.sh <paper-dir>`.
