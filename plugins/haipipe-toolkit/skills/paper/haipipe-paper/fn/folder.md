# Door verb: folder (paper-folder scaffold)
...
Division of labor: `/haipipe-paper enter <paper-path> [--org <owner>]` (the door's CONSOLE step; a missing path confirms then creates) handles repo creation + submodule wiring for papers inside Project-* repos (per the papers-inside recipe in `project/haipipe-project/fn/repo-project.md`), then runs THIS fn to fill the folder.
This fn never runs gh or git submodule commands.
...
A file that breaks the build when deleted has no business carrying a number. The door's
`conform` verb (`fn/conform.md`) runs this as an actual test; scaffold to it, and a fresh
folder passes on day 0.
...
IMRD (most)        Abstract, Introduction, Methods, Results, Discussion, Back-matter, SI
IS (MISQ/ISR)      Abstract, Introduction, Literature Review, Theory, Methods/Data,
                   Empirical Analysis, Discussion, Conclusion, Appendices
```
...
After any upgrade, run the door's `conform` verb (`fn/conform.md`) and report its verdict. An
upgrade that leaves the delete test failing is not done.
...
tex spine, venue-template stubs) is history, kept for reference only.
