...
def main():
    ...
    family = family.title()
    # Single-letter units (R, C, N) are upper by convention; word units (Seed,
    # Venue, Pitch) keep the case stage.md declares, matching S-Open-Seed.md.
    ...
    # Pages group heading: post-260803 board.md uses "### S01 · Delivery Opening",
    # so the SNN token from the lifecycle directory is the group key; the legacy
    # "S-<Family>" form stays as the fallback for pre-restructure boards.
    snn = directory.split("-", 1)[0]
    pages_group = snn if re.fullmatch(r"S\d{2}", snn) else f"S-{family}"
