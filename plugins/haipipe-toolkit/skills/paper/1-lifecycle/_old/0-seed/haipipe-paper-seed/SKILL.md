...
- [ ] Probe files verify clean: locate the checker layout-agnostically (installed skills flatten the tree, so the `../../../2-phase/...` relative path is NOT reliable) AND filter on the FAMILY -- TWO files named `check-probe-cards.sh` exist on disk (paper + application), and a bare `-name` find resolves to whichever the filesystem hands back first, silently asserting a paper against application invariants:
      ...
      `--stage seed` IS PART OF THE COMMAND, not a refinement of it. Without it the checker globs the WHOLE paper, and this stage's gate inherits every other stage's open work: on a mature paper the seed gate reports section-edit's placeholders as its own failures. A gate that reds on work its stage does not own is a gate people learn to read past -- and then it stops reporting the failures it DOES own. (Same ruling as resource's gate, `../../ref/08-stage-gate.md`, and the same C8-i carve-out PASS 1 already had.)
...
5. **Q-consumer is explicit, and generous.**
   ...
   The `1-probes/` probe files carry the question ENTRIES and their bindings, for the questions this stage dispatches (principle 5a).
...
DRAFT's Step 4b raises what the draft cannot answer. These are the kinds seed DISPATCHES — feasibility only, because that is what seed's own probe budget is for: "can this paper exist at all?"
...
**This list bounds DISPATCH, never RAISING** (principle 5a). A question of any other shape — profiling OUR OWN data (cohort size, field coverage, label availability), or any other prerequisite — is still RAISED: it keeps its `## Q-Seed-<n>` block, takes `Answer: deferred -> RESOURCE`, and carries a `[FORWARD -> RESOURCE]` pointer in `_LOG_0-seed.md`. What it does not get is an entry in `1-probes/`.

There is no cap on how many questions a seed raises. The count that matters is how many it DISPATCHES, and the DRAFT gate sets that — not this document.
