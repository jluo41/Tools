...
PERSIST that comment verbatim as a LaTeX comment `%% {USER}: <comment>`, and KEEP it
...
Format: `%% {USER}: <comment>` (aligning the user's stated `%{XX}: xxx` with the repo
...
append it as `%% {USER}: ...` near that display's entry in
