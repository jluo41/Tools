...
Fix: v3.0.0 — the tex is now GENERATED by sync from 4-display.md; sync always emits a
\section* banner at each section change + a named \subsection* per display before the
\input, ordered by the Display Map. Done-gate item in CHECKLIST.md (stage-doc section).
