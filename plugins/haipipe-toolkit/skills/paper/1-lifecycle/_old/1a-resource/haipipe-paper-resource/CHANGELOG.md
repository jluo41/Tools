...
## 2.4.0 — 2026-07-19 — questions this stage typically raises
...
### Changed (JL: "resource 我们现在有什么数据，有什么算法，要去网上找什么数据，找什么算法，是不是自己要开发算法，自己构造数据之类的。")

- The list is cut on JL's own axes: DATA × ALGORITHM, each asked three ways — what we HAVE, what to GET from outside, what we must BUILD ourselves — plus linkability cutting across both. The first draft of this section (existence / coverage / reusability) missed that "must we build it ourselves" is a resource question at all, and that the algorithm axis is co-equal with the data axis.
