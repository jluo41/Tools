...
DRAFT's Step 4b raises what the draft cannot answer. Resource asks about the INGREDIENTS, on two axes: DATA and ALGORITHM. For each, the same three-way question — do we HAVE it, can we GET it, or must we BUILD it?

```
                    📊 DATA                          🧠 ALGORITHM
                    ─────────────────────────        ─────────────────────────
🗄️ what we HAVE     Which store already holds it?     Which model/code already exists?
                    Name the path and the             Name the checkpoint or the repo,
                    producing pipeline, and which     and whether it has actually run
                    years/cohorts have actually       on anything like our data.
                    completed a run.

🌐 what to GET      Is there a public dataset that    Is there a published method or a
   (from outside)   covers this? Named, licensed,     released implementation we can
                    obtainable — not "surely          adopt instead of writing one?
                    someone has this".

🔨 what to BUILD    Must we construct it ourselves    Must we develop the algorithm
   (ourselves)      — derive, label, simulate,        ourselves? Then that is the
                    join? Then that is a task, and    paper's method contribution,
                    the question is what it costs.    not a resource — say so here.
```

And the one that cuts across both:

```
🔗 linkability      Can the two sides be JOINED at the unit of analysis? Name the key.
                    Data that cannot be linked to the outcome is not a resource.
```

NOT here: whether the evidence SUPPORTS a claim. Resource settles what we have, what we can get, and what we must build; claims settles what it shows.
