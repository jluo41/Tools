# Paper Architecture Overview (reference)

> Merged into `haipipe-paper-minimap` (2026-06-22) from the retired
> architecture skill. This is the former README, kept as reference material for
> the minimap stage. The capability is now reached through
> `/haipipe-paper-minimap`; the invocation examples below have been repointed
> accordingly. Not a standalone skill.
...
```bash
/haipipe-paper-minimap @paper-structure.md
```
...
```bash
/haipipe-paper-minimap @paper-structure.md @slides-with-results.pdf
...
```
/haipipe-paper-minimap @abstract.md
...
```
/haipipe-paper-minimap @paper-draft.md
```
...
This content now lives under `haipipe-paper-minimap/ref/` (was: the retired
architecture skill folder):

```
haipipe-paper-minimap/ref/
├── architecture-readme.md              ← You are here
├── architecture-blueprint.md           ← the blueprint generator body
...
**Workflow:** `/paper-structure-planning` → `/haipipe-paper-minimap` → Write paper
