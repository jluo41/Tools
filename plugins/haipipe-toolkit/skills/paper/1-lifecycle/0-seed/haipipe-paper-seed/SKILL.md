...
description: "Create or update the paper folder's 0-lifecycle/0-seed/0-seed.md + _LOG_0-seed.md: the venue-FREE earliest stage contract that keeps a paper possibility alive before evidence is mature. States why the paper might exist, current evidence status, open evidence needs (routed to probe/discover/task/insight), a promotion gate, and kill criteria. Venue-free: does not change when retargeting to a different journal. Markdown only (no .tex; argument documents don't need compilation). Use for paper seed, prospectus, why this paper, project seed, kill criteria, 0-seed."
...
Maintain the **seed** stage of a concrete paper folder. This is the first stage of the lifecycle spine (stage 0, venue-FREE) and the contract at `prospectus` maturity.
...
The seed is not a pitch, claim ledger, or outline. It keeps a paper-shaped possibility alive while the evidence is still forming, and it states the conditions under which the paper is promoted or dropped.
...
- `0-lifecycle/0-seed/_LOG_0-seed.md` -- changelog
...
- Current Evidence Status -- task/probe/discovery/insight state
- Open Evidence Needs -- what to get next, each with a route (probe/discover/task/insight)
- Promotion Gate -- concrete conditions for promoting to an active paper seed
- Kill Criteria -- what evidence would make this paper not worth pursuing

**Done-criteria:**
- [ ] Promotion gate criteria answered (enough evidence to advance to claims)
- [ ] Kill criteria evaluated and paper still viable
- [ ] Every open evidence need has a route (probe/discover/task/insight)
- [ ] _LOG entry records the current state

**DGPC applicability:**
- DRAFT: write the seed contract
- GATHER: identify evidence needs, emit probe plans
- POLISH: refine prose clarity
- CHECK: evaluate promotion gate and kill criteria
...
The file is TeX-first and should compile on its own (the standalone preamble comes from the scaffold). This skill owns its body.

Principles
----------

1. A seed may be intuition. It does not require evidence yet.
2. The seed must name what evidence would make the paper worth pursuing, and what would kill it.
3. Open evidence needs route out to project workers; the seed never fakes them.
4. Do not create `0-sections/`, displays, or compile obligations from the seed. Those start at manuscript maturity.
5. **Seed is venue-FREE.** Venue selection happens after claims (seed → claims → [venue] → pitch). Do not reference a target venue here.
...
Accept the paper root or any path inside it. Find the root by looking upward for `0-lifecycle/`, a `0-*.tex` master, or `1-compile.sh`. If none exists, ask the user to run `/haipipe-paper-lifecycle folder <paper-root>`.

### Step 2: Ensure `0-lifecycle/0-seed/0-seed.tex` exists

Copy the canonical template and fill it. The template is the single source of truth for section order and the standalone preamble:
...
Do not add a "Parent Project" section; the parent project is recorded in `STATUS.md`, not the seed.

### Step 3: Route open needs

For each open evidence need, emit a delivery need using `../../ref/delivery-need.md` and suggest the route:
...
4. Ask: "Stage seed looks ready -- confirm to close and move to claims?"
