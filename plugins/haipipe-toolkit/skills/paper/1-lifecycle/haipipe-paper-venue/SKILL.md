...
1. **Build the paper's contribution profile.** From the seed/pitch/claims (or the
...
2. **Read the candidate packs.** For each `../../_venue/playbook-<venue>/README.md`,
...
3. **Score each venue** on five dimensions, each High/Med/Low with a one-line reason:
...
4. **Rank and shortlist** the top 3. For each: a fit rationale, what to emphasize for
   that venue, and the main why-not / risk.
5. **Recommend ONE primary** + 1-2 backups. The primary is the one whose rewards the
...
1. Build the profile from the text (no seed/pitch to read): contribution = a clinical
   prescribing-safety association; method = the LLM trait measure (an enabler);
   design = observational; audience = clinical / policy.
2-5. Score the packs, shortlist, recommend (here: `playbook-jama-portfolio` ->
   JAMA Internal Medicine primary, `playbook-clinical-medicine` backup).
6. `--no-pin`, so report only and stop; offer to scaffold a paper folder and pin if
