...
C5  DPRC loop. DRAFT reads the template, follows the RULE comments, creates the doc, raises `## Q-<Prefix>-<n>` + cites `[Q-<Prefix>-<n>]` inline. PROBE fills `Answer` (evidence, stops there). REVISE weaves the answer into every citing sentence AND discharges the bracket (prose). Born from content (DRAFT drops the bracket), dies into content (REVISE discharges it).
