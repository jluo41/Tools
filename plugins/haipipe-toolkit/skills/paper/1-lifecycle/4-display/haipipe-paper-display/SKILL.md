...
Maintain the **display layer** for a concrete paper folder (stage 4, venue-ALIGNED).
...
- `0-lifecycle/4-display/_LOG_4-display.md` -- changelog
...
- `%% {JL}: ...` author comments on displays (verbatim, kept across iterations)
...
- [ ] JL comments in 4-display.tex addressed or acknowledged
- [ ] CHECKLIST.md walked and all items pass
- [ ] No orphan displays (every unit has a claim, source, section)

**DGPC applicability:**
- DRAFT: plan the display set (display index in 0-displays/README.md)
- GATHER: route to tasks for generation (/haipipe-task-for-display)
- POLISH: visual quality, captions, label consistency
- CHECK: all rendered + linked, gallery compiles, CHECKLIST.md passes

New here? Read `../README-display.md` first: how a figure/table gets made, which renderer to pick (table/figure/diagram/illustration), the unit contract, and the hard-won lessons (settle the model upstream; controlled AI illustration).
...
**Illuminate.** Before creating/refreshing the display index, follow `ref/stage-illuminate.md`. Present the current display set and ask: which display is the hero figure? Which are main vs supplement? What visual forms (forest plot, panel, flow diagram, table)? Elicit taste before proceeding.

Inputs to read when present:

- `0-lifecycle/2-pitch/2-pitch.tex`
- `NARRATIVE_REPORT.md`
...
Reference hook: Load `Tools/references/aris/skills/paper-framework-figure-studio-pro/SKILL.md` and follow its candidate-generation loop when generating alternatives and revision passes. It is useful when `figure1` requires several concept drafts before settle.
...
Turn a `planned` / `data-ready` display into a real RENDERED asset by actively calling the evidence and render workers. This stage never hand-authors a figure or pastes numbers into `float.tex`.
...
Compile standalone preview PDFs for one display item or all display items with `preview.tex`.
