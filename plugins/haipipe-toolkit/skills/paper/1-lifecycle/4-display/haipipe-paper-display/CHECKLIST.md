...
- [ ] `4-display.md` present and is the ONLY hand-edited stage file: Venue Set + gallery config, Display Map (row order = narrative order = gallery order), one block per display, Parking section.
...
- [ ] Method candidates recorded: 2–3 rows for non-trivial displays (different forms/routes, incl. diagram/illustration where apt); every row PROBE-filled (output + one-line self-assessment) or explicitly struck.
