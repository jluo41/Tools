...
| `0-seed` | `seed` | Why might this paper exist? | State possibility: question, motivations, claim shape | seed notes, project evidence | `0-lifecycle/0-seed/0-seed.md` + `_LOG`, STATUS.md | none | seed contract | `0-seed.md` | not viable -> drop | venue: FREE |
| `1-resource` | `resource` | What must EXIST for this paper to be testable, does it exist, can it CARRY the claim? | Derive Demand (one `N<n>` per `H<n>`), ASK the Questions (`Q<n>`); the PROBE phase opens an entry per Q and routes it | seed (Tentative Claim Shape), `_LOG_0-seed.md` forward pointers | `0-lifecycle/1a-resource/1a-resource.md` + `_LOG`, STATUS.md | the PROBE phase ONLY (never executes; mints no PP ids) | Demand + Questions (with their **A**) | `1a-resource.md` | GATE 1 (approve the questions + the SPEND), GATE 2 (exit) | venue: FREE |
| `1-claims` | `claims` | What must be true? What evidence do we have? | Maintain claim ledger (the ONLY home of a claim's status), mark needs | seed, `1a-resource.md`, the answering QA files a probe section's `target:` names | `0-lifecycle/1b-claims/1b-claims.md` + `_LOG`, STATUS.md | `/haipipe-probe`, `/haipipe-discovery`, `/haipipe-task` | claim ledger | `1b-claims.md` | claim unsupported/too strong, no route | venue: FREE |
...
| `5-section-edit` | `section-edit` | How is each section written for THIS venue? | Per-section DRAFT -> PROBE -> REVISE -> CHECK | 3-narrative, display units, `venue/playbook-<venue>` | `0-lifecycle/5-section-edit/<section>/` (outline `.md`, `_LOG`), `0-sections/*.tex` | compile / overleaf | section outlines + draft PDF | section scaffolds, section files | writing exposes missing evidence -> 1-claims | venue: SPECIFIC |
