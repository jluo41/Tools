...
| `1-resource` | `haipipe-paper-resource` | What must EXIST for this paper to be testable, does it exist, can it CARRY the claim? | Derive Demand (one `N<n>` per `H<n>`), ASK the Questions (`Q<n>`); the PROBE phase opens a section per Q and routes it | seed (Tentative Claim Shape), `_LOG_0-seed.md` forward pointers | `0-lifecycle/1-resource/1-resource.md` + `_LOG`, STATUS.md | the PROBE phase ONLY (never executes; mints no PP ids) | Demand + Questions (with their **A**) | `1-resource.md` | GATE 1 (approve the questions + the SPEND), GATE 2 (exit) | venue: FREE |
| `1-claims` | `haipipe-paper-claims` | What must be true? What evidence do we have? | Maintain claim ledger (the ONLY home of a claim's status), mark needs | seed, `1-resource.md`, the answering QA files a probe section's `target:` names | `0-lifecycle/1-claims/1-claims.md` + `_LOG` + `_EVIDENCE_`, STATUS.md | `/haipipe-probe`, `/haipipe-discovery`, `/haipipe-task` | claim ledger | `1-claims.md` | claim unsupported/too strong, no route | venue: FREE |
