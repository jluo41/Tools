...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from paper-weaving to haipipe-paper-edit-weaving; consolidated into 3-write-edit/ (haipipe-paper-* name unification)."
    - "1.0.0 (2026-05-31): baseline metadata added."
---

Skill: haipipe-paper-edit-weaving (orchestrator)
====================================
...
- **Q: auto quality check after every PROPOSE action is APPLIED.**
  haipipe-paper-edit-weaving fans out via `Skill()` to a configurable list of sibling
  check skills (`paper-check-numeric`, `haipipe-paper-edit-check-reference`,
...
```
/haipipe-paper-edit-weaving                                -> dispatch help (lists specialists)
/haipipe-paper-edit-weaving <path>.tex                     -> default: section-level diagnose, embed in .tex
/haipipe-paper-edit-weaving section <path>.tex             -> same as above (explicit granularity)
/haipipe-paper-edit-weaving paragraph <path>.tex P<n>      -> dispatch to paper-revise-paragraph
/haipipe-paper-edit-weaving sentence <path>.tex P<n>.S<m>  -> dispatch to paper-revise-sentence
/haipipe-paper-edit-weaving check numeric <path>.tex       -> dispatch to paper-check-numeric
/haipipe-paper-edit-weaving check reference <path>.tex     -> dispatch to haipipe-paper-edit-check-reference
/haipipe-paper-edit-weaving write <path>.tex               -> dispatch to paper-write
/haipipe-paper-edit-weaving check <path>.tex               -> Gate Q on demand (runs all configured checks)
/haipipe-paper-edit-weaving cleanup <path>.tex             -> Gate G2: prompt author to delete plan blocks
/haipipe-paper-edit-weaving "<natural language>"           -> infer granularity + verb, route accordingly
```
...
```
haipipe-paper-edit-weaving (this)         section-level diagnose + plan embedded in .tex  (DEFAULT route)
...
haipipe-paper-edit-check-reference        verify every citation against bib + venue metadata
paper-write                  draft a new section from PAPER_PLAN.md outline
```

`/paper-revise` is a sibling, not a child. haipipe-paper-edit-weaving routes to it
...
```
section, this section, revise this section, weave, /haipipe-paper-edit-weaving                -> section
...
write, draft, from outline, /haipipe-paper-edit-write                                         -> write
```
...
Step 4. If verb suggests a specific check (not generic Q), override granularity:
  ...
  - verb = check-reference                      -> dispatch to haipipe-paper-edit-check-reference only
...
```
%%@ ═══════════════════════════════════════════════════════════════════════════════
%%@ 📜 <Section name>  v<tag>  ·  Logic + Proposed Edits   [haipipe-paper-edit-weaving plan]
%%@ ═══════════════════════════════════════════════════════════════════════════════
%%@
...
1. Automatically, when every PROPOSE action in every per-paragraph block
   ...
   `[APPLIED v<tag>]`.
2. On demand, via `/haipipe-paper-edit-weaving check <path>.tex` (e.g., to sanity-check
...
haipipe-paper-edit-weaving without dispatching to a sibling skill:
...
Skill("haipipe-paper-edit-check-reference", args="<path>.tex")
```

Optional checks (run only if explicitly enabled in `haipipe-paper-edit-weaving.yml`
or requested by the author):

```
Skill("haipipe-paper-edit-claim-audit",     args="<path>.tex")   # heavy: claims-vs-evidence
Skill("haipipe-paper-edit-manual-review-values", args="<path>.tex")  # heavy: every number from raw files
...
issue list). haipipe-paper-edit-weaving captures the structured tail of each report.
...
Skill checks (Substep Q2):
  ...
  haipipe-paper-edit-check-reference:  🟡 FAIL  (2 unverified refs: doe2024foo, smith2025bar)
...
#### Configuration: `haipipe-paper-edit-weaving.yml`
...
haipipe-paper-edit-weaving:
  gate-q:
    ...
    skills:
      - paper-check-numeric
      - haipipe-paper-edit-check-reference
      # - haipipe-paper-edit-claim-audit          # uncomment for pre-submission rounds
      # - haipipe-paper-edit-manual-review-values
    # Skip Gate Q entirely on micro-rounds (one-sentence apply passes)
    skip-on-micro-round: true
```

If the config file is malformed, haipipe-paper-edit-weaving prints a one-line warning
...
**Triggered**: invoked via `/haipipe-paper-edit-weaving cleanup <path>.tex`, OR
...
🧹 Gate G2 — Cleanup is your call.
   ...
   (2) Ask me to delete them — say "/haipipe-paper-edit-weaving cleanup <path>.tex confirm-delete"
...
|---|---|
| diagnose one section + plan the revision, embedded in .tex | **haipipe-paper-edit-weaving** (this, default route) |
| rework a paragraph, iterate sentences inside it | `Skill(paper-revise-paragraph)` via haipipe-paper-edit-weaving |
| rewrite a single sentence | `Skill(paper-revise-sentence)` via haipipe-paper-edit-weaving |
| verify every quantitative claim against raw results | `Skill(paper-check-numeric)` via haipipe-paper-edit-weaving |
| verify every citation against bib + venue metadata | `Skill(haipipe-paper-edit-check-reference)` via haipipe-paper-edit-weaving |
| run all quality checks on demand (without an apply round) | `/haipipe-paper-edit-weaving check <path>.tex` (Gate Q) |
| clean up the embedded plan blocks before submission | `/haipipe-paper-edit-weaving cleanup <path>.tex` (Gate G2) |
...
| build a TikZ figure for the published paper | `/haipipe-paper-structure-figure-spec` or `/diagram-drawio` |
