---
name: haipipe-paper-revise-weaving
description: "REVISE phase worker for paragraph flow and transitions. Orchestrates revision on a single LaTeX file: diagnoses paragraph arc, embeds revision plan as %% comments, applies flow fixes directly. Fully automatic (no human gate). Leaves %% {CC-weaving}: comments explaining structural changes for CHECK review. Use for /haipipe-paper-revise-weaving or requests to weave, revise, diagnose, or rework paragraph flow."
...
Skill: haipipe-paper-revise-weaving (orchestrator)
====================================
...
REVISE is fully automatic. The agent applies flow fixes directly and leaves `%% {CC-weaving}: <why>` comments explaining structural changes for CHECK to review. No human gate during REVISE. The agent:

1. Diagnoses the paragraph arc and embeds a revision plan as `%%` comments
2. Applies flow fixes directly (transitions, paragraph openings, rhythm)
3. Runs an inline quality scan (em-dash, AI-voice, sentence-length, paragraph ceiling)
4. Leaves explanatory comments for non-trivial structural changes

Human review of all changes happens in CHECK, where the human can add `> USER:` comments and restart REVISE if needed.
...
```
section, this section, revise this section, weave, /haipipe-paper-revise-weaving                -> section
...
write, draft, from outline, /haipipe-paper-draft                                         -> write
```
...
```
%%@ ═══════════════════════════════════════════════════════════════════════════════
%%@ 📜 <Section name>  v<tag>  ·  Logic + Proposed Edits   [haipipe-paper-revise-weaving plan]
%%@ ═══════════════════════════════════════════════════════════════════════════════
%%@
...
%%@ ═══════════════════════════════════════════════════════════════════════════════
%%@
%%@   Status: <one-line — which user asks were APPLIED / MID-ITERATION / DEFERRED>
%%@
%%@   Context: <short paragraph — what the author flagged, what they asked>
%%@
%%@   Author asks (verbatim):
%%@     {USER} v<tag>: <verbatim Chinese or English, no translation/compression>
...
2. On demand, via `/haipipe-paper-revise-weaving check <path>.tex` (e.g., to sanity-check the section before submission even if no round of edits just finished).

**Skill behavior** — three substeps, run in this order:

#### Substep Q1. Inline cheap scan (no Skill() calls)

Grep the .tex prose region (everything not on a `%%` line) for cheap quality regressions. These are O(file-size) scans, run inline by haipipe-paper-revise-weaving without dispatching to a sibling skill:
...
Optional checks (run only if explicitly enabled in `haipipe-paper-revise-weaving.yml`
or requested by the author):

```
Skill("haipipe-paper-edit-claim-audit",     args="<path>.tex")   # heavy: claims-vs-evidence
Skill("haipipe-paper-probe-values", args="<path>.tex")  # heavy: every number from raw files
Skill("citation-verifier",     args="<path>.tex")   # heavier than -reference
```

The fan-out runs in a single message so the checks execute concurrently. Each check skill is expected to return a structured report (PASS / FAIL + issue list). haipipe-paper-revise-weaving captures the structured tail of each report.
...
If the config file is malformed, haipipe-paper-revise-weaving prints a one-line warning and falls back to defaults.
...
**Triggered**: invoked via `/haipipe-paper-revise-weaving cleanup <path>.tex`, OR automatically immediately after Gate Q presents its summary (whether PASS or 🟡 ATTENTION — the author still owns the cleanup call). Gate G2 NEVER fires directly out of the apply phase; Gate Q always runs first.
...
🧹 Gate G2 — Cleanup is your call.
   ...
   (2) Ask me to delete them — say "/haipipe-paper-revise-weaving cleanup <path>.tex confirm-delete"
...
If the workspace still has `.txt` plan files from the prior format (e.g. `1-rounds/v0516/01_introduction_logic.txt`), do the following on the first run of `haipipe-paper-revise-weaving` against the matching `.tex`:
