...
description: >-
  ...
  and cleanup gates. Use for /haipipe-paper-section-edit-weaving or requests to weave,
...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from paper-weaving to haipipe-paper-section-edit-weaving; consolidated into 3-write-edit/ (haipipe-paper-* name unification)."
...
- **Q: auto quality check after every PROPOSE action is APPLIED.**
  haipipe-paper-section-edit-weaving fans out via `Skill()` to a configurable list of sibling
  check skills (`paper-check-numeric`, `haipipe-paper-section-edit-citation`,
  optionally `haipipe-paper-edit-claim-audit`) AND runs an inline cheap scan
...
`/paper-revise` is a sibling, not a child. haipipe-paper-section-edit-weaving routes to it
...
1. Automatically, when every PROPOSE action in every per-paragraph block
   ...
   `[APPLIED v<tag>]`.
2. On demand, via `/haipipe-paper-section-edit-weaving check <path>.tex` (e.g., to sanity-check
...
haipipe-paper-section-edit-weaving without dispatching to a sibling skill:
...
issue list). haipipe-paper-section-edit-weaving captures the structured tail of each report.
...
If the config file is malformed, haipipe-paper-section-edit-weaving prints a one-line warning
...
**Triggered**: invoked via `/haipipe-paper-section-edit-weaving cleanup <path>.tex`, OR
...
on the first run of `haipipe-paper-section-edit-weaving` against the matching `.tex`:
