---
name: haipipe-paper-section-edit-write
...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from paper-write to haipipe-paper-section-edit-write (haipipe-paper-* name unification)."
...
6. **Venue style pack**: `../../_venue/playbook-<pack>/style-profile.md` (general venue norms) + per-section style guide at `../../_venue/playbook-<pack>/<outlet>/<outlet>-<section>/style.md` (section-specific word budget, arc, signature moves, exemplar sentences, anti-patterns). The per-section file OVERRIDES general norms for this specific section. Resolution: STATUS.md `venue:` → outlet + pack (e.g., "MISQ 2026" → `playbook-utd-is/MISQ/MISQ-theory/style.md`). See `_venue/_SCHEMA.md` for the full mapping. The `TARGET_VENUE` constant above stays for formatting mechanics (style file, page limit, anonymity); the pack supplies the content and language style.
