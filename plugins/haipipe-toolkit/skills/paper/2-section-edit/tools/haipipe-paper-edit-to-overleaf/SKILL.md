---
name: haipipe-paper-edit-to-overleaf
...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from overleaf-sync to haipipe-paper-edit-to-overleaf; moved from components/compile/ into 3-write-edit/ (haipipe-paper-* name unification)."
...
- If the user is in an active Overleaf editing session, ARIS skills should **read-only** access `paper/` until the user runs `/haipipe-paper-edit-to-overleaf pull`.
- If ARIS is in the middle of `/haipipe-paper-edit-improve-loop` or `/haipipe-paper-edit-write`, the user should pause Overleaf editing until the loop finishes and `/haipipe-paper-edit-to-overleaf push` is run.
