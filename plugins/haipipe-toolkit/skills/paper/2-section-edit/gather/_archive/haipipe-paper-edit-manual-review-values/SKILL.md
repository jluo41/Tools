---
name: haipipe-paper-edit-manual-review-values
...
metadata:
  ...
  changelog:
    - "1.1.0 (2026-06-05): renamed from paper-manual-review-values to haipipe-paper-edit-manual-review-values; consolidated into 3-write-edit/ (haipipe-paper-* name unification)."
...
`/haipipe-paper-edit-claim-audit` runs a one-shot autonomous review and writes a JSON report. That is fast and good enough for routine checks — but it is not what you want before a top-tier submission.
...
| `/haipipe-paper-edit-claim-audit` | autonomous, one-shot | fresh GPT-5.4 | JSON+MD | routine pre-submission check |
| **`/haipipe-paper-edit-manual-review-values`** | **interactive, human-paced** | **same agent + human at every gate** | **edits applied + change log** | **high-stakes final submission** |
...
1. **After `/haipipe-paper-edit-claim-audit`** — the autonomous audit catches obvious issues fast; this skill catches the subtle ones.
...
- During early drafting (numbers churn too fast; use `/haipipe-paper-edit-claim-audit` for periodic checks instead)
