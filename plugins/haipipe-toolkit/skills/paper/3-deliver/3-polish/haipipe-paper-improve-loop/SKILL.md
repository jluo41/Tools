---
name: haipipe-paper-improve-loop
...
This skill is designed to run **after** the write workflow (`/haipipe-paper-display-figure` → `/haipipe-paper section-edit` → `/haipipe-paper-compile`).
...
> 💡 Override: `/haipipe-paper-improve-loop "paper/" — human checkpoint: true`
...
| Writing clutter / passive voice | Apply sciwrite 5-pass audit: clutter extraction → active voice → sentence architecture → keyword consistency → numerical integrity. See `2-phase/REF/prose-quality.md` |
| Number mismatch (paper vs results) | Run `/haipipe-paper-claim-audit` if PAPER_CLAIM_AUDIT.md doesn't exist; fix any `number_mismatch` or `aggregation_mismatch` claims |
