---
name: haipipe-paper-format
description: "Bring an existing LaTeX draft into line with venue formatting and style conventions. CHECK-phase whole-paper format pass in deliver. Self-contained format checks. STUB — scope defined, checklist to be filled. Trigger: format pass, venue style, fix formatting, style conventions."
metadata:
  version: "0.0.1"
  status: stub
  stage: deliver
...
# haipipe-paper-format  (stub)

CHECK-phase whole-paper format pass in the deliver layer.
...
Like every deliver sub-skill it is **comment-first**: Round 1 inserts `%% {CC-format-vMMDD}: finding | suggestion ========>` and changes no text; apply waits for the human `========> {XX}:` reply.
