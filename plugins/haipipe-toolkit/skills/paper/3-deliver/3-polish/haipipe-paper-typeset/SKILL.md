---
name: haipipe-paper-typeset
description: "Fix typesetting defects in a compiled LaTeX draft: widows, orphans, overfull boxes, bad breaks. CHECK-phase whole-paper typeset pass in deliver. Self-contained typeset checks driven by the compiled PDF. STUB — scope defined, checklist to be filled. Trigger: widow orphan, overfull box, fix line breaks, typeset pass, page breaks."
metadata:
  version: "0.0.1"
  status: stub
  stage: deliver
...
# haipipe-paper-typeset  (stub)

CHECK-phase whole-paper typeset pass in the deliver layer — the **last** pass, because every earlier edit moves the type.
Self-contained: carries its own check logic, driven by the compiled PDF and the LaTeX log.

Like every deliver sub-skill it is **comment-first**: Round 1 inserts `%% {CC-typeset-vMMDD}: finding | suggestion ========>` and changes no text; apply waits for the human `========> {XX}:` reply.
...
Compile with `haipipe-paper-compile` (sibling skill in `3-deliver/`) and read the PDF + log (`Overfull` / `Underfull` warnings, widow/orphan) — fix at the **prose** level first (tighten a sentence to pull a widow back) before reaching for manual break commands.
A diff of before/after PDFs (`haipipe-paper-diffpdf`) confirms nothing else moved.
