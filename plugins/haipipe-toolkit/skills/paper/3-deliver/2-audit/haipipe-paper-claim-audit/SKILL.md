---
name: haipipe-paper-claim-audit
...
| **`/haipipe-paper-claim-audit`** | **Does the paper report the data truthfully and precisely?** |
...
The full auditor prompt -- the audit protocol (A extract every quantitative claim -> B trace each to evidence -> C the 7 failure-mode checks), the per-claim output schema, and the PASS/WARN/FAIL verdict -- lives in `ref/auditor-prompt.md`. Dispatch it verbatim as a fresh `mcp__codex__codex` thread (model `gpt-5.4`, `model_reasoning_effort: xhigh`), filling in the listed paper `.tex` paths and raw result-file paths.
...
```json
{
  "audit_skill":      "haipipe-paper-claim-audit",
  ...
  "trace_path":       ".aris/traces/haipipe-paper-claim-audit/<date>_run<NN>/",
