...
Re-house an existing paper in the layout defined by `2-phase/REF/paper-folder-anatomy.md` **without changing a single sentence**.
Two modes:
...
Not this skill: building a folder from nothing (`haipipe-paper-scaffold`), or any wording change (`1-lifecycle/5-section-edit`).

Usage
-----

```
/haipipe-paper-restructure paper/                      migrate to gold layout
...
figs/pipeline.pdf                       →  0-displays/displayNN-pipeline/assets/pipeline.pdf  (+ rewrite path)
...
5. Install `1-compile.sh` from `../haipipe-paper-scaffold/templates/compile.sh.tpl` if no conforming build script exists; `chmod +x`.
...
3. `../haipipe-paper-conform/scripts/check_structure.sh .` → exit 0.
...
next:      /haipipe-paper-conform (re-audit) or /haipipe-paper section-edit (resume prose work)
```
