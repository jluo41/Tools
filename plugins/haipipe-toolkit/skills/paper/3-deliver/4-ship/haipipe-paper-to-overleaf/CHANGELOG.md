...
Reference update only; no behavior change here. The `REVIEW` verb of the retired `haipipe-paper-probe-citation` — the slow, human-paced pre-submission walk over every citation — moved to `haipipe-paper-check-evidence` in `2-phase/3-check/`, dispatched conditionally by the CHECK gate the way the proof-checker is. This file's pointers now name it. Background: `../../../_console/260719-PHASE-BOUNDARY-REFACTOR.md`, ruling D11 (JL confirmed the NARROW reading: verification belongs to the CHECK PHASE, not to a probe lane, and not — the alternative reading — pushed across the wall to the executor).
