...
```
  ...
  PROBE    go and get the evidence: name the producing run, land its data     ≈ ②
...
- A display page's CHECK is a HUMAN GATE. `haipipe-page-orchestrator-agent` may run DRAFT, PROBE and REVISE for a display page and must STOP at CHECK, leaving the page at ③ with the reason recorded.
