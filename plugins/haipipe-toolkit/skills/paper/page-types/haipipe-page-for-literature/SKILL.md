...
The stake stays on this page. The QA-probe's Q-executor carries the neutral question only, and the wall between them is PROBE's (`page-phases/haipipe-page-probe`).
...
After PROBE returns, the finding has two distinct products: a citation binding that protects the paper's positioning, and a candidate display that makes the positioning visible.
