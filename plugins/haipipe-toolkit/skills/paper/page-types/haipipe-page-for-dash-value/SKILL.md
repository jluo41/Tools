...
The evidence for the merge was already on the real pages: `S-Work-R1-cms` points at `tasks/A11_CMS-pipeline/A01_cms_pipeline/` and sits at `🔴 PROBE pending`, with no `route:` line and no `E` divisions. It was an inward evidence page that never adopted the inward contract.
