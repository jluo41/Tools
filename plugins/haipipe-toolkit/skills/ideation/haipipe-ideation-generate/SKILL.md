...
4. **Write the question first, then the minimum scientific object.** For
   every provisional candidate, write the research question it asks: one
   sentence ending in `?`, naming one object, one relation or contrast, one
   outcome, and the boundary it is asked inside, informative even when the
   answer is no (sharpen a fuzzy interest with the vendored
   `framing-research-questions` reference). That question is the card's
   `question` and the Idea's name everywhere downstream; the `title` is a
   2–5 word handle derived from it, for slugs. Then state one falsifiable
   claim, mechanism, method in 2–4 steps, minimum informative experiment,
   expected signal, failure interpretation, and 1–5 Core Claims. A candidate
   with a title and a claim but no question is not admissible (JL 260922:
   "write the idea in terms of the research question").
