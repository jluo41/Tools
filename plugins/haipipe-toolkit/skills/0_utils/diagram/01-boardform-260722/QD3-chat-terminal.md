...
The hard part is that the terminal must run on the machine the files are on and squeeze through Remote-SSH's single forwarded port, and several boards and questions open at once must not fight over ports. The drawer is ultimately a re-built chat box, so any job needing commands or skills gets stuck; without a real terminal the board can only do "edit some text" work. It also reframes the split with `QD2`, which stops being "safe vs. unsafe" (QD2 can now open up fully too) and becomes a **difference of form**: the drawer is a rebuilt chat box, the terminal is the CLI verbatim.
