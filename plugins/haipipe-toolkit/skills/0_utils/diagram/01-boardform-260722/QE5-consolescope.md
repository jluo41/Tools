...
- Why it is hard
  The console's whole rail is scope-filtered (DATA/INSIGHT/ACTION views take the patient or the cohort as their subject). Boards take the **workroom** as their subject. Park them inside a scope and the placement lies; invent a third scope and the rail's paradigm needs a seam.
- What breaks if we leave it
  v1 parked Boards inside both scopes' Action group — findable, but it implies boards are patient-or-cohort work. The first RA who opens `/individual` → Boards and sees skill-design boards will be confused exactly the way `QA5` warns about.
