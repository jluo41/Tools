...
## Question
打开一个空的 `QA5-xxx.md`，我该往里面填什么？哪些段落必须有，哪些可以不写？
...
│ ## Question                    │ 必填 → ❓ 最显眼那行
│ ## Now                         │ 必填 → 上面黄框
│ ## Done when  - [ ] 清单        │ 必填 → 下面绿框（自动数 2/5）
│ ## Why here                    │ 必填 → 再下一栏
│ ## Diagram    ascii 图         │ 选填 → 问句下面
├────────────────────────────────┤ ↓ 全折叠，不上台面
│ ## Law    ## Lesson            │ 选填 → 规矩 / 踩坑
...
## Now
模板落地并验过 —— `ref/q-template.md` 就是加一题时直接复制的那份，四条完成线全部达到，本题关掉。

- 模板长什么样
  顶部 `# 标题 / state / owner / method`，加 11 个 `##` 段。每段正文第一行是引导句，开头标 `必填 ·` 或 `选填 ·`，填的时候覆盖掉。顶部一段 `<!-- 用法 -->` 注释讲怎么用，生成时被丢掉、不上页面。
- 必填 / 选填
  必填七样：`# 标题`、`state`、`owner`、`## Question`、`## Now`、`## Done when`、`## Why here`。其余（`method`、`## Diagram` 和全部折叠段）选填，用不上就整段连标题一起删。
...
- 必填七样
  `# 标题`、`state`、`owner`、`## Question`、`## Now`、`## Done when`、`## Why here`。其余全选填，用不上就整段删掉。
- 折叠段顺序由 build.py 固定
  页面上永远是 Discussion · Comments · Law · Lesson · Glossary · Log。文件里怎么排都行。
- Log 倒序
