...
The generator only recognizes a fixed set of section names, so misspell one and that section **silently disappears, no error**; silent failure is the hardest kind to debug. Without a clear template, everyone writes a different-shaped Q and the page turns messy, and the RA and future agents touch this file every day. The template decides what the page can display at all, and it is the flip side of `QA4`: when `QA4` changes the on-stage order, this template must follow, or newly written questions drift back to the old shape.
