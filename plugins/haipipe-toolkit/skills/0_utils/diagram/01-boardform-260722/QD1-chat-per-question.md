...
## Question
板上每个 Q 挂一个自己的对话，这件事本身要先说清楚：板级和题级各管什么？题级能读能写到什么程度？对话本身去哪？
...
## Why here
这一题跟前面所有 QA 不同：**前面在定「板长什么样」，这一题在问「板上能不能直接干活」。**
所以单开一个 QD 组。

它也是这块板到目前为止唯一一个会改变工作方式的问题。原来的循环是
「你在页面上评论 → 我在这个对话里读 → 我改 md」，中间隔着一个人转述。
题级 chat 成立之后，循环变成「你在那一题上评论 → 就在那一题上对话 → 改完」，转述这一步没了。
