...
- [ ] An in-page edit writes its own `## Log` line
      `## Log` is hand-written today. If the page can change body text without leaving a trace, the board starts lying.
- [ ] Actually have two people edit one question and verify nothing is lost
      That is the acceptance test. Not "a lock was added", but someone really collided and saw the warning.
...
## Glossary
CRDT: a data structure that lets several people change the same content at once without locking, merging edits automatically. Yjs is the common implementation.
