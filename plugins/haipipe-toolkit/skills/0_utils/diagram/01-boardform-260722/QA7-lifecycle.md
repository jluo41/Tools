...
`QA6` built the feature, but "comments pile up forever", "broken quotes that nobody repairs", and "can a board close with open comments": the feature itself answers none of these. With open comments sitting in `## Comments`, nobody can say whether the board counts as closable, so the close condition is hollow. It rewrites the closing rule (`close:`), and decides who owns the repair after an anchor is flagged.
