...
## Question
在网页里直接开一个对话：它读这一题的内容和未解决评论，改这一题的 md。权限该开多大？

- 为什么难
  开小了干不了活，开大了在浏览器里就能随便改仓库。JL 定的方向是「跟 CLI 一致」—— 该问就问，而不是硬编码一条白名单。
- 不定会怎样
  抽屉是最常用的入口（比开终端轻）。权限档次不定，要么不敢用，要么不敢给别人用。
- 定了会影响什么
  它跟 `QD3` 是同一需求的两种形态，取舍完全不同，必须各定各的，不能互相将就。

## Boundary
- ✅ 这题管
  **网页抽屉这一种实现**：三档权限、流式输出、markdown 渲染、成本、以及它怎么遵守 `QD1` 那条 LAW。
- ❌ 这题不管
  规则本身（分几级、边界在哪）—— 那是 `QD1`。也不管真终端 —— 那是 `QD3`。
...
## Files
- `serve.py`
  `chat()` —— `ClaudeSDKClient` + `can_use_tool` 权限回调 + NDJSON 流式，全在这儿。
- `build.py`
  抽屉的 UI（`#chat` / `#chatfab`）和 markdown 渲染在页面脚本里。
