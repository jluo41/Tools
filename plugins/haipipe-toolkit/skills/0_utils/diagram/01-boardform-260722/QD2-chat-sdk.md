...
Too little permission and it cannot work; too much and a browser tab can edit the whole repo at will. JL's direction is "same as the CLI": ask when asking is due, instead of a hard-coded whitelist. The drawer is the most-used entry, lighter than opening a terminal, so without settled tiers either you dare not use it or you dare not hand it to anyone. It and `QD3` are two forms of one need with entirely different trade-offs, so each must be settled on its own, neither bent to fit the other.
