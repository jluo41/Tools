...
- [x] **C1** 🔴 `[M]` food-to-description's entire documented CLI is dead on arrival: `food-to-description/pipeline.py:20` imports `from stages import stage_1_decompose` (through line 23) but `stages/__init__.py` exports nothing and the modules are named `1_decompose.py` etc. (leading digit, not importable by that name). Verified: `python3 -c "from stages import stage_1_decompose"` raises ImportError. Every command in `food-to-description/SKILL.md:105`-135 crashes. Fix: alias in `stages/__init__.py` via importlib (`stage_1_decompose = importlib.import_module(".1_decompose", __name__)`, x4).
  - **RESOLVED 2026-07-12 (v3.0.0)** — not by the aliasing fix, which treats the symptom. The digit-named modules were themselves the symptom: an installed package (`haiutils`) was reaching into a skill directory via `sys.path.insert()` to import them. The resolver moved into `code/haiutils/food_enrichment/` (declared in pyproject.toml beside haipipe/hainn/haifn), the modules got importable names, and `pipeline.py` became a thin argv parser over `enrich_food_to_nutrition()`. With the dependency pointing the right way there is nothing left to alias. Fixing C1 also killed three latent bugs it was hiding: 40 lines of `spec_from_file_location` boilerplate, a `from code.haiutils` import that collides with the stdlib `code` module (fatal in any notebook), and a `parents[2]` that pointed at `code/Tools/...`. Verified: `python pipeline.py --lexicon` now runs. See `food-to-description/CHANGELOG.md`.
