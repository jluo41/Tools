...
**Default: this page does not exist.** Since 260824 the warrant rides inline on each design card's `stance:` (haipipe-plugin-design §card), and the group slot (1) stays vacant. A principle page is PROMOTED into existence by exactly two conditions, and its Opening must say which: a warrant reused by two or more Design pages (restating it drifts), or two InsightBoards whose counsel conflicts (the adjudication needs a page with a Log). A board may not scaffold this group empty.
