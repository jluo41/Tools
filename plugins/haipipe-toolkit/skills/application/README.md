...
| per-stage `_PROBE/PPNN_*.md` cards; `1-probe-plans/README.md` index; `/haipipe-probe plan from-need` | flat pool `1-probes/PPNN_<topic>/` (one file per TOPIC, one `## QX<n>` ENTRY per q-executor, states `planned\|commissioned\|answered\|read\|answered-local\|failed`); `/haipipe-application probe "<q>"` raises a section + `probe run` |
