---
name: haipipe-application-digest
description: "Utility verb. Digests a session -- the CURRENT one, or a PAST session named/id'd as an argument (run from a fresh session) -- scanning its transcript for tool/skill feedback (gripes + preferences), distilling discrete items, deduping (merge-or-create), and after a MANDATORY confirm gate routing each through the feedback router into sub-skill inboxes. The bulk harvester for /haipipe-application feedback. Never auto-files."
...
harvest -- `/haipipe-application digest "SMS-for-Engagement-v0618"`. With no
argument it digests the CURRENT session instead. Running from a fresh session is
what keeps the digest's judgment uncontaminated by the work it is reviewing.
...
## Run: `/haipipe-application digest ["<session-name|id>"] [--dry-run]`
...
1. SCAN the target session's transcript for TOOL/SKILL feedback signals:
     ...
     DROP  - intervention-content discussion (about the ARTIFACT, not the TOOL)
           - one-off task instructions ("now draft the SMS", "fix this wording")
...
2. DISTILL into discrete candidate items, ONE concern each, in YOUR words
   ...
   feedback + a one-off task instruction + a file path (e.g. "the SMS draft
   should respect the venue character limit ... Please go ahead and draft it, and
   my comments go in 4-display.md"). Extract ONLY the feedback clause(s); DROP the
   task clause ("go draft it", "run X now"); a path is a location, not feedback.
...
3. DEDUP each candidate, and infer its target via the feedback router. ROUTING
   ...
   keyword-routed to -display. Route by the complaint's SUBJECT, not by a
   skill-name or path it MENTIONS: "add feedback to /haipipe-draft that the
   display must follow predefined content" is about DISPLAY (the subject), not
   draft (a mis-named destination).
...
4. PRESENT the candidate list for the MANDATORY confirm gate. Group by target
   ...
   on its own (e.g. two display files both about "panel ordering + comments").
...
6. REPORT what landed where: "N new, M merged, K dropped" (dup-in-batch items
   ...
   `/haipipe-application feedback list` to review the result.
```
...
not an application-skill defect). Do NOT file those -- FLAG them in the report
and suggest you `/remember` them. One job per verb.
  skill defect    "the SMS draft ignored the venue character limit"  -> inbox
...
- drop transient:  one-off instructions and intervention-content talk are excluded.
