# design · create or resume one stable Design Folder

1. Resolve the Brief and one line of the Brief (audience × behavior job × primary venue).
2. Reuse or create `<DesignTopic>-DesignBoard/2-Design/Design-<NN>-<audience>-<job>-<venue>/<stem>.md`
   with `folder-kind: design`. Load `haipipe-design` as its canonical owner.
   Reject D0–D5/GD0–GD6, `design/DU*/`, PageX, v1, or `rNN_design_*` shapes;
   there is no Design migration path.
3. Pin relevant authorized inputs. Validate signed contextual W handoffs before
   empirical use; do not inspect raw Task/Discovery results or invent Insight
   Supporting Run identities. A commissioned brief-only creative unit is legal.
4. Use `haipipe-design-workflow` to write/release commissions, allocate YAML
   Run Tickets, generate through `haipipe-design-unit`, independently verify,
   and present exact candidate previews for a person's adoption.
5. Run the Page Face through its own `rp00` → paragraph Page Runs → release →
   CHECK lifecycle. The Page explains goals, constraints, candidate differences
   and selected members; it does not generate the candidate. A DU is a
   generation Result, not a nested phase-owned Folder.
6. Missing load-bearing premises become named gaps for the caller; hold only
   dependent work. Do not silently launch another lane.
7. Record adoption of exact Result/member/verification/render/handoff versions.
   That receipt is the Folder's `domain-gate`; Page CHECK reuses it instead of
   asking the person to accept the candidate twice. Release and check the Page
   projection, then stop. Do not ship or measure.

New input/content/config changes get new `rdNN_*` Runs and stale only affected
current bindings. Unsupported old Design shapes are never read as inputs.
Render with fn/render.md; projections are never the authoritative DU content.

Return Folder path, commissioned Runs, generated/verified Result paths, adopted
members, Design frontier, Page frontier, freshness, open gaps, and the next
bounded action.
