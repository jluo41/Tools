...
probe    claims from runs                         (yaml + verdicts)
insight       cross-probe KB                      (D/I/K/W markdown)
paper         academic publication                     (paper-*)
application   session workflows                ← THIS SKILL FAMILY
...
├── insights/                               (insight)
├── paper/                                  (paper)
└── applications/                           ← application writes here
...
/haipipe-application ask <question>                 research session (probe-allowed)
...
application READS  ←──── task results (ask kind only)
application READS  ←──── probe claims (ask kind only)

application TRIGGERS ──→ probe (ask kind only, sole external trigger)
application TRIGGERS ──→ task       (ask kind only)
application TRIGGERS ──→ haipipe-insight-{data,information,knowledge,wisdom}
                          (ask kind, files KB cards from materialized evidence)

insight     NEVER reads from applications/
```

The ask kind is the **only** outside trigger for probe.
...
ref/application-input-contract.md         how to load K/W from insight
...
```
discover    seeds ideas → may suggest research questions for ask
project     project umbrella → owns examples/Proj-X/applications/
task        runs code → triggered by ask
probe  claims → triggered by ask
insight     K/W → REQUIRED INPUT for all kinds
paper       parallel external artifact (academic-only twin of report)
```
