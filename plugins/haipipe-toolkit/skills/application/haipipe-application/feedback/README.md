# haipipe-application — Feedback Inbox (orchestrator fallback)

Capture complaints, confusions, and wishes about the application SKILL (its
output is hard to read, a step is clunky, a verb is missing) while using it, then
fix them later in a skill-revision pass. This is feedback about the TOOL, not the
...
discipline that no single stage owns (stage strip, illuminate-every-stage, the
stage gate, the status tail, depth-by-venue, the diagram-ascii habit,
interrogate-every-unit) plus anything the router could not classify. The folder a
file lives in IS the record of which skill it concerns; there is no cross-skill
shared feedback.

## How

```
capture   /haipipe-application feedback "<what bugged you>"
...
digest    /haipipe-application digest
...
move      /haipipe-application feedback move <file> <skill>
