...
#
# (application family; family-local fork of the paper checker -- same conventions,
# vocabulary adapted: paper_root -> intervention_root. Venue scaling happens at
# lane CREATION, not here: an OWED lane is a debt regardless of venue, and absent
# lanes/docs are simply not scanned.)
#
...
#                              (read-target-working: the artifact claims it READ an UNFINISHED
...
#   invokes this same whole-intervention glob. (JL ruling C8-i)
#
...
function stage_leak(s, mode,   low) {
  ...
  if (mode == "comm" && low ~ /this paper|the intervention we|seeds stage/) return 1
          ...
          case "$target" in
            ...
            *)
              ...
              if [ -z "$tgt" ]; then
                ...
                # internally consistent, the artifact's claim is FALSE, and before R19/R20
                # NOTHING fired. `answered-local` targets are the intervention's OWN
                # registries, not QA files, so the `*/QA/*` guard leaves them alone.
                case "$target" in
                  */QA/*)
