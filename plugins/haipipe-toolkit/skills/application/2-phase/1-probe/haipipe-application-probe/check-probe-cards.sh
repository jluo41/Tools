...
# (application family; fork of paper/2-phase/1-probe/haipipe-paper-probe/
# check-probe-cards.sh, stage-strip.sh precedent: family-local copy, same
# conventions -- vocabulary adapted: paper_root -> intervention_root,
# manuscript -> artifact).
...
#
# VENUE SCALING: happens at lane CREATION, not here. TRANSLATE writes lane
# lines only for lanes the pinned venue fires (_VALUES_ always; _CITATION_
# sectioned venues only; _DISPLAY_ only if the artifact has display units),
# so this checker is presence-driven exactly like paper's: an OWED line is a
# debt regardless of venue, and absent lanes/docs are simply not scanned.
# No venue lookup needed.
#
...
set -u
intervention_root=${1:?usage: check-probe-cards.sh <intervention_root> [project_root]}
project_root=${2:-}

# Resolve project_root: FIRST ancestor of intervention_root containing discoveries/.
# Never git rev-parse here -- repo-backed projects make --show-toplevel return
# the wrong root (paper-side lesson; same walk-up keeps the two forks identical).
if [ -z "$project_root" ]; then
  d=$(cd "$intervention_root" && pwd)
...
[ "$found" -eq 0 ] && echo "WARN  no _PROBE/PP*.md cards under $intervention_root/0-lifecycle/"

# Working-doc pass: the no-bibtex / no-tables rules must hold durably,
# not only during a harvest run. Mirrors the harvest-acceptance greps.
# Presence-driven: lanes the venue never fired have no doc and are skipped.
for doc in "$intervention_root"/0-lifecycle/*/_CITATION_*.md "$intervention_root"/0-lifecycle/*/_VALUES_*.md "$intervention_root"/0-lifecycle/*/_DISPLAY_*.md; do
  [ -e "$doc" ] || continue
