...
#                         (application family; fork of the paper checker).
#
...
#
# (application family. Same file, same rules, same shared LEAK_AWK pattern set as the
# paper checker, with `intervention_root` in place of `intervention_root` -- and WITHOUT the
# paper-only RESOURCE-STAGE pass. Keep the two in step; the paper copy is the source.)
#
# Usage: sh check-probe-cards.sh <intervention_root> [project_root] [--stage <key>]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) if [ -z "$intervention_root" ]; then intervention_root=$1
       elif [ -z "$project_root" ]; then project_root=$1
       else echo "FAIL  unexpected arg: $1"; exit 1; fi ;;
  esac
  shift
done
[ -n "$intervention_root" ] || { echo "usage: check-probe-cards.sh <intervention_root> [project_root] [--stage <key>]"; exit 1; }

# Resolve project_root: FIRST ancestor of intervention_root containing discoveries/.
# Never git rev-parse here -- repo-backed projects are their own git repos, so
# --show-toplevel returns intervention_root itself, not the project.
if [ -z "$project_root" ]; then
  ...
  d=$(cd "$intervention_root" 2>/dev/null && pwd) || d=""
  [ -n "$d" ] || { echo "FAIL  no such root: $intervention_root"; exit 1; }
  while [ ! -d "$d/discoveries" ] && [ "$d" != / ] && [ "$d" != . ]; do d=$(dirname "$d"); done
  if [ ! -d "$d/discoveries" ]; then
    echo "FAIL  no project_root: no ancestor of $intervention_root contains discoveries/"
    exit 1
  fi
  project_root=$d
fi
...
function stage_leak(s, mode,   low) {
  ...
  if (low ~ /claims-stage|the seed|the pitch|the narrative|our intervention/) return 1
  if (mode == "comm" && low ~ /this intervention|the intervention we|seeds stage/) return 1
  if (mode == "bank" && low ~ /this intervention needs/) return 1
  return 0
}
        ...
        if [ -z "$target" ] || printf '%s' "$target" | grep -q '<'; then
          ...
          case "$target" in
            ...
            *)
              ...
              elif [ -e "$intervention_root/$target" ];   then tgt="$intervention_root/$target"
              fi
              if [ -z "$tgt" ]; then
      ...
      commissioned)
        ...
        case "$target" in
          ...
          */QA/*)
            ...
            elif [ -e "$intervention_root/$target" ];   then ctgt="$intervention_root/$target"
            fi
...
for legacy in "$intervention_root"/1-probe-plans/PP*.md \
              "$intervention_root"/0-lifecycle/*/_PROBE/PP*.md \
              "$intervention_root"/0-lifecycle/*/*/_PROBE/PP*.md; do
  [ -e "$legacy" ] || continue
  echo "MIGRATE  ${legacy#"$intervention_root"/}  -- pre-v8 location; ORGANIZE rewrites it into 1-probes/ on first touch"
done


# ---------------------------------------------------------------------------
# No probe files found (or none serving the requested stage). Application has no
# resource stage, so there is no vacuous-green backlink pass here (paper-only).
# ---------------------------------------------------------------------------
if [ "$found" -eq 0 ]; then
