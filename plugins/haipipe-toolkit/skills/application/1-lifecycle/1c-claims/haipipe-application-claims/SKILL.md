...
description: "Stage orchestrator for rung 1c of the venue-FREE evidence ladder (1a-descriptions -> 1b-themes -> 1c-claims -> 1d-advice): the claim ledger + evidence campaign brain — what generalizes, with status + anchor. THE ONLY home of a claim's status (supported | weak | GAP). Three sections: Claims (theme-tagged, status + PP reference), Q-consumer (the evidence questions raised, one `## Q-Claim-<n>` block each), Evidence Campaign (dispatch order + dependencies). The pinned venue sets how much of the campaign must SETTLE (light/medium/full). Markdown only. Trigger: claims, claim ledger, what must be true, what generalizes, evidence plan, probes, supported, GAP, /haipipe-application claims."
...
`supported | weak | GAP` lives HERE, per-claim, private to this intervention — never in a probe file.
A probe entry carries the evidence (its `### a-executor`, a copy of the answering QA file's answer); the judgment is ours to write.
A claim is `supported` only when it traces to a JUDGED artifact (a probe answer, or an equivalently reviewed result) — never from intuition, never from a raw unjudged number.
...
Q-consumer         one `## Q-Claim-<n>` block (Ask / Why / Answer=__TO_BE_FILLED__) per evidence question this stage raises; the route,
...
Sidecar: `_LOG_1c-claims.md` (phase journal). Verified numbers stay INLINE on the claim (+ a resolving pointer to the task/discovery result, which is the source of truth) — no `_VALUES_` / `_CITATION_` sidecar.
