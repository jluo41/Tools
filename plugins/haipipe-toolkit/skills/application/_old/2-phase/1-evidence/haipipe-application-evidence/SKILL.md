---
name: haipipe-application-evidence
description: "Application-specific EVIDENCE phase worker (internal). Owns the whole five-step loop from each stage Page Q-consumer through a neutral Q-executor and returned A-executor to the Page-facing A-consumer. The persisted Probe file uses the existing QX schema, binds by path to a QA file in the task/discovery bank, and dispatches only the Q-executor. DRAFT raises questions and stops. Users invoke stage skills, not this directly."
allowed-tools: Bash, Read, Write, Edit, Grep, Glob, Skill, Agent
metadata:
  argument_hint: "[from-buffer <intervention_root> [PPNN] | stage <stage-name>]"
  version: "0.3.3"
  last_updated: "2026-08-04"
  summary: "Application-specific EVIDENCE worker layered on haipipe-page-evidence and haipipe-probe; it runs all five steps because DRAFT authors no executor-side field."
---

Skill: haipipe-application-evidence — the EVIDENCE-phase worker for an application
============================================================================
...
Not user-facing: users invoke stage skills; a stage calls `Skill("haipipe-application-evidence", args="from-buffer <intervention_root> [PPNN]")`.
...
The EVIDENCE-phase rules live in `../../../../probe/haipipe-probe/SKILL.md` → **Phase rules · EVIDENCE phase** (+ **The QA file**, **The two LAWS**). Follow those; on conflict, that file wins. Application-specific additions:
...
For each STILL-COLLECTING entry (`target: NEW`, bank `run`/`code`/`new`, not resolved by T1 LOCAL), hand the SET to the collector agent, tagging each with the EVIDENCE-authored `route:` (task|discovery — AUTHORITATIVE, not a hint):
...
DEFERRED / ASYNC is the agent's too: an entry it cannot land synchronously comes back `in-flight` and stays `commissioned`; the NEXT EVIDENCE run re-hands it. This worker writes NOTHING under `tasks/` or `discoveries/`, ever — no stub, no mailbox.
...
Never report a green EVIDENCE over a FAIL.
...
Siblings: DRAFT (haipipe-application-draft) → EVIDENCE (this) → REVISE (haipipe-application-revise) → CHECK (haipipe-application-check).
