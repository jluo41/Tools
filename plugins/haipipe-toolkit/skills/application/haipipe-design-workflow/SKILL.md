...
description: >-
  The DesignBoard-level phase machine: five phases named after the artifact classes the lane owns — D0 Brief (frame) → D1 Direction (bet) → D2 Unit (realize) → D3 Verdict (evaluate) → D4 Division (decide) — with gates GD0-GD5, each a checkable assertion over artifacts on disk. The frontier's atomic unit is the THREAD: a card until it lands, the division row after; a round is one full D0→D4 pass, and ROUND ONE ALWAYS COMPLETES — insufficiency never stalls a round, it exits as an EMIT that lands on the Brief's needs block and becomes a register question by the ordinary need-first birth, which is the problem-solution co-evolution edge. Evaluation has two mandated faces: reflect (ex-post, the judge's conformance verdict, on-disk word judged) and prospect (ex-ante, a scorable forecast of the artifact in use, written at realize and checked here). It refines the application machine's design lane (P3 = D0, P4 = D1-D4) and accepts a one-sentence COMMISSION; interior law stays with the door /haipipe-design, page lifecycle with haipipe-page-workflow, every verdict with an independent CHECK plus a human tick. Use when asking where a design round stands, whether a thread may advance, what the next runnable step is, how to commission a design end-to-end, or where a run must stop. Trigger: design workflow, run the design board, design round, commission a design, design me a message, next division, frontier thread, reflect, prospect, ex-ante, ex-post, forecast, emit a question, /haipipe-design-workflow.
metadata:
  version: "0.5.0"
...
A **design phase** is one digit, `D0`-`D4`. A Design PAGE id always carries two digits (`DS01`), so a one-digit `D<n>` in a design-lane document is a phase, never a page — the same digit-count rule the other machines use. Against the application machine: `🎨P3 = D0`, `🎨P4 = D1-D4`, card proposal included; the aliases frame, bet, realize, evaluate, decide are legal in prose, never in a folder or file id.
...
## 🗺 The five phases · the phases are the wager's stations

The insight lane's aliases are verbs of KNOWING (scope, ask, observe, derive, claim, hand off); this lane's are verbs of WAGERING — frame, bet, realize, evaluate, decide:
...
D1 Direction (bet)       direction/DR<NN>-<slug>.md    the bets: stance · thesis ·
                                                       expected effect (the PRIOR) ·
                                                       falsification line · grant
D2 Unit (realize)        design/DU<NN>-<slug>/         the artifact, complete: README ·
                                                       spec · evidence ⊆ grant ·
                                                       prospect.md · content/
...
D4 Division (decide)     the DS page's division row    ✋ accepted: per the Design page
   ...
   ↺ D1→D4 runs per THREAD · D4→D0 turns the ROUND · exits at GD5, whose
     terminals include EMIT
```

**The thread, precisely.** The frontier's atomic unit is a THREAD: it is a CARD from proposal to landing (state words proposed · released · landed · killed) and a DIVISION ROW after landing (the door's law — one division per landed unit — is untouched). A card at `proposed` blocks its own fan-out and nothing else: it never blocks a round from closing, it simply carries to the next round unreleased.
...
D2 realize   🎨 one arm-agent per released card, in parallel, packet = the grant ·
...
D4 decide    each division: ✋ accepted  ──▶ all threads terminal, all landed ones
             accepted = the lane's ACCEPTED close · STOP ───────────────────────┐
...
## 🎲 The three stances, and why generate exists

D1's cards take three POSTURES toward evidence, and the lane needs all three or it is not a design lane (added 0.3.0, after two live rounds proved the machine would only ever redistribute fielded copy). The stance WORDS are the direction plugin's five; the postures group them:
...
GD1  bet → realize     ✋ per card: proposed → released, a person's act — card by
                       ...
                       (haipipe-plugin-direction §release) · a release binds only
                       cards that EXIST when the words are recorded · no expected
                       effect, no release · kill is the equal-rank terminal
GD2  realize → verdict the unit exists complete per the plugin anatomy: README ·
                       spec · evidence · prospect.md · content/ · evidence ⊆ grant ·
                       the card's landed: back-pointer resolves · a generate unit's
...
GD3  reflect passes    ex-post: grant ⊆, spec acceptance list and venue rails are
                       ...
                       state: judged · the judge is never the unit's arm
...
GD5  decide            per division: ✋ an accepted: row satisfying the Design page
                       ...
                       blanket over named divisions · every thread terminal closes
                       the round · all landed threads accepted = ACCEPTED, STOP
...
evaluate → STOP at GD5, renders visible, decisions owed
```
...
D1        2-DS-design/DS<NN>-<slug>/direction/DR<NN>-<slug>.md
D2        2-DS-design/DS<NN>-<slug>/design/DU<NN>-<slug>/
...
run      realize through agents/haipipe-design-arm-agent, one per card, packet =
...
Per thread: the highest gate whose assertion currently holds. Per page: the division rows and open cards read whole — a page-level scalar is a lie this file refuses to mint. The application machine's `🎨P<n>` reading derives from the same artifacts (P3 = BR00 owed, P4 = any thread from proposal to GD5).
