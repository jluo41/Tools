...
3. Read `skills/C_task/haipipe-task/ref/stata-dialect.md` (the engine contract —
...
□ orchestrator resolves Stata via Resolve-StataExe (no hardcoded Stata18) and
  runs from $PSScriptRoot (the task folder)
□ runner resolves ws_root by walking up to pyproject.toml; passes -wsRoot
