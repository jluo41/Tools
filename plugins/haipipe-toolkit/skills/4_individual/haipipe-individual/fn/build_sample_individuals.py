"""Build per-individual folders under _WorkSpace/A-User-Store/UserGroup-{DatasetTag}/.

Extract-and-copy approach: filter existing global 1-SourceStore + 2-RecStore
parquet files by PatientID and write per-individual slices. Fast (~seconds per
individual) vs. re-running full source/record pipelines.
...
Usage:
    python build_sample_individuals.py OhioT1DM 540 544 552 559 563
    python build_sample_individuals.py OhioT1DM --n 5
    python build_sample_individuals.py --all --n 5
"""
...
def flatten_rel(rel: Path) -> Path:
    ...
    for p in parts:
        ...
        # We pass these in via context; see build_individual below.
        keep.append(p)
...
#   pid_values      — list of PatientID strings matching this individual
...
def build_ohio(individual_id: str) -> dict:
    cohort_2018 = {"559", "563", "570", "575", "588", "591"}
    cohort_2020 = {"540", "544", "552", "567", "584", "596"}
    if individual_id in cohort_2018:
        cohort = "2018"
    elif individual_id in cohort_2020:
        cohort = "2020"
    else:
        raise ValueError(f"Unknown OhioT1DM individual_id: {individual_id}")
    raw_dir = WORKSPACE / "0-RawDataStore" / "OhioT1DM" / "Source" / cohort
    raw_paths = [raw_dir / "train" / f"{individual_id}-ws-training.xml",
                 raw_dir / "test" / f"{individual_id}-ws-testing.xml"]
    return {
        ...
        "pid_values": [f"ohio-{individual_id}_train", f"ohio-{individual_id}_test"],
        "raw_paths": raw_paths, "raw_copy": True,
    }


def build_mimic_v31(individual_id: str) -> dict:
    return {
        ...
        "pid_column": "PatientID",
        "pid_values": [f"mimic-{individual_id}"],
        "raw_paths": [WORKSPACE / "0-RawDataStore" / "mimiciv-3.1"],
        "raw_copy": False,
    }


def build_cgmacros(individual_id: str) -> dict:
    return {
        ...
        "pid_column": "PatientID",
        "pid_values": [f"CGMacros-{individual_id}"],
        "raw_paths": [],
        "raw_copy": False,
    }


def build_welldoc_2022cgm(individual_id: str) -> dict:
    return {
        ...
        "pid_column": "PatientID",
        "pid_values": [f"2022cgm-{individual_id}"],
...
def list_candidates(dataset: str, limit: int = 5) -> list[str]:
    """Return first N individual IDs (sorted numerically when possible)."""
    if dataset == "OhioT1DM":
    ...
    # strip dataset prefix to get pure individual_id
    prefix = pids[0].split("-")[0] + "-"
    individual_ids = [p[len(prefix):] for p in pids]
    try:
        individual_ids = sorted(individual_ids, key=int)
    except ValueError:
        individual_ids = sorted(individual_ids)
    return individual_ids[:limit]
...
def filter_parquet(src: Path, dst: Path, pid_col: str, pid_values: list[str]) -> int:
    ...
    try:
        ...
        if pid_col not in schema.names:
            return -1  # dictionary table — don't duplicate per-individual
...
def copy_nonparquet(src: Path, dst: Path) -> None:
    ...
    per-partition data bounds that don't apply to a individual-scoped slice, and
    ...
    for ds in datasets:
        ...
        for sid in ids:
            try:
                ...
                print(tag)
                summary.append({"dataset": ds, "individual_id": sid, **result})
            except Exception as e:
                print(f"  Subject-{sid}: ERROR {e}")
                summary.append({"dataset": ds, "individual_id": sid, "status": "error", "error": str(e)})
