...
metadata:
  ...
  summary: "The producer for context/PREPARE and the generated off-stage Context record."
...
cannot override this binding: `run` is context and `cycle` is PREPARE.
...
`haipipe-workbench-page/ref/record-shape.md`. Load the owner once when the same
...
`actor: haipipe-page-context-agent`, `Run: CONTEXT`, and `cycle: PREPARE`.
