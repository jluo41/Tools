...
metadata:
  ...
  summary: "The producer for writing/WRITE, replacing active DRAFT and REVISE agents."
...
cannot override this binding: `run` is writing and `cycle` is WRITE.

Follow the router's canonical order: `haipipe-page` →
`haipipe-page-workflow` → `haipipe-page-writing` → exact Folder-owning
...
`actor: haipipe-page-writing-agent`, `Run: CONTENT`, and `cycle: WRITE`.
