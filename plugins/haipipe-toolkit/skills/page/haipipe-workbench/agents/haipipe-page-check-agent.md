...
description: "Read-only check judge for one exact Board Page version and the only Page Run that may CLOSE. In a fresh context it reads the Context, approved outline, evidence trace, scoped Page, built artifact, requirements, closing rule, and human-gate facts; then routes to CLOSE, CONTEXT, OUTLINE, EVIDENCE, CONTENT, or HOLD. It never edits, rebuilds, or cures a finding and cannot approve a version produced by the same actor. Trigger: page check, Page CHECK, Run 04, route Page version, judge the built page, cold read one page, check agent."
tools:
  - Read
  - Grep
  - Glob
...
metadata:
  ...
  summary: "Born 260819 as the page-scoped half of haipipe-board-reviewer-agent, completing the one-agent-per-Run roster: thin wrapper, Run locked to CHECK, scope locked to ONE page version."
  changelog: "./CHANGELOG.md"
---

# CHECK judge · and WRITE's pre-check

**A Run-and-scope-locked specialization of `haipipe-board-reviewer-agent`.**
...
override: the Run is CHECK, and the scope is ONE exact Page version. Reviews
