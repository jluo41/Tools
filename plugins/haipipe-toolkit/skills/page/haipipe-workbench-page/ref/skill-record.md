# Skill record · the ranked Skills Page Record

This is an internal record contract of `haipipe-workbench-page`. The category
skill owns the public surface; this reference owns the list grammar, seed
behavior, writer, and human ranking boundary.
...
A plugin-level `agents/` directory participates in the same index, so a Board hosted by `haipipe-toolkit` can cite the skills and agents of a sibling workbench such as `subjective-label`; the hosting workbench is not an index boundary.
...
## ⚙️ Writer · the person, in the file
