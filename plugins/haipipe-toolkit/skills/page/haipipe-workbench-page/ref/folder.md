# Folder tab · the folder is the truth

**LOAD `haipipe-workbench` and `../SKILL.md` (`haipipe-workbench-page`) FIRST.** The
first owns what any workbench is: storage, surface, writer, boundary; the second
owns the Page-level tabs this lane belongs to. This reference owns only the 📂
Folder tab's delta, and the delta is an inversion: every other lane is a
subfolder the tab surfaces, and this one is the tab that surfaces the
subfolders. It is the live inventory surface, one row per explicit material
lane with file count, weight, age, and a ⚠️ STALE flag when a derived lane
predates the page's .md. Stores nothing, renders live.
...
The workbench's material is the page folder ITSELF: every sibling's subfolder is what this surface reads, so it holds no roster row — the roster lists subfolder names, and this is the view OVER them.
...
This workbench owns only the Folder tab's live **display** lens. It reports what
...
- `../../../../servers/workbench-page/folderstat.py`
  The live walk, the staleness rule, the GET and its no-write POST twin.
- `../../../../servers/workbench-page/assets/js/10-drawer/06-workbench-folder.js`
  The registration: ordered after Delivery, folded pages only.
- `../../haipipe-workbench/ref/roster.md`
  The list this surface renders the truth of — the one lane here with no row of its own.
