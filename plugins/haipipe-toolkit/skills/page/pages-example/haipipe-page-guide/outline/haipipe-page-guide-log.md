...
### 260912 1624 · Add the standalone category-workbench pane
User request: “Could you add the workbench as what we do in the board page workbench as well?”

The standalone host now presents Outline, Runs, Delivery, and Folder in the canonical Page-workbench order. Each row reaches a real read-only presenter over the same Page Folder; Evidence remains inside Outline. Studio is omitted because standalone does not yet own its chat/draw backend. Read-only hosting removes the Source editor-shaped surface instead of showing disabled editing controls.
