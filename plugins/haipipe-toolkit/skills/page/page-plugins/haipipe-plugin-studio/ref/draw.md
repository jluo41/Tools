# Draw lane · one scene per owner, saved to exactly one owner

This is an internal lane contract of `haipipe-plugin-studio`. The category
skill owns the public surface; this reference owns scene storage, writer,
ownership boundary, and edit guarantees.
...
A folded page's scene lives in its own Draw lane; the group scene stays with the group, because relations BETWEEN pages belong to no one page.
...
├── QX1-page/
│   ├── QX1-page.md
│   └── studio/draw/QX1.excalidraw   the page's own source
└── QX2-page/
    └── studio/draw/QX2.excalidraw
```

`draw/` is lowercase everywhere. A folded Page writes only through the nested
...
(`live/xcal.py`), so the editor never falls back to a leftover buffer. If only
...
The ✨ Draw it button on Studio's Draw segment POSTs `/_board/autodraw`; `live/autodraw.py` runs `claude -p` over the page's own `.md` and writes the scene server-side.
...
- `../../../../board/haipipe-board/live/autodraw.py`
