...
- B2 · [Mechanism] Plugins present records through their declared writers.
  ...
  Draft: Plugins present the Page's planning records, conversations, runs, outputs, and files through their own declared writers.
- B3 · [Criterion] A visible label does not establish working integration.
  ...
  Draft: Putting a plugin's name on a folder or button is not enough; the plugin must actually open the Page's records and perform the actions it promises.
...
- B2 · [Gate] Mermaid Structure closure freezes a Page-global `P01..PN` index.
  ...
  Draft: That Mermaid Structure closure freezes the Page-global reading order as `P01`, `P02`, through `PN`, with every serial mapped to its plan address.
- B3 · [Scope] A closed paragraph index defines the Step scope for each Page Run.
  ...
  Draft: After the Mermaid Structure closes, the Runs function creates Steps, each with a scope covering one or more of the `N` numbered paragraphs.
