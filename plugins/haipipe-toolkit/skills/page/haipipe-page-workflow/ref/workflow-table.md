...
serialized Run names `CONTEXT`, `OUTLINE`, `EVIDENCE`, `CONTENT`, and
`CHECK` remain routing/progress coordinates only; they are not Run authority
or Run identities.

## Space roster

Page uses the Spaces exposed by its workbenches:
...
| `revise` | `Page.interactive-writing.revise` | fresh agent/human | compare two frozen texts of one target (Before, After by hash); one ledger row per material change with kind, Why, decision | every row decided | `writing` (NEW_VERSION with the accepted text), `CLOSE`, `HOLD` | `0..N` per target | Draft (read-only) + Runtime | `REVISE` |
...
`haipipe-workbench-page/ref/space-mapping.md`. It is the physical
projection of this normalized table: each cell records `mode · schema · path`.
In the Page workbench, the user-facing word is `Space`; the normalized schema
...
worker_skill_chain: [haipipe-page-structure, haipipe-writing]
...
Count allocated identities and receipts, not Run names, Steps,
...
| human Scratch capture | `haipipe-page-scratch` |
| Structure/Bullet/evidence-route planning | `haipipe-page-structure` |
| interactive prose | `haipipe-writing` |
| evidence execution/binding | `haipipe-page-evidence` |
| adoption and writing | `haipipe-page-writing` |
| delivery build and receipt | `haipipe-page-delivery` |
| before/after comparison and decisions | `haipipe-page-revise` |
