...
- Docs: no Mermaid map or `.mmd` deliverable anywhere in the Run list; the structure is the
  Outline's headings, shown as a text list. Revise Run gains the direct-edit door.

## 0.64.0 · 2026-09-22

- `ref/phase-cards.md` is `ref/run-cards.md`; the controller, its refs, and the Run skills say Run,
  never phase. `run`, `cycle`, `next_cycle` are the serialized receipt fields.

## 0.63.0 · 2026-09-22

- Moved up one level to `skills/page/haipipe-page-workflow`: the Workflow is the list of
  Runs, and each Run Spec now has one skill under `skills/page/workflow-runs/`, named by the Run
  (`haipipe-page-context`, `-structure`, `-scratch`, `-writing`, `-evidence`, `-revise`,
  `-delivery`, `-check`). `page-workflows/` is gone; the 00–04 phase words survive only as
  route and Step names.
- New `revise` Run Spec (`rp-revise-NN_<target>`): compare two frozen texts, settle every
  change, hand the accepted text to the writing Run as NEW_VERSION.
...
- Make CONTEXT, OUTLINE, and EVIDENCE share `haipipe-workbench-page` through
...
- Route LAND through the current Outline-owned Evidence Workspace and remove
  the retired standalone Evidence workbench from the active chain.
...
- PROBE retired with `/haipipe-probe` (both in `_old/`): MATCH → SURVEY's Run
  ...
  the outcome words. `ref/run-cards.md` rewritten per cycle; the run refs
...
- `ref/run-cards.md` ① and ④ aligned with `haipipe-page-structure` 0.12.0 and
...
- **The count was short by one, on every unclosed page.** `ticks_owed` carried
  four entries while `run-cards.md` § "The five person-reserved ticks,
...
- **260819 coherence sweep across SKILL.md and the three refs.** SKILL.md: §🔁
  ...
  transcribed from `src/page_lifecycle.py`. run-cards.md: the 🧑 LOOK gate
...
- §🧭 and §👷 caught up with their own carve-out: the shared producer law lives
  ...
  `haipipe-page-structure-agent` — both sections still said the creator agent
...
- **run-cards.md: seven stale phase numbers fixed.** PROBE's, DRAFT's, REVISE's
...
`ref/run-cards.md` added, and the QPw roster corrected twice by JL.
...
- New `ref/run-cards.md`: every phase stated ONCE in the SAME six fields,
...
`haipipe-workbench-page` said PROBE). One rule now, and it reads down the loop:
...
- The member table's PROBE row points at `../haipipe-page-probe`, which now
  exists, instead of borrowing `../workflow-runs/haipipe-page-evidence` "until split out".
...
- The receipt section now names its shipped reader: the 🪜 Workflow menu's
  `📄 Page phases` stepper (`65-workbench-pageflow.js` + `GET /_board/pageruns`),
