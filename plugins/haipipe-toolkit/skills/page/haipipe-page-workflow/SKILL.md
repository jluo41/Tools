...
description: >-
  The Page workflow controller: context/PREPARE, structure/SHAPE+SURVEY,
  evidence/LAND+EMBED, writing/WRITE, and check are dispatch labels
...
The formal controller/API serialization carries `run`, `cycle`, and
...
`haipipe-workbench-page`; the Page surface has already installed that workbench
...
`ref/workflow-table.md` and the physically named `ref/Run-cards.md` are
...
candidate, the frozen Structure description for each selected
...
```text
context     haipipe-page-context     PREPARE · Collect, Resolve, Freeze
structure     haipipe-page-structure     SHAPE · SURVEY
evidence    haipipe-page-evidence    LAND · EMBED
writing     haipipe-page-writing     WRITE · Adopt, Integrate, Build, Pre-check
check       haipipe-page-check       CHECK · whole-Page close gate
```
...
## 🧭 One Outline workbench serves three Workspaces

CONTEXT, OUTLINE, and EVIDENCE all use `haipipe-workbench-page`:

```text
haipipe-workbench-page
├── Draft Space          Structure list + Bullet/Draft table; read-only projection
...
`haipipe-workbench-context` or a second Evidence workbench. The Run Spec owner skills write;
the workbench reads and presents.
...
Each Run Spec in this table has one skill under `workflow-runs/`, named by the Run:
`haipipe-page-context`, `haipipe-page-structure` (rp-struct), `haipipe-page-scratch`
(rp-scratch), `haipipe-page-writing` (rp-sec, rp-para), `haipipe-page-evidence`
(re-*), `haipipe-page-revise` (rp-revise), `haipipe-page-delivery` (rd), and
`haipipe-page-check` (the gate). The CONTEXT/OUTLINE/EVIDENCE/CONTENT/CHECK
words below are the old Run names; they survive as route and Step names,
not as skill names.
...
The formal API may serialize the Run names as Run, cycle, and
...
`haipipe-workbench-page/ref/...` material contracts needed by the Run Spec; do
...
haipipe-page
  ...
  → haipipe-workbench-page/ref/plan-grammar.md
...
OUTLINE   haipipe-page-structure-agent    producer
...
auditor is `../haipipe-page/src/page_lifecycle.py`.

## 🧾 Receipts and terminal states

Every controller dispatch appends one receipt with the adapter dispatch label (serialized as `run`), cycle, actor, role,
...
└── ref/
    ...
    ├── Run-cards.md          physically named compact Run Spec cards
