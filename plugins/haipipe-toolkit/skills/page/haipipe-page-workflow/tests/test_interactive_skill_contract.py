...
WORKFLOW = SKILLS / "page/haipipe-page-workflow"


class InteractiveSkillContractTest(unittest.TestCase):
    def test_workflow_is_run_spec_graph_not_run_label_authority(self):
        workflow = (WORKFLOW / "SKILL.md").read_text()
        table = (WORKFLOW / "ref/workflow-table.md").read_text()
        cards = (WORKFLOW / "ref/run-cards.md").read_text()
        ...
        self.assertIn("not Run authority", table)
        self.assertIn("define Run authority", cards)
    ...
    def test_relative_instruction_links_resolve(self):
        files = [
            ...
            SKILLS / "page/workflow-runs/haipipe-page-writing/SKILL.md",
            SKILLS / "page/workflow-runs/haipipe-page-writing/ref/paragraph-run.md",
    ...
    def test_entry_points_route_to_one_interactive_contract(self):
        for relative in (
            ...
            "page/workflow-runs/haipipe-page-writing/SKILL.md",
            "page/workflow-runs/haipipe-page-structure/SKILL.md",
    ...
    def test_content_default_is_adoption_not_paragraph_allocation(self):
        text = (SKILLS / "page/workflow-runs/haipipe-page-writing/SKILL.md").read_text()
    ...
    def test_entering_an_open_run_returns_a_pre_step_review_packet(self):
        ...
        for text in (packet, contract):
            ...
            self.assertIn("Structure description", text)
    ...
    def test_step_run_close_and_page_release_have_distinct_write_boundaries(self):
        ...
        workflow = (WORKFLOW / "SKILL.md").read_text()
        contract = (WORKFLOW / "ref/interactive-writing-run.md").read_text()
        content = (SKILLS / "page/workflow-runs/haipipe-page-writing/SKILL.md").read_text()
        template = (WORKFLOW / "ref/writing-step-template.md").read_text()
    ...
    def test_structure_run_fuses_shape_survey_and_allows_multiple_contributors(self):
        ...
        outline = (SKILLS / "page/workflow-runs/haipipe-page-structure/SKILL.md").read_text()
        runs_fn = (SKILLS / "page/haipipe-page/fn/runs.md").read_text()
    ...
    def test_draft_paragraph_header_does_not_reserve_bullet_gutter(self):
        outline = (SKILLS.parent / "servers/workbench-page/outline.py").read_text()
        self.assertIn(
    ...
    def test_evidence_labels_use_generic_display_namespace(self):
        ...
        page = (SKILLS / "page/haipipe-page/SKILL.md").read_text()
        evidence = (SKILLS / "page/workflow-runs/haipipe-page-evidence/SKILL.md").read_text()
        displays = (SKILLS / "page/haipipe-workbench-page/ref/evidence/displays.md").read_text()
        citations = (SKILLS / "page/haipipe-workbench-page/ref/evidence/citations.md").read_text()
