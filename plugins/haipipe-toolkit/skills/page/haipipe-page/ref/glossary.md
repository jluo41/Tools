...
- **reader-facing URL** — the HTTP address a person opens to read a generated
  ...
  `127.0.0.1`. `<DOMAIN>/b/<board-slug>/<page-id>`
...
- **controller receipt** — one Page dispatch/pass record, including operation
  and route; legacy `Run/cycle` fields identify control progress. It is not
...
Defined once in `haipipe-page-workflow` §🔤; repeated here only
...
- **Run** — retained controller/API dispatch label: `context` ·
  `structure` · `evidence` · `writing` · `check`. It is not a
...
- **controller step** — a numbered Run dispatch, not a Writing Step.
- **round** — which PROMISE era; repeats when a page is reopened.
- **RUN, not ADVANCE** — a page may repeat a Run, branch, HOLD, or go back;
  which Run runs next is chosen by AUTHORITY, never by position.

## 🧑‍🔧 Words for who acts

- **producer** — the hand that WRITES in one Run. One per Run.
  `page/haipipe-workbench/agents/haipipe-page-<run>-agent`
...
- **Run producer** — the current writer for CONTEXT, OUTLINE, EVIDENCE, or
  CONTENT. The active roster is `haipipe-page-context-agent`,
  `haipipe-page-structure-agent`, `haipipe-page-evidence-agent`, and
  `haipipe-page-writing-agent`; Draft, Revise, and Probe are not Run-agent
  identities.
...
- **🖼 a display unit** — a FOLDER that builds a real picture into the PDF.
  A page that DECLARED a unit and created no folder is not done, and the Run
