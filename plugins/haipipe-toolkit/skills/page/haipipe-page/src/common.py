...
def registered_page_source(folder):
    """Return a manifest's same-stem Page Face, or None for unsafe/invalid input.

    Registration is explicit; arbitrary Markdown and imported materials never
    become Board Pages merely because they exist. All manifest paths, including
    symlink targets, must stay inside the owning Page Folder.

    Registered Pages require Python 3.11+ (stdlib tomllib). Older Python keeps
    legacy Q/S Board imports and discovery; manifest registration is unavailable.
    """
    try:
        import tomllib
    except ImportError:
    ...
    try:
        ...
        if type(data.get("version")) is not int or data["version"] != 1:
            return None
        if data.get("source") != f"{folder.name}.md":
            return None
        ...
        if not source.is_file():
            return None
        # Authored Page Faces need no imported material. A present content key
        # still declares an import and must satisfy the full path contract.
...
def _in_plugin(p, d):
    ...
    for seg in parts[:-1]:
        nxt = cur / seg
        if ((cur != d or not (d / "board.md").is_file())
                and _page_home(cur) and not _page_home(nxt)):
            return True
        cur = nxt
    return ((cur != d or not (d / "board.md").is_file())
            and _page_home(cur) and p.name != f"{cur.name}.md")
