"""Where ONE Page sits in its Run list, computed from disk.

ONE copy of this logic, because two would drift: `cli/pageprogress.py` prints the
full strip, `status.py` prints the compact row inside the closing block, and
both call `run_progress()` here. Never writes.
...
The Runs, in list order, and what each state is read from:
    ...
    ✏️ writing    the Page's content divisions postdate the plan; when a LaTeX
                 delivery lane exists, its newest PDF also postdates the Page
    🔍 check      the newest _runs/page/<page>/*.json receipt routed CLOSE

The keys are the skill suffixes (`haipipe-page-<key>`). Receipts written before
2026-09-22 carry uppercase tokens (CONTEXT, OUTLINE, PROBE, DRAFT, REVISE, COMPILE,
CONTENT, CHECK); `run_of()` maps them and is the ONE place that knows the old words.

⚠️ `now` is the FIRST Run whose exit test fails, in list order. That is a
REPORT, never a routing: which Run runs next is decided by authority
(haipipe-page's authority test) and the check gate may route anywhere.
"""
import json
import re
...
# intentionally variable: a Run-owned Folder may declare that it owes no
...
# The Run's own emoji, taken from haipipe-page-workflow §🔁's loop diagram so
...
#
# ⚠️ ONE deliberate substitution: the loop draws the check gate as ✅, which is
# also this module's DONE marker, so a bar pairing them reads `✅✅` and says
# nothing. check carries 🔍 here, the act of judging, and only in the bar.
RUNS = (("context", "🧭"), ("structure", "🧩"), ("evidence", "🃏"),
        ("writing", "✏️"), ("check", "🔍"))
ORDER = tuple(name for name, _ in RUNS)
EMOJI = dict(RUNS)
LABEL = {"context": "Context", "structure": "Structure", "evidence": "Evidence",
         "writing": "Writing", "check": "Check"}
# Receipt tokens from before the Run vocabulary (2026-09-22). One map, read-only.
LEGACY_RUN = {"CONTEXT": "context", "OUTLINE": "structure", "PROBE": "evidence",
              "EVIDENCE": "evidence", "DRAFT": "writing", "REVISE": "writing",
              "COMPILE": "writing", "CONTENT": "writing", "CHECK": "check"}


def run_of(token):
    """`run` or legacy `phase` token -> canonical Run key; unknown tokens pass through lowercased."""
...
# the step words of the structure Run live on the item table (src/item_table.py)
...
def run_progress(page_md, board=None):
    """Return every Run's state plus the counts each one is judged on.
    ...
    states = {
        ...
        "structure": ("done" if approved and (not has_table or items["decided"] == items["rows"])
        ...
        "writing": "done" if content_ready else ("part" if divs else "owed"),
        "check": "done" if (last and run_of(last.get("run") or last.get("phase")) == "check"
                           and str(last.get("route", "")).upper() == "CLOSE") else "owed",
    }
    ...
    return {
        ...
        "state_line": (re.search(r"^state:\s*(.+)$", page_txt, re.M)
                       or [None, ""])[1].strip(),
        # The ledger is variable. Four workbench ticks are artifact-driven; the
        # owner RULING is Run-driven. Legacy Pages conservatively owe it,
        # while a Run contract may say `none` or reuse its domain gate.
...
def owed_ledger(st):
    ...
    # comes from the owning Run, not from the generic Page loop.
...
def render_ledger(st, width=78):
    """The ledger as text. `cli/pageprogress.py --owed` prints this."""
    rows = owed_ledger(st)
    if not rows:
