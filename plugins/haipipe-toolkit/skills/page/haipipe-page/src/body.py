...
RENDER_ROOT = None     # standalone/import boundary; Board legacy context leaves this unset
...
def resolve(token):
    ...
    if RENDER_ROOT is not None:
        ...
        for base in (PAGE_DIR, BASE):
            if base is None:
                continue
            try:
                relative = (Path(base) / tok).relative_to(RENDER_ROOT)
                candidate = render_confined(RENDER_ROOT, relative)
...
# 没写就默认 🔹。build.py 不去猜——图标随内容变，是作者写的，不是机器生成的。
