...
- Move the shared Runs and Delivery presenters under Page ownership; Board
  retains compatibility links to the same implementation.

## 0.66.0 · 2026-09-12

- Add the Page configuration/completion checklist and route creation and
  whole-Page audits through it. Assess Opening, generated Outline, Content and
  Aims explicitly, separating configuration, content, acceptance and hosting.
- Preserve standalone intake and Page-owner exceptions; do not equate a
  scaffold, build, or unverified human gate with completion.
...
- Move the canonical Page family to `skills/page`, retaining old path adapters.
- Own the shared Page parser/renderer and Outline/Evidence presenters.
- Add `cli/page.py init/inspect/build/serve`: byte-preserved file intake,
  portable Page registration, source editor and standalone static/live sites.
- Keep Board membership optional and reuse the same Page source in Board.
- Distinguish technical workspace setup from scholarly writing/delivery gates.
