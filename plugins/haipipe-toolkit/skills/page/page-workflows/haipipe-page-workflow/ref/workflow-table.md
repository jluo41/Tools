...
| `scratch` | `Page.interactive-writing.scratch` | human | rough thinking for one Section, Subsection, or whole paragraph group; no B/symbol target | non-empty human Summary on Finish | `SELF`, `CLOSE`, `NEW_RUN` | `0..N` per target; closed Run immutable | Draft Scratch + Runtime | `OUTLINE/SCRATCH` |
