...
- The Structure Run no longer writes or reviews `outline/<stem>-logic.mmd`. Its review projection
  is the Structure card Draft Space reads from the plan's `## C<n>` / `### C<n>.P<m>` headings.

## 0.47.0 · 2026-09-22

- Renamed from `haipipe-page-outline` and moved to `skills/page/workflow-runs/`: the skill is named
  by its Run, `rp-struct-NN`. Scratch moved out to its own skill, `haipipe-page-scratch`.
...
- An older-grammar plan is rewritten as `v0.<k+1>` while no `v1.0` exists
  ...
  (haipipe-workbench-page 0.38.0, plan-grammar §6).
...
- Require the frozen `haipipe-page-context` PREPARE record before SHAPE or
  SURVEY and present it through the shared Outline workbench's Context Workspace.
...
- Route an approved, fully folded plan to `haipipe-page-writing` instead of the
...
- Define the Outline UI as Bullet Workspace + Evidence Workspace, with Plan
  Context and Page Records as subordinate groups rather than peer workbenches.
...
- Make the Outline workbench the one review surface for Shape, Evidence Items,
...
- **Head style rule in the checklist** (JL 260831): heads of 4 to 11 plain
  ...
  `haipipe-workbench-page` §✂️ for the approved SM00 v3 shape.

## 0.11.2 — 2026-08-31

- **⓪ REQUIRE** (JL: "add the fn for the haipipe-page-structure as well … so it
...
- The THREADS and LOG boxes carry haipipe-workbench-page 0.18.0's record
...
- **⓪ COLLECT**: the fold gains a second source. Every OUTLINE pass opens by
  ...
  (JL 260831: a function of this phase, not a new workbench). ① COVERAGE gains
...
template，然后 haipipe-page-structure 目的就是想要讲什么样的 story arcs."
...
- **The Aims enter the authority test**: agreed at OUTLINE, living in the
  plan file with `Done when:` tests (haipipe-workbench-page 0.14.0 owns the
  grammar); plus a pointer to the workbench's §✂️ bullet grammar, stated once
  there.
...
- **Coherence sweep (260819)**: §🔀 routes the four-pass plan through the
  ...
  Capitalized per the workbench's §✂️ bullet style; §📦 counts five marks, not
  six (✅ retired).
...
haipipe-page-structure · Changelog
================================
...
`haipipe-workbench-page` 0.1.0, which had ruled in these words that *"the answer
...
- The deliverable is `<page>/outline/<stem>-outline-v<N>.md`; its shape,
  addressing and marks stay in `haipipe-workbench-page` and are not restated.
