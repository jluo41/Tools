...
# /haipipe-page-structure · one Structure Run with SHAPE, then SURVEY
...
haipipe-page
  ...
  → haipipe-workbench-page/ref/review-packet.md (only for a human review or approval)
  → haipipe-run + selected workers (SURVEY inventory only; no dispatch)
```

The Page surface has already installed `haipipe-workbench-page` as the shared
presenter. The refs above are this Run's schema/material dependencies; the
...
The reader sees these authorities through one Outline workbench with three minimal
workspaces: **Draft Space** for the structure list and Bullet/Draft table,
...
Bullet, using the Structure list's flow, paragraph move, neighboring Points, and
...
workbench. Published Page Content and PDF remain unchanged until CONTENT promotion.
...
- **Trust the plan's `Answered:` and `Drawn:` lines as written.** Re-read only
  ...
  this Run again, never a quiet correction.
- **In session or as an agent, the trace is the same**: the plan file (or the
  table), the log record, the receipt. `haipipe-page-structure-agent` runs the
...
new or empty paragraphs); this Run owns everything the text cannot do, namely
Bullets, Point roles, paragraph jobs and re-addressing a paragraph that has
points. There is no separate map file to generate or keep fresh; the plan is
the only structure authority.
...
load `haipipe-workbench-page/ref/review-packet.md` and give the human its
...
Board route, always return both direct workbench views: **Draft Space**
...
The Outline workbench's Evidence Space joins the authored Item contract with
...
```text
haipipe-page-structure/
├── SKILL.md            this Run: the SHAPE and SURVEY cycles
└── CHANGELOG.md        version history, and the only home for what this Run used to say
```

Owns no scripts. The base is `haipipe-page`; the folder, the tab, the plan
grammar and the item table's grammar are `haipipe-workbench-page`'s
(`ref/plan-grammar.md`, `ref/item-table.md`, `ref/record-shape.md`,
`ref/specimen-section-plan.md`); the Page loop and receipt law are
`haipipe-page-workflow`'s; the six-field card of every Run is
`../../haipipe-page-workflow/ref/Run-cards.md`; the next Run is
`haipipe-page-evidence` (LAND, EMBED), and after the plan exits,
`haipipe-page-writing`. The design page is `QPw1-outline` on
