...
haipipe-page
  ...
  → haipipe-workbench-page/ref/plan-grammar.md (EMBED only)
  → haipipe-workbench-page/ref/evidence/values.md | citations.md | displays.md
    (LAND only, select the item's exact type contract)
  → haipipe-run
  → the exact Supporting Run workers selected by SURVEY
  → the renderer craft selected by haipipe-workbench-page for DISPLAY
```

The Page surface already installs `haipipe-workbench-page` as the shared
...
2. **Resolve Supporting routes.** Work only the routes SURVEY selected:
   ...
   also be reused under `haipipe-workbench-page/ref/item-table.md`'s qualified
