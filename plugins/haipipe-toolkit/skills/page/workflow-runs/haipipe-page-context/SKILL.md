...
description: >-
  The Context Run of a Board Page (`Page.context`, the first row of the Page workflow's Run table). PREPARE collects, resolves, and
  ...
  Context record inside the shared Outline workbench. Use before SHAPE, whenever
  the governing inputs changed, or when another Run Spec reports missing or
  conflicting context. Trigger: page context, context Run, PREPARE context,
...
Enter through `haipipe-page` and `haipipe-page-workflow`, then load this Run,
the Folder-owning workflow or canonical family skill, the exact Page Face
owner, and
`haipipe-workbench-page/ref/record-shape.md`, in that canonical order. The
Outline workbench keeps the generated record in the off-stage Markdown process
...
Workbench or Level-4 Run. Its output is presented in
`haipipe-workbench-page` beside the Page's other process records:

```text
haipipe-page-context      owns PREPARE and the generated context record
haipipe-workbench-page   owns the three reader Spaces; Context stays off-stage
...
2. Read the Folder owner and Page Face owner contracts, if applicable. The
   Page Face owner is the exact workflow-Run, canonical family, or legacy
...
Page Face owner; Venue is a library, not a paper-journey Run. After the exact owner
...
Resolve the five facts later Runs need:
...
- the current plan/evidence binding in a way that changes what later Runs
...
- Do not create `haipipe-workbench-context`. Context is an internal workspace of
