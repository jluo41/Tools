...
║                                           ║  ║                across these runs?"       ║
║                                           ║  ║                                          ║
║  artifacts:   code (*.py)                  ║  ║  artifacts:   experiment.yaml             ║
...
║               task-log.md (derived)        ║  ║                                          ║
║                                           ║  ║                                          ║
║  location:    examples/Proj-X/tasks/...    ║  ║  location:    examples/Proj-X/experiments║
║                                           ║  ║                                          ║
...
─────────────────                       ──────────────
 experiment.yaml                         tasks/A01_*/
   ...
   ③ result aggregate (D reads C via arms[] pointers):                  ││
        ...
        write result: block in experiment.yaml                           │
                                                                         │
...
[t=1]  📐 /haipipe-experiment design new E02
         ...
         │
[t=2]  🌉 /haipipe-experiment bridge E02
         │   for each arm:
...
[t=4]  🔗 /haipipe-experiment design link E02 <run-path>
         ...
         │   appends the run-path to the correct arm in experiment.yaml
         │
[t=5]  📊 /haipipe-experiment result aggregate E02
         │   scans arms[*].metrics.json
         │   computes mean / std / paired-t / sign-test
         │   writes result: block in experiment.yaml
         │     status: pending → confirmed | inconclusive | refuted
         │
[t=6]  🔍 /haipipe-experiment review E02
         │   structural QA: arms ≥1, metric set, caveats present, ...
         │   Codex semantic verdict (out-of-family review)
         │   writes review.md + CLAIMS_FROM_RESULTS.md
         │
[t=7]  📝 /haipipe-experiment result claim E02
         │   final 1-2 sentence statement with stats + caveats
         │   "Treatment X beats baseline by Δ ± std (p=Y, N=3). +caveat."
         │
[t=8]  🗺️  /haipipe-experiment explore (optional)
         │   coverage map across all experiments
         │   proposes next experiment to ask
         │
...
│   │   ├── experiment.yaml                 ← steering state (this thread)
│   │   ├── review.md                       ← latest QA
...
└── paper/                                  📰 F_paper — writing
    └── Paper-X-icml/...                    (consumes experiments' claims)
```
...
A: tasks/ — specifically a `display`-type task-folder (C-series).
   ...
   The experiment then references the plot via `evidence:` in
   experiment.yaml. The CLAIM (in plain English) lives in the
   experiment; the PLOT lives in a task.

**Q: Can one task be referenced by two experiments?**
A: Yes. Tasks are atomic. Experiments compose. E.g., one
...
**Q: Can one experiment span multiple projects?**
A: Not in this design. `examples/Proj-X/experiments/` is project-
   scoped. Cross-project comparison happens in a higher-level
   skill (F_paper, or a meta-experiment), not in experiment.yaml.

**Q: What about exploratory runs that aren't part of any experiment?**
A: Fine — they live in tasks/ alone, unlinked. No requirement to
   create an experiment for every run. Experiments exist for runs
...
A: When the run's purpose is "verify the algorithm class doesn't
   ...
   typically isn't linked to any experiment.

**Q: experiment.yaml has a `result:` block with numbers. Isn't that
     a result file then?**
A: No — those numbers are AGGREGATED REFERENCES to per-run metrics.
   ...
   `metrics.json`. If you regenerate the experiment.yaml result
   block, you'd read the same per-run metrics and recompute.

**Q: Where do daily notes about an experiment go?**
A: `experiments/<NN>_<slug>/logs/<YYYY-MM-DD>.md`. Append-only,
...
D_experiment overall            D_experiment/haipipe-experiment/SKILL.md
D_experiment bridge skill       D_experiment/haipipe-experiment-bridge/SKILL.md
D_experiment.yaml schema        D_experiment/ref/experiment-yaml-schema.md
D_experiment caveats checklist  D_experiment/ref/experiment-caveats-checklist.txt
D_experiment legacy migration   D_experiment/ref/_legacy-scope-expmt.md
...
- New code? → tasks/  (never experiments/)
- New claim? → experiments/  (never tasks/)
- New plot? → tasks/display/  (referenced by experiments/<X>/experiment.yaml evidence:)
- New hypothesis? → experiments/<NN>_<slug>/experiment.yaml hypothesis: field
...
- New cross-run statistic? → experiments/<X>/experiment.yaml result: (via /result aggregate)
- New "this didn't work, here's why" narrative? → experiments/<X>/logs/<DATE>.md
- New CGM trace for one patient? → tasks/individual/  (never experiments/)
