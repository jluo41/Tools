---
name: haipipe-task-digest
description: "Utility verb. Digests a session -- the CURRENT one, or a PAST session named/id'd as an argument (run from a fresh session) -- scanning its transcript for tool/skill feedback (gripes + preferences), distilling discrete items, deduping (merge-or-create), and after a MANDATORY confirm gate routing each through the feedback router into domain-folder inboxes. The bulk harvester for /haipipe-task feedback. Never auto-files."
...
3. DEDUP each candidate, and infer its target via the feedback router. ROUTING
   ...
   (fn/feedback.md resolve step 0), so a spine-wide habit like "every stage must
   mirror the IPO shape" is caught by the guard / the Scope rule below and is
   NEVER keyword-routed to 2_nn just because it mentions a model. Route by the
   complaint's SUBJECT, not by a skill-name or path it MENTIONS: "add feedback to
   /haipipe-task-for-fit that the figure must follow the predefined display
   plan" is about DISPLAY (the subject -> 7_display), not fit (a mis-named
   destination).
...
4. PRESENT the candidate list for the MANDATORY confirm gate. Group by target
   ...
   on its own (e.g. two display files both about "broken .tex from the renderer").
   You: approve / edit text / re-route / drop each item. NOTHING is written
   before you confirm. (This is the same illuminate->confirm discipline the
   lifecycle stages use; a session sweep WILL mis-read, so the gate is
   non-negotiable.)
...
6. REPORT what landed where: "N new, M merged, K dropped" (dup-in-batch items
   ...
   `/haipipe-task feedback list` to review the result.
```
...
```
digest files ONLY skill-feedback into domain-folder inboxes. Some session
remarks are GLOBAL BEHAVIORAL preferences (how the agent should act across ALL
sessions, not a task-skill defect). Do NOT file those -- FLAG them in the report
and suggest you `/remember` them. One job per verb.
  skill defect    "the figure renderer emitted a broken .tex"   -> inbox
...
- drop transient:  one-off instructions and task-work talk are excluded.
