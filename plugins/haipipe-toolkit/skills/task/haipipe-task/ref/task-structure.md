...
====================
  ...
  *.py            one or more Python scripts (freestyle naming, # %% cell format)
  ...
  QA/             OPTIONAL: the task-folder's READABLE digests — QA/<n>-<slug>.md, <n> = creation order, so `ls QA/` IS the index (no INDEX file). One file per DIRECTION this task has explored: `# Q — <question>` + `## Answer` (with [→ results/<file>] anchors) + `## Caveats` + `## Not-done`. Write-once; a later question ADDS QA/<n+1>-<slug>.md. Written at Report, by THIS layer, for one of three reasons only (a question arrived · results/ already answered one but no digest existed · a finding was judged worth digesting) — a QA/ mirroring every result is noise. Slug only: no external id, and no vocabulary this layer could not have produced (no claim ids, no "the paper"). Contract: fn/qa.md.
