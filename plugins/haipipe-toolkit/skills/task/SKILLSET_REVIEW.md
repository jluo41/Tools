...
- [x] **B6** 🔴 `[M]` Step 3b routes scaffolds to verbs this skill no longer has: `:263` `Skill("haipipe-task", args="project ...")` (project scaffolding moved to /haipipe-project, as :51 and :73 themselves say) and `:265` `Skill("haipipe-task", args="task-group ...")` (no `task-group` verb in Commands). Fix: route project to `/haipipe-project`; either add a `task-group` verb (fn/task-group.md exists) or route to the right owner. **FIXED: project → /haipipe-project; group → new task-group verb.**
- [x] **B7** 🟡 `[J]` `fn/task-group.md` + `fn/scan-status.md` (+ `ref/scan_status/`) were received from the project layer on 2026-07-03 but are wired nowhere: no Commands verb, no dispatch-table row. Decide: expose as verbs (`/haipipe-task task-group ...`, `/haipipe-task scan-status`) or fold their content elsewhere. **FIXED: task-group + scan-status wired as verbs (Commands, dispatch table, Step 2 cascade).**
...
- [x] **D1** 🟡 `[J]` `7_display/haipipe-task-for-display/SKILL.md:45,69` + `fn/scaffold.md:67-68` name paper-layer skills as "useful adjacent skills" (`/haipipe-paper-display-figure`, `/haipipe-paper-display-illustration-gemini`), and recommend the gemini FALLBACK renderer for diagram-style figures (family default is the diagram/illustration renderers). Recommend: drop the paper-skill mentions (output purpose "paper figures" stays, skill names go). Decide. **FIXED: paper-skill names removed from SKILL.md + fn/scaffold.md; output purpose kept.**
