...
These agents are used by `haipipe-task` (single task lifecycle via task-lifecycle.workflow.js) and by all type specialists (`haipipe-task-for-*`) that invoke them during their scaffold flow. That's why they live at `task/agents/` (shared), not inside any single skill.
