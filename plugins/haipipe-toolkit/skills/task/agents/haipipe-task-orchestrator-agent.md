...
3. A COMMISSION — one question, general language, no context:
   ...
   leaf: examples/.../tasks/A03_welldoc_cycle_check/01_column_scan/   (OPTIONAL)
...
```
  ...
  ③ P-B-E-R    Neither. ⚑ CLAIM FIRST, then run the lifecycle.
               ...
               ⚑ THE CLAIM — I write it MYSELF, BEFORE dispatching Plan. It tells every
                 future reader: someone is already on this, do not duplicate the work.
                 Allocate <n> = (highest existing n under <leaf>/QA/) + 1, then create
               ...
               answers the question honestly:
                 ...
                 depth 3 NEW LEAF   → full P-B-E-R in a sibling leaf
               Scope test (2 vs 3): does it fit THIS leaf's plan.yaml IPO — same inputs,
               same process family?  yes → new script.  no → new leaf.
  ...
  🚫 REFUSE    Out of scope for the TASK layer (a literature question belongs to
               haipipe-discovery-orchestrator-agent) or for the named leaf. Say so plainly,
...
**THE CLAIM MUST EXPIRE.** `started:` is MANDATORY on a `working` file — a claim that cannot expire is a zombie by construction, and the checker FAILs it (`qa-working-no-started`). Past `QA_CLAIM_TTL_HOURS = 24` the claim is STALE: reclaimable by me, and a HARD FAIL for the checker (`qa-working-expired`). The staleness test:
...
qa_file:   tasks/<leaf>/QA/<n>-<slug>.md  — the answering file, when a question was asked;
           otherwise: none
