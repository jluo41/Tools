...
**Partition Mode (large datasets):**
  ...
  Notebook: set `NUM_PARTITIONS=20` in the parameters cell
  Output: `2-RecStore/{name}/@i1n20/` through `@i20n20/`
  Memory: ~30GB per partition vs 120GB+ full (MIMIC-IV scale)
...
**Partition Mode (follows RecordSet partitions):**

  Case auto-discovers RecordSet partitions and processes each independently.
  ...
  Output: `3-CaseStore/{RecSet}/@i{i}n{n}/@v{ver}CaseSet-{Trigger}/`
...
**Multi-partition Mode (merges CaseSet partitions):**
