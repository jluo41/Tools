...
- [ ] `ref/workflow-plan-sample.yaml` internal steps (Setup → Execute → Parse) are too generic — every task could be described this way.
- [ ] Zero task instances in any project. Never been used end-to-end.
- [ ] Possible overlap with the `insight` and `design` families, which already orchestrate LLM-driven sessions with plans and gates. Is `for-agent` a simpler version of the same thing, or a genuinely different scope?
      ⚠️ PREMISE DEAD (2026-09-20) — this item compared against `haipipe-application-ask` and "DIKW cards". Neither exists, and the whole `application/` bucket is retired: Insight is `skills/insight/`, Design is `skills/design/`.
- [ ] Decision needed: update to be useful, or remove; no `application` family remains to own agent-driven tasks.
