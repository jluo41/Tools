---
name: haipipe-subject-inference-report
description: "Per-subject prediction-interpretation report. Loads one subject's data + recent CGM, hits the deployed prediction endpoint, then asks Claude (via claude_agent_sdk) to compose a dual-layer Report — structured JSON + natural-language text — for a specified audience persona (patient / clinician / etc.). Builds on haipipe-subject-inference (which provides the load+predict). The persona library (system prompt, schema, safety rules, tone) is pluggable: name a shipped persona or pass an absolute path to your own. Use to test how a deployed endpoint's prediction reads to a real audience, or to generate reports for downstream LLM-judge / doctor evaluation. Trigger: subject report, prediction interpretation, generate patient message, /haipipe-subject-inference-report."
argument-hint: --subject <id> --persona <name_or_path> [--endpoint-url URL] [--model X]
allowed-tools: Bash, Read
---

Skill: haipipe-subject-inference-report
========================================

Per-subject prediction → interpretation → audience-tailored report.

```
  📥 subject data        🌐 endpoint prediction       🤖 LLM compose
...
Sibling progression in `5_subject/`:

| Skill | Adds | Output |
|-------|------|--------|
| `haipipe-subject` | (data load only) | ctx dict |
| `haipipe-subject-inference` | + payload + POST | forecast JSON |
| `haipipe-subject-inference-report` | + persona + LLM | Report{json, nl} |
...
scripts/
  make_report_cli.py    end-to-end CLI: subject + persona → report
...
```
python Tools/plugins/haipipe-toolkit/skills/5_subject/haipipe-subject-inference-report/scripts/make_report_cli.py \
    --subject Subject-18 \
    --persona patient-friendly
```

Output: `_WorkSpace/7-AgentWorkspace/reports/<subject_id>/<persona>/<ts>/`
```
...
```xml
<report>
  <basics>{subject_id, dataset, gender, year_of_birth, disease_type}</basics>
...
- `haipipe-subject-inference` for `load_patient_ctx`, `build_payload`, `client.call_predict`
