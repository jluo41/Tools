"""Build an Endpoint_Set dataframe_records payload from a subject path.
...
def build_payload(
    ...
    *,
    ...
    """Subject path/id → Endpoint_Set dataframe_records payload."""
    ctx = load_patient_ctx(subject, cgm_tail=history_len)
    cgm = ctx["tables"].get("CGM")
    if cgm is None or len(cgm) == 0:
        raise ValueError(f"No CGM data for subject {subject!r}")
