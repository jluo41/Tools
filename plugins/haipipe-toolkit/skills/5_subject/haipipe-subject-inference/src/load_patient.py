"""Load a single subject's context from _WorkSpace/A-User-Store.

Subject layout (haipipe-subject contract):
...
def resolve_subject_path(subject: str) -> Path:
    ...
    if p.is_absolute() and p.exists():
        return p
    candidate = USER_STORE / subject
    if candidate.exists():
        return candidate
    matches = list(USER_STORE.glob(f"UserGroup-*/{subject}"))
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        raise ValueError(f"Subject id {subject!r} is ambiguous: {matches}")
    raise FileNotFoundError(f"Cannot resolve subject {subject!r} under {USER_STORE}")


def load_patient_ctx(subject: str, cgm_tail: Optional[int] = None) -> dict:
    """Load a patient's context dict.

    Args:
        subject: subject path or shorthand (e.g. 'UserGroup-WellDoc2022CGM/Subject-18',
                 'Subject-18', or absolute path).
        cgm_tail: if set, keep only the most recent N CGM rows (sorted by time).

    Returns:
        dict with keys: subject_path, manifest, tables (DataFrame per table).
    """
    subject_dir = resolve_subject_path(subject)
    src = subject_dir / "1-SourceStore"
    if not src.exists():
        raise FileNotFoundError(f"No 1-SourceStore at {subject_dir}")

    manifest = {}
    mf_path = subject_dir / "manifest.yaml"
    if mf_path.exists():
