---
name: haipipe-subject-inference
description: "Per-subject endpoint inference test. Loads one subject from _WorkSpace/A-User-Store, builds an Endpoint_Set dataframe_records payload, POSTs it to a deployed endpoint URL (local FastAPI / Databricks / SageMaker — same wire contract), and prints the forecast response. Sibling of haipipe-subject (which loads data only). Use to smoke-test that a deployed endpoint accepts subject data correctly. Trigger: subject inference, test endpoint with subject, /haipipe-subject-inference."
argument-hint: --subject <id_or_path> [--endpoint-url URL] [--json]
allowed-tools: Bash, Read
---

Skill: haipipe-subject-inference
=================================

Single-subject inference smoke test against a deployed Endpoint_Set.

Pipeline:

```
  Subject-XX/                                            (haipipe-subject layout)
...
scripts/
  show_ctx_cli.py            inspect a subject's loaded context
  test_subject_predict.py    end-to-end: subject id → POST → forecast
```
...
In another shell, hit it with a subject:

```
python Tools/plugins/haipipe-toolkit/skills/5_subject/haipipe-subject-inference/scripts/test_subject_predict.py \
    --subject Subject-18
```
...
CGM_ENDPOINT_URL=https://<workspace>/serving-endpoints/<name>/invocations \
    python ...test_subject_predict.py --subject Subject-18
```
...
| `ValueError: Cannot resolve subject` | `--subject` shorthand matched 0 or >1 paths | Use full path or `UserGroup-X/Subject-Y` |
| `ValueError: No CGM data` | Subject parquet empty/missing | Check `1-SourceStore/CGM.parquet` exists and has rows |
...
- Loads the subject layout produced by `haipipe-subject` (sibling skill).
