"""End-to-end demo: subject path → endpoint POST → forecast.

Two-step agent flow (no LLM yet):
  1. agent-side: build dataframe_records payload from subject path
  2. agent-side: HTTP POST to endpoint_url → forecast

    python predict_cli.py --subject Subject-18
    python predict_cli.py --subject UserGroup-WellDoc2022CGM/Subject-26 --json
    CGM_ENDPOINT_URL=http://my-databricks-host/invocations python predict_cli.py --subject Subject-18
"""
