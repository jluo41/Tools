---
name: haipipe-subject-inference-judge
description: "LLM-as-judge evaluator for prediction-interpretation Reports. Consumes the report.json produced by haipipe-subject-inference-report and scores it against a rubric persona (e.g. patient-comprehension, safety-review) via claude_agent_sdk. Each judge persona defines its own rubric dimensions, scoring rules, and verdict thresholds — pluggable, can live outside the plugin. Builds on haipipe-subject-inference-report. Use to score reports in CI batches, doctor pre-review, persona ablation studies, or red-team safety checks. Trigger: judge a report, score interpretation, evaluate report, /haipipe-subject-inference-judge."
argument-hint: --report-dir <path> --persona <name_or_path> [--model X]
allowed-tools: Bash, Read
---

Skill: haipipe-subject-inference-judge
=======================================

LLM-as-judge for the dual-layer Reports produced by
`haipipe-subject-inference-report`. Same SDK + persona pattern, applied
...
| Skill | Adds | Output |
|-------|------|--------|
| `haipipe-subject` | (data load) | ctx |
| `haipipe-subject-inference` | + payload + POST | forecast |
| `haipipe-subject-inference-report` | + audience persona + LLM | Report |
| `haipipe-subject-inference-judge` | + judge persona + LLM | **Judgment** |
...
```
python Tools/plugins/haipipe-toolkit/skills/5_subject/haipipe-subject-inference-judge/scripts/judge_report_cli.py \
...
- `haipipe-subject-inference-report` produces the input (`report.json`)
...
- Pattern reference: `Physician-SPACE/.../A3_cross_family_judge/run_sdk_judge.py`
  ...
  was first proven; this skill adapts it to the haipipe per-subject layer)
...
- per-batch driver (separate skill, not under 5_subject) for cohort eval
