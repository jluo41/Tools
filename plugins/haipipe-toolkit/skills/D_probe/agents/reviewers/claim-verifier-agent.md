...
description: "REVIEWER agent for D_probe. Codex-backed semantic verdict: does the experimental evidence support the intended claim? Returns yes|partial|no + confidence + what-is/isn't-supported + missing evidence + suggested revision + next probes. Auto-runs integrity first if no recent audit; integrity=fail blocks it. The author of the claim (haipipe-probe-result) never verifies it — builder != judge. Writes CLAIMS_FROM_RESULTS.md. Trigger: claim verdict, does evidence support, supports?, /haipipe-probe review claim."
tools:
  - Read
  - Grep
  - Glob
