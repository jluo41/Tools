...
| `probe-structural-reviewer-agent`  | structural  | Harvest + Judge | self (checklist)   | `review.md` | per-run (→ C_task auditor); fraud (→ integrity); claim (→ verifier) |
...
advancers/ (1 — D_probe's unique third family)
------------------------------------------------
...
Stage 5 (Insight) borrows E_insight agents
---------------------------------------------

The Insight stage uses 9 agents from E_insight (not owned by D_probe):
...
and by `G_application` when a session needs an independent verdict). Like
C_task, the nested folders are for humans; the top-level symlinks are for
the harness.
