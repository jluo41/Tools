...
│  Custom nn.Module: also algorithm_*.py (see haipipe-nn-1)       │
├─────────────────────────────────────────────────────────────────┤
│  Layer 1: Algorithm                                             │
│  Two forms:                                                     │
...
Step 1: Algorithm (Layer 1)
   ...
   [ ] Custom nn.Module (adds embeddings, fusion, etc.): create
       ...
       See haipipe-nn-1-algorithm for the algorithm_*.py pattern.
...
display_df(pd.DataFrame([
    ...
    {'step': '3. Create [object]',    'status': 'PASSED', 'key_metric': class_str,    'artifact': domain_str},
    {'step': '4. Prepare data',       'status': 'PASSED', 'key_metric': shape_str,    'artifact': str(sample_input_row)},
    {'step': '5. Fit / Fwd pass',     'status': 'PASSED', 'key_metric': metric_str,   'artifact': 'is_fitted=True'},
    {'step': '6. Infer / Grad flow',  'status': 'PASSED', 'key_metric': infer_str,    'artifact': str(sample_output_row)},
...
**Step 5 key_metric is family-specific** — show the most meaningful training signal:
