...
description: "Utility verb. Digests a session -- the CURRENT one, or a PAST session named/id'd as an argument -- scanning its transcript for feedback about the learn-databricks lessons/journey docs (gaps, wrong info, outdated instructions, missing lessons), distilling discrete items, deduping, and after a MANDATORY confirm gate routing each through feedback capture. The bulk harvester for /learn-databricks feedback. Never auto-files."
...
feedback you gave about the LEARNING MATERIAL (lesson gaps, wrong instructions,
outdated concepts, missing lessons), dedups them, and (after you confirm) routes
each into the feedback/ inbox.
...
1. SCAN the target session for LEARNING-MATERIAL feedback signals:
     KEEP  - "this lesson is wrong/outdated/missing X"
           - "the journey doc doesn't cover Y"
           - "the instructions didn't work because Z"
           - "we should add a lesson about W"
...
6. REPORT as an ITEMIZED LIST of every item produced:
   ...
   Then close with the tally and "review: /learn-databricks feedback list".
```

## Scope: learning-material feedback only

```
digest files ONLY feedback about lesson/journey content into the inbox.
Global behavioral preferences (how the agent should act) go to PREFERENCES.md.
  lesson defect  "lesson 04 doesn't mention the --no-cache workaround"  -> inbox
