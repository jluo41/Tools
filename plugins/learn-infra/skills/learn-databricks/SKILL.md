...
description: "Databricks-specific lessons and knowledge base. 10 hard-won lessons from the REACH-ADHD deployment (VM stockout, init scripts, env vars, pandas compatibility, partition mismatches, etc.) plus the full deployment journey doc. Use when running pipelines on Databricks, debugging cluster/notebook issues, or onboarding. Verbs: lesson (capture/list/search gotchas), feedback (lesson doc gaps), digest (bulk harvest from session). Trigger: databricks, cluster issue, notebook, dbutils, pipeline on databricks, /learn-databricks."
...
----------------
  ...
  lesson/     10 hard-won lessons from the REACH-ADHD Databricks deployment
