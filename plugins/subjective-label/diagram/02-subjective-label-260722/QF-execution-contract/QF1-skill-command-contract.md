...
#### 1.1 · Initialization
`/sl-init` validates the corpus, reserves sealed-test identifiers, creates embeddings for retrieval, initializes artifacts, and prepares the random Round 1 batch.
It does not require an objective function or a pre-existing label geometry.

#### 1.2 · Calibration Round
`/sl-round` is resumable across candidate, prelabel, session, and checkpoint phases.
...
`/sl-evaluate` requires a QD4 stop and frozen G*, then creates late human gold and scorecards without optimization.
`/sl-complete` requires an eligible production policy, labels the remainder, routes risk, and closes only after final audit.
...
### A2 · 🔁 Compatibility aliases
- ✅ A2.1 · Iterate, validate, and scale are thin aliases that reject retired semantics.
...
- `../../skills/sl-init/SKILL.md`
  Initialization must reserve the seal and prepare a random Round 1.
- `../../skills/sl-round/SKILL.md`
  The canonical Calibration Round from C_t through closed D_t and G_t.
- `../../skills/sl-evaluate/SKILL.md`
  The sealed final-test and executor-scorecard command.
- `../../skills/sl-complete/SKILL.md`
  The validated production, reconciliation, and final-audit command.
- `../../skills/sl-status/SKILL.md`
  The read-only state and gate dashboard.
- `../../skills/sl-iterate/SKILL.md`
  This becomes the compatibility alias for the canonical round contract.
- `../../skills/sl-validate/SKILL.md`
  This becomes the compatibility alias for sealed project evaluation.
- `../../skills/sl-scale/SKILL.md`
  This becomes the compatibility alias for audited corpus completion.

### 🔗 Related Board Pages · what this Page READS BY SCOPE
- `constrained by · ALL` · [QA0 page](QA-semantic-contract/QA0-the-revised-conception.md)
