...
def deck(md: str) -> str:
    ...
    # ---- 2 the storyline --------------------------------------------------
    add("要做的6件事", "那要做哪 6 件事", pick("拿 license", "收尾"))

    # ---- 3 the engine pipeline, and where the 6 topics pin ----------------
    add("引擎全流程", "引擎完整跑一遍是什么样", pick("拿 license", "先摸底"))

    # ---- 4 built but never run --------------------------------------------
    add("已做到哪", "我们已经做到哪一步", pick("agreeableness"),
        "commit 1eb432f6 · 2026-07-21 23:28 · +1616 行 · 6 个 lib 自测全绿 · "
        "note-update Part 12: “NOT yet done = real runs”")

    # ---- 5 F1-F8 ----------------------------------------------------------
    add("0.93不算数", "但这个 0.93 不算数", pick("0.932", "0.667"))

    # ---- 6 盘面数字 --------------------------------------------------------
    add("出路", "出路：正确性只能从外面借", pick("per-rater", "天花板"))
    add("术语", "术语对照", pick("construct", "guideline", "battery"))
