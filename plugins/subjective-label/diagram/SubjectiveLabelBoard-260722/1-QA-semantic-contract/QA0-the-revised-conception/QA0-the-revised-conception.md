...
## Log
260816 · Took the Board family's 260816 shape rule: the seven group folders now carry their place in `## Pages` as a leading number, `1-QA-semantic-contract` through `7-QG-page-type`, so the folder listing and the board read in one order. The pages were already folded one-to-a-folder, so only the group level moved, and the deck that had been sitting in a board-level `slides/` went into its own page at `QG1-for-labeling/slide/`, where the 🎞 tab looks for it. Two dead links repaired at the same time: the board-design board numbered its own groups on 260816, which broke both of this board's citations into it, `QB4` and `QB6`. The board now checks clean.
