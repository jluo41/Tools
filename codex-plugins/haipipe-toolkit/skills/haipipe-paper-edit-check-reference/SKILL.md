...
use `/citation-verifier` or `/haipipe-paper-edit-manual-review-citations`.

Usage
=====

```
/haipipe-paper-edit-check-reference <paper-root-dir-or-root-tex>
```
...
- **Number accuracy** (does the prose number match the data source?) — use `/haipipe-paper-edit-claim-audit` or `/haipipe-paper-edit-manual-review-values`.
...
```bash
python3 Tools/plugins/research-toolkit/skills/06_write/haipipe-paper-edit-check-reference/check_refs.py \
...
|---|---|
| Find broken refs / orphan labels / phantomsection issues | **haipipe-paper-edit-check-reference** (this) |
| Verify a citation actually supports a claim | `/citation-verifier` |
| Verify a number in prose matches the data file | `/haipipe-paper-edit-claim-audit` or `/haipipe-paper-edit-manual-review-values` |
| Diagnose & propose paragraph-level revisions | `/paper-revise-section` |
| Compile & visually inspect the PDF | `/paper-compile` |
| Build a tracked-changes diff PDF | `/haipipe-paper-edit-diffpdf` |
