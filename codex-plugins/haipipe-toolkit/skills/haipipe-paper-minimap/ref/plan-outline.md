# Reference: Paper Outline Blueprint

> Merged into `haipipe-paper-minimap` (2026-06-22) from the retired
> `haipipe-paper-plan` skill. Reference material for the minimap stage:
> venue page-budget/citation constants, per-paper-type section skeletons, and the
> Claims-Evidence / Figure / Citation plan tables. Not a standalone skill.
---
...
This plan is downstream of the paper pitch. If `0-lifecycle/1-pitch/1-pitch.tex` exists,
...
- **TARGET_VENUE = `ICLR`** — Default venue. User can override (e.g., `/haipipe-paper-minimap "topic" — venue: NeurIPS`). Supported: `ICLR`, `NeurIPS`, `ICML`, `CVPR`, `ACL`, `AAAI`, `ACM`, `IEEE_JOURNAL` (IEEE Transactions / Letters), `IEEE_CONF` (IEEE conferences).
...
0. **`0-lifecycle/1-pitch/1-pitch.tex`** — current one-minute public-facing story for
...
- Apply standard writing principles when framing the one-sentence contribution, Abstract, Introduction, Related Work, or hero figure.
- Check the target venue's structure norms before freezing the outline.
...
Before committing to a structure, apply the narrative principle:
...
- The abstract, introduction, hero figure, and discussion/application paragraph
  ...
  `0-lifecycle/1-pitch/1-pitch.tex` with a logged reason.
...
## Pitch Alignment
- **Current pitch**: [from 0-lifecycle/1-pitch/1-pitch.tex]
...
- **Preserve the pitch** — `PAPER_PLAN.md` must not create a different public-facing story from `0-lifecycle/1-pitch/1-pitch.tex`; if the evidence or venue requires a different story, update the pitch first and append `PITCH_LOG.md`
...
> When this blueprint drives an actual output file, write a timestamped version
> first then copy to the fixed name, and respect the project's language setting.
